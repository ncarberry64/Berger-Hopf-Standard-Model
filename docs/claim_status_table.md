...
| Neutral boundary tensors `chi_nu_AB`, `lambda_nu`, `neutral_boundary_condition` | `OPEN_LOCALIZABLE` | `theory/theorem_discharge_neutral_boundary_tensors.md` | Must not be claimed until the explicit scalar/topographic boundary action, variation convention, internal/subsurface metric, neutral profile, explicit boundary-condition derivation, and positivity/stability proof are derived before comparison |
