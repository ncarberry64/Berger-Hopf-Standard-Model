...
Candidate channel space: `Hom(V_u,V_d) direct_sum Hom(V_d,V_u)`.

Dimension count: `dim(V_u)=2`, `dim(V_d)=4`, one-way `2*4=8`, adjoint direction `4*2=8`, bidirectional adjoint-pair `N_CKM=16`.
...
The source audit locates CKM charged-current target evidence with a Hermitian-conjugate term, but the located provenance still labels the target as a convention-level object rather than a normalized BHSM action derivation.
