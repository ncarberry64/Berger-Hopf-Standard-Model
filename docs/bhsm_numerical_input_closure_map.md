...
The full freeze protocol / charged `K_f` sprint records the derive -> freeze -> predict -> compare rule and implements a deterministic candidate generator for sector projectors, `Omega(C,sigma)`, `A(C,sigma)`, `T(C,sigma)`, the boundary graded defect equation, charged `rho_ch in {1,2,3}` branches, exact charged suppression fractions, minimal real tridiagonal charged `K_f` matrices, and an operator-level `(ln 2)` insertion only on the up-sector `(6,0)` construction-basis slot. The statuses are candidate/conditional only: `Boundary_Graded_Defect_Theorem: STRONGLY_SUPPORTED_CANDIDATE`, `minimal_charged_Kf_generator: STRONGLY_SUPPORTED_CANDIDATE`, `Mode_Identity_Threshold_Readout_Theorem: STRONGLY_SUPPORTED_CANDIDATE`, `Z_virt_u1: DERIVED_CONDITIONAL`, `rho_ch_exact_value: OPEN_LOCALIZABLE`, `full_threshold_operator: OPEN`, `RG_transport: OPEN`, and `numerical_closure: OPEN`. No empirical comparison input, frozen prediction change, or official prediction change is introduced.
