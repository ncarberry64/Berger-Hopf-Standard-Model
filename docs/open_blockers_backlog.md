...
| `S_EFF_NU_NEUTRAL_ACTION_OPEN` | open `Delta_y_nu` dependency | Needed for the stationary-point formula. | Derive `S_eff^(nu)` from the neutral boundary/topographic action. | Insert an effective action chosen from neutrino residuals. |
