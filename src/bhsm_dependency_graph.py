...
def bhsm_dependency_nodes() -> tuple[DependencyNode, ...]:
    ...
    return (
        ...
        DependencyNode("scalar_decoupling", "scalar/topographic decoupling", FINITE_BASIS_SCAFFOLD, ("state_ontology",), ("v1.5 current proxy inventory has one Higgs projection and no current forbidden/open scalar modes.",), ("Full scalar action proof remains open.",)),
...
def smallest_open_obligations() -> tuple[str, ...]:
    ...
    return (
        ...
        "Prove scalar/topographic decoupling from the full internal action.",
