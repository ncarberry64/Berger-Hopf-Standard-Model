"""Scan both orientations of a bounded geometric probe from v19.07."""
...
VERSION = "v19.09"
CLASSIFICATION = "BHSM_N3_FOURTEENTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.797206261170734
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v19_09_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_fourteenth_bidirectional_merit_manifold_probe_v19_09.json"
    ).read_text(encoding="utf-8"))
    selected = payload["fourteenth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v19.09 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def fourteenth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_seventeenth_direct_residual_scale_audit_v19_08.json"
    ).read_text(encoding="utf-8"))
    measured = audit["seventeenth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v19.08 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v19_07_selected_raw_vector()
    ...
    return {
        "source_state": "v19.07_thirteenth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = fourteenth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v19_07": result["source_state"].startswith("v19.07"),
    ...
    return {
        "artifact": "BHSM_aether_n3_fourteenth_bidirectional_merit_manifold_probe_v19_09",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "fourteenth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_fourteenth_bidirectional_merit_manifold_probe_v19_09.json"
...
__all__ = [
    ...
    "v19_09_selected_raw_vector", "fourteenth_bidirectional_merit_manifold_probe",
