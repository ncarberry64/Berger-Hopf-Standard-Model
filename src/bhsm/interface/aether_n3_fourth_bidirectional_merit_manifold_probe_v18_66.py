"""Scan both orientations of a bounded geometric probe from v18.64."""
...
from bhsm.interface.aether_n3_third_bidirectional_probe_promotion_v18_64 import v18_64_selected_raw_vector
...
VERSION = "v18.66"
CLASSIFICATION = "BHSM_N3_FOURTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
...
def v18_66_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path("artifacts/BHSM_aether_n3_fourth_bidirectional_merit_manifold_probe_v18_66.json").read_text(encoding="utf-8"))
    selected = payload["fourth_bidirectional_merit_manifold_probe"]["selected_true_merit_candidate_pending_child_acceptance"]
    if selected is None:
        raise ValueError("v18.66 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def fourth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path("artifacts/BHSM_aether_n3_seventh_direct_residual_scale_audit_v18_65.json").read_text(encoding="utf-8"))
    measured = audit["seventh_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v18.65 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v18_64_selected_raw_vector()
    ...
    def direct_response(direction_y: np.ndarray, step: float = response_step) -> np.ndarray:
        ...
        if norm == 0.0:
            return np.zeros(376)
        unit = direction / norm
        finite = (_square_physical_residual(y + step * unit) - _square_physical_residual(y - step * unit)) / (2.0 * step)
        return norm * finite

    transform, transform_audit = _action_curvature_transform(raw)
    operator = LinearOperator((376, 376), matvec=lambda direction_x: direct_response(transform @ direction_x), dtype=float)
    ...
    response_consistency = float(np.linalg.norm(direct_response(unit, response_step) - direct_response(unit, comparison_step)) / max(1.0, np.linalg.norm(direct_response(unit, response_step))))
    trials = []
    eligible = []
        ...
        for backtrack in range(BACKTRACKS):
            ...
            try:
                ...
                row = {
                    ...
                    "action_curvature_coordinate_step_norm": float(unsigned_fraction * np.linalg.norm(direction_x)),
                    "physical_scaled_coordinate_step_norm": unsigned_fraction * direction_norm,
                    "raw_coordinate_step_norm": float(unsigned_fraction * np.linalg.norm(direction_y / scales)),
                    "eta_minimum": eta,
                    "metrics": metrics,
                    "complete_norm_reduction": initial["complete"] - metrics["complete"],
                    "raw_vector_hex": [float(value).hex() for value in candidate_raw],
                }
                row["true_merit_eligible"] = bool(eta > 1.0e-5 and row["complete_norm_reduction"] > MARGIN)
                trials.append(row)
    ...
    for orientation in ("positive", "negative"):
        ...
        best_by_orientation[orientation] = None if not rows else min(rows, key=lambda row: row["metrics"]["complete"])
    return {
        "source_state": "v18.64_third_bidirectional_probe_promoted_state",
        ...
        "linear_probe": {
            ...
            "relative_exact_linear_residual": float(np.linalg.norm(linear_residual) / max(1.0, np.linalg.norm(residual))),
...
def completion_payload() -> dict[str, Any]:
    result = fourth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v18_64": result["source_state"].startswith("v18.64"),
        "source_norm_reproduced": abs(result["source_complete_norm"] - 0.8115026784613) < 5.0e-12,
        ...
        "probe_only_not_newton_claim": response["used_only_as_local_geometric_probe"] and probe["never_promoted_as_newton_without_independent_validation"],
    ...
    solver_interpretation_valid = bool(probe["info"] == 0 and response["resulting_direction_relative_change"] < 5.0e-3)
    return {
        "artifact": "BHSM_aether_n3_fourth_bidirectional_merit_manifold_probe_v18_66",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "fourth_bidirectional_merit_manifold_probe": result,
        ...
        "real_physical_property_explained": "A_FRESH_BOUNDED_LOCAL_GEOMETRIC_PROBE_FROM_V18_64_LOCATES_EXACT_NONLINEAR_MERIT_STATES_WITHOUT_A_NEWTON_CLAIM",
        "dependency_advanced": "NONLINEAR_N3_EVENT_SADDLE_CLOSURE",
        "active_calculation": "RECONSTRUCT_AND_TEST_COMPETITIVE_SELECTED_COMPLETE_CHILD_STATES" if selected is not None else "REMEASURE_OR_GENERATE_THE_NEXT_BOUNDED_GEOMETRIC_PROBE",
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_fourth_bidirectional_merit_manifold_probe_v18_66.json"
    path.write_text(deterministic_json(completion_payload()), encoding="utf-8")
    return path


__all__ = ["VERSION", "CLASSIFICATION", "FULL_BHSM_COMPLETE", "v18_66_selected_raw_vector", "fourth_bidirectional_merit_manifold_probe", "completion_payload", "materialize"]
