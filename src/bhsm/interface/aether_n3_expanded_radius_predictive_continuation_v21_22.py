"""Use the validated v21.21 curvature once at the promoted v21.21 state."""
...
from bhsm.interface.aether_n3_eigenpair_curvature_expanded_radius_v21_21 import v21_21_selected_raw_vector
...
VERSION = "v21.22"
CLASSIFICATION = "BHSM_N3_EXPANDED_RADIUS_PREDICTIVE_CONTINUATION"
FULL_BHSM_COMPLETE = False


def v21_22_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_N3_EXPANDED_RADIUS_PREDICTIVE_CONTINUATION_V21_22.json"
    ).read_text(encoding="utf-8"))["expanded_radius_predictive_continuation"]
    if not payload["promotion"]["promoted"]:
        raise ValueError("v21.22 has no physically promoted state")
...
def completion_payload() -> dict[str, Any]:
    ...
    if not source_artifact["validation_passed"]:
        raise ValueError("v21.21 curvature is not validated")
    source_result = source_artifact["eigenpair_curvature_expanded_radius"]
    curvature_raw = v21_20_selected_raw_vector(); scales = kkt_variable_scales()
    ...
    schedule = source_result["dual_metric_model"]["radius_schedule"]
    result = dual_metric_range_space_proposal(
        v21_21_selected_raw_vector(), source_label="v21.21",
        curvature_override=curvature, radius_schedule_override=schedule,
    )
    result["dual_metric_model"]["curvature_artifact"] = (
        "SINGLE_PREDICTIVE_REUSE_OF_VALIDATED_V21_21_EIGENPAIR_CURVATURE"
    )
    best = result["exact_search"]["best"]
    validation = {
        "source_v21_21_reproduced": abs(
            result["source"]["exact_rayleigh_f376_l2"] - 0.781486218597499
        ) < 5.0e-12,
        "validated_curvature_reused_once": result["dual_metric_model"]["curvature_artifact"].startswith("SINGLE_PREDICTIVE_REUSE"),
        "same_25_point_history_owned_interval": len(schedule) == 25,
    ...
    return {
        "artifact": "BHSM_N3_EXPANDED_RADIUS_PREDICTIVE_CONTINUATION_V21_22",
        "version": VERSION, "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "expanded_radius_predictive_continuation": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_N3_EXPANDED_RADIUS_PREDICTIVE_CONTINUATION_V21_22.json"
    path.write_text(deterministic_json(completion_payload()), encoding="utf-8"); return path


__all__ = ["VERSION", "CLASSIFICATION", "FULL_BHSM_COMPLETE", "v21_22_selected_raw_vector", "completion_payload", "materialize"]
