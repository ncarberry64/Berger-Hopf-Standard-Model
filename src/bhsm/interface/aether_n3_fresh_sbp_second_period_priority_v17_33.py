"""Fresh period-priority continuation from the validated v17.31 state."""
...
VERSION = "v17.33"
CLASSIFICATION = "BHSM_N3_FRESH_SBP_SECOND_PERIOD_PRIORITY_MEASURED_TANGENT_FAMILY"
FULL_BHSM_COMPLETE = False


def v17_31_selected_raw_vector() -> np.ndarray:
    payload = json.loads(
        Path(
            "artifacts/BHSM_aether_n3_fresh_sbp_third_v0_priority_v17_31.json"
        ).read_text(encoding="utf-8")
    )
    values = payload["fresh_sbp_third_v0_priority"][
        "selected_v0_priority_maximin"
    ]["raw_vector_hex"]
    raw = np.asarray([float.fromhex(value) for value in values])
    if raw.shape != (376,):
        raise ValueError("v17.31 selected vector has wrong dimension")
    return raw


def second_period_priority() -> dict[str, Any]:
    return period_priority_family_from(
        v17_31_selected_raw_vector(),
        source_state="v17.31_selected_third_v0_priority_state",
        priority_owner="period",
        priority_key="period_priority",
        selection_key="selected_period_priority_maximin",
...
def completion_payload() -> dict[str, Any]:
    result = second_period_priority()
    best = result["selected_period_priority_maximin"]
    validation = {
        "v17_31_residual_reproduced": math.isclose(
            result["initial_metrics"]["complete"],
            1.025239795381184,
            rel_tol=0,
            abs_tol=2e-8,
        ),
        "v17_31_event_reproduced": math.isclose(
            result["initial_metrics"]["event"],
            0.088928305324622,
            rel_tol=0,
            abs_tol=2e-8,
        ),
        "v17_31_period_reproduced": math.isclose(
        ...
        "source_state_owned": (
            result["source_state"] == "v17.31_selected_third_v0_priority_state"
        ),
    ...
    return {
        "artifact": "BHSM_aether_n3_fresh_sbp_second_period_priority_v17_33",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": False,
        "fresh_sbp_second_period_priority": result,
        "status": "VALIDATED" if all(validation.values()) else "RECLASSIFIED",
        "real_physical_property_explained": (
            "FRESH_SIX_OWNER_DESCENT_WITH_PERIOD_PRECONDITIONING_AFTER_THE_"
            "V17_31_PERIOD_LIMITER_TRANSITION"
        ),
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_fresh_sbp_second_period_priority_v17_33.json"
