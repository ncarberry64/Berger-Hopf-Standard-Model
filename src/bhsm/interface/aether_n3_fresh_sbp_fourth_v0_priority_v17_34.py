"""Fresh v0-priority continuation from the validated v17.33 state."""
...
VERSION = "v17.34"
CLASSIFICATION = "BHSM_N3_FRESH_SBP_FOURTH_V0_PRIORITY_MEASURED_TANGENT_FAMILY"
FULL_BHSM_COMPLETE = False


def v17_33_selected_raw_vector() -> np.ndarray:
    payload = json.loads(
        Path(
            "artifacts/BHSM_aether_n3_fresh_sbp_second_period_priority_v17_33.json"
        ).read_text(encoding="utf-8")
    )
    values = payload["fresh_sbp_second_period_priority"][
        "selected_period_priority_maximin"
    ]["raw_vector_hex"]
    raw = np.asarray([float.fromhex(value) for value in values])
    if raw.shape != (376,):
        raise ValueError("v17.33 selected vector has wrong dimension")
    return raw


def fourth_v0_priority() -> dict[str, Any]:
    return period_priority_family_from(
        v17_33_selected_raw_vector(),
        source_state="v17.33_selected_second_period_priority_state",
        priority_owner="v0",
...
def completion_payload() -> dict[str, Any]:
    result = fourth_v0_priority()
    best = result["selected_v0_priority_maximin"]
    validation = {
        "v17_33_residual_reproduced": math.isclose(
            result["initial_metrics"]["complete"],
            1.022619475373664,
            rel_tol=0,
            abs_tol=2e-8,
        ),
        "v17_33_event_reproduced": math.isclose(
            result["initial_metrics"]["event"],
            0.088785177341884,
            rel_tol=0,
            abs_tol=2e-8,
        ),
        "v17_33_v0_reproduced": math.isclose(
        ...
        "source_state_owned": (
            result["source_state"] == "v17.33_selected_second_period_priority_state"
        ),
    ...
    return {
        "artifact": "BHSM_aether_n3_fresh_sbp_fourth_v0_priority_v17_34",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": False,
        "fresh_sbp_fourth_v0_priority": result,
        "status": "VALIDATED" if all(validation.values()) else "RECLASSIFIED",
        "real_physical_property_explained": (
            "FRESH_SIX_OWNER_DESCENT_WITH_V0_PRECONDITIONING_AFTER_THE_"
            "V17_33_V0_LIMITER_TRANSITION"
        ),
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_fresh_sbp_fourth_v0_priority_v17_34.json"
