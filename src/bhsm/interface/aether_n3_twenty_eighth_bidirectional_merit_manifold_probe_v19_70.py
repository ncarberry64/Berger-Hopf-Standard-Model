"""Scan both orientations of a bounded geometric probe from v19.68."""
...
VERSION = "v19.70"
CLASSIFICATION = "BHSM_N3_TWENTY_EIGHTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.788591183052825
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v19_70_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_twenty_eighth_bidirectional_merit_manifold_probe_v19_70.json"
    ).read_text(encoding="utf-8"))
    selected = payload["twenty_eighth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v19.70 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def twenty_eighth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_thirty_first_direct_residual_scale_audit_v19_69.json"
    ).read_text(encoding="utf-8"))
    measured = audit["thirty_first_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v19.69 has no common stable direct-response scale")
    ...
    return {
        "source_state": "v19.68_twenty_seventh_bidirectional_fallback_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = twenty_eighth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v19_68": result["source_state"].startswith("v19.68"),
    ...
    return {
        "artifact": "BHSM_aether_n3_twenty_eighth_bidirectional_merit_manifold_probe_v19_70",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "twenty_eighth_bidirectional_merit_manifold_probe": result,
        ...
        "real_physical_property_explained": (
            "A_FRESH_BOUNDED_LOCAL_GEOMETRIC_PROBE_FROM_V19_68_LOCATES_"
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_twenty_eighth_bidirectional_merit_manifold_probe_v19_70.json"
...
__all__ = [
    ...
    "v19_70_selected_raw_vector", "twenty_eighth_bidirectional_merit_manifold_probe",
