"""Scan both orientations of a bounded geometric probe from v19.17."""
...
VERSION = "v19.19"
CLASSIFICATION = "BHSM_N3_SIXTEENTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.795713884217715
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v19_19_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_sixteenth_bidirectional_merit_manifold_probe_v19_19.json"
    ).read_text(encoding="utf-8"))
    selected = payload["sixteenth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v19.19 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def sixteenth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_nineteenth_direct_residual_scale_audit_v19_18.json"
    ).read_text(encoding="utf-8"))
    measured = audit["nineteenth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v19.18 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v19_17_selected_raw_vector()
    ...
    return {
        "source_state": "v19.17_fifteenth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = sixteenth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v19_17": result["source_state"].startswith("v19.17"),
    ...
    return {
        "artifact": "BHSM_aether_n3_sixteenth_bidirectional_merit_manifold_probe_v19_19",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "sixteenth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_sixteenth_bidirectional_merit_manifold_probe_v19_19.json"
...
__all__ = [
    ...
    "v19_19_selected_raw_vector", "sixteenth_bidirectional_merit_manifold_probe",
