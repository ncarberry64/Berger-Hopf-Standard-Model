"""Reconstruct the next admissible v18.43 line child after v18.45 fails flux."""
...
VERSION = "v18.46"
CLASSIFICATION = "BHSM_N3_THIRD_DIRECT_ADMISSIBLE_LINE_CHILD"
FULL_BHSM_COMPLETE = False


def v18_46_selected_raw_vector() -> np.ndarray:
    ...
    remaining = [
        ...
        and row["backtrack"] > rejected["backtrack"]
    ]
    if not remaining:
        raise ValueError("v18.43 has no remaining exact-merit candidate")
    selected = min(remaining, key=lambda row: row["metrics"]["complete"])
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def third_direct_admissible_line_child() -> dict[str, Any]:
    """Apply the unchanged 14-row child map to the next exact-merit point."""
    proposal = json.loads(Path(
        "artifacts/BHSM_aether_n3_third_direct_residual_jfnk_v18_43.json"
    ).read_text(encoding="utf-8"))["third_direct_residual_jfnk"]
    rejected = proposal["selected_true_merit_candidate_pending_child_acceptance"]
    ...
    selected = min(remaining, key=lambda row: row["metrics"]["complete"])
    raw = v18_46_selected_raw_vector()
    ...
    prior_child = prior["second_direct_line_child"]["child_state"]
    germ = _pack_child(
    ...
    return {
        "line_selection": {
            "rule": "LOWEST_EXACT_MERIT_REMAINING_AFTER_V18_45_CHILD_FLUX_FAILURE",
            "rejected_backtrack": rejected["backtrack"],
            "selected_backtrack": selected["backtrack"],
            "selected_fraction": selected["fraction"],
            "selected_complete_norm": selected["metrics"]["complete"],
            "selected_complete_norm_reduction": selected["complete_norm_reduction"],
            "selected_eta_minimum": selected["eta_minimum"],
        },
        "source_event": "v18.43_next_independent_exact_merit_line_proposal",
        "source_solver_model": "INVALIDATED_NOT_REASSERTED",
        "source_germ": "v18.40_validated_complete_child",
...
def completion_payload() -> dict[str, Any]:
    result = third_direct_admissible_line_child()
    chart = result["chart"]
    rows = result["physical_residuals"]
    validation = {
        "next_candidate_selected_by_exact_merit": (
            result["line_selection"]["selected_backtrack"] == 3
            and result["line_selection"]["selected_complete_norm_reduction"] > 0.0
        ),
        "invalidated_solver_model_not_reasserted": result["source_solver_model"] == "INVALIDATED_NOT_REASSERTED",
    ...
    return {
        "artifact": "BHSM_aether_n3_third_direct_admissible_line_child_v18_46",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "third_direct_admissible_line_child": result,
        "status": "VALIDATED" if passed else "INVALIDATED",
        "real_physical_property_explained": (
            "THE_NEXT_EXACT_MERIT_LINE_STATE_IS_TESTED_WITH_THE_UNCHANGED_COMPLETE_"
            "CHILD_MAP_AFTER_THE_MORE_AGGRESSIVE_STATE_FAILS_FLUX"
        ),
        "dependency_advanced": "NONLINEAR_N3_EVENT_SADDLE_CLOSURE",
        "active_calculation": (
            "VERIFY_FLUX_ENVELOPE_AND_POSITIVE_DURATION_PERSISTENCE_THEN_PROMOTE"
            if passed else "TEST_THE_NEXT_EXISTING_EXACT_MERIT_LINE_STATE"
        ),
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_third_direct_admissible_line_child_v18_46.json"
...
__all__ = [
    ...
    "CLASSIFICATION",
    "FULL_BHSM_COMPLETE",
    "v18_46_selected_raw_vector",
    "third_direct_admissible_line_child",
    "completion_payload",
    "materialize",
]
