"""Scan both orientations of a bounded geometric probe from v18.87."""
...
VERSION = "v18.89"
CLASSIFICATION = "BHSM_N3_NINTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.804250811090346
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v18_89_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_ninth_bidirectional_merit_manifold_probe_v18_89.json"
    ).read_text(encoding="utf-8"))
    selected = payload["ninth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v18.89 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def ninth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_twelfth_direct_residual_scale_audit_v18_88.json"
    ).read_text(encoding="utf-8"))
    measured = audit["twelfth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v18.88 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v18_87_selected_raw_vector()
    ...
    return {
        "source_state": "v18.87_eighth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = ninth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v18_87": result["source_state"].startswith("v18.87"),
    ...
    return {
        "artifact": "BHSM_aether_n3_ninth_bidirectional_merit_manifold_probe_v18_89",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "ninth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_ninth_bidirectional_merit_manifold_probe_v18_89.json"
...
__all__ = [
    ...
    "v18_89_selected_raw_vector", "ninth_bidirectional_merit_manifold_probe",
