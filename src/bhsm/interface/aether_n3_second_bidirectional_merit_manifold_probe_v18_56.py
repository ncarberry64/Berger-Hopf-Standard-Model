"""Scan both orientations of a bounded geometric probe from v18.54."""
...
VERSION = "v18.56"
CLASSIFICATION = "BHSM_N3_SECOND_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
...
def v18_56_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path("artifacts/BHSM_aether_n3_second_bidirectional_merit_manifold_probe_v18_56.json").read_text(encoding="utf-8"))
    selected = payload["second_bidirectional_merit_manifold_probe"]["selected_true_merit_candidate_pending_child_acceptance"]
    if selected is None:
        raise ValueError("v18.56 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def second_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path("artifacts/BHSM_aether_n3_fifth_direct_residual_scale_audit_v18_55.json").read_text(encoding="utf-8"))
    measured = audit["fifth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v18.55 has no common stable direct-response scale")
    ...
    return {
        "source_state": "v18.54_bidirectional_next_candidate_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = second_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v18_54": result["source_state"].startswith("v18.54"),
        "source_norm_reproduced": abs(result["source_complete_norm"] - 0.817942595938606) < 5.0e-12,
    ...
    return {
        "artifact": "BHSM_aether_n3_second_bidirectional_merit_manifold_probe_v18_56",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "second_bidirectional_merit_manifold_probe": result,
        ...
        "real_physical_property_explained": "A_FRESH_BOUNDED_LOCAL_GEOMETRIC_PROBE_FROM_V18_54_LOCATES_EXACT_NONLINEAR_MERIT_STATES_WITHOUT_A_NEWTON_CLAIM",
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_second_bidirectional_merit_manifold_probe_v18_56.json"
    path.write_text(deterministic_json(completion_payload()), encoding="utf-8")
    return path


__all__ = ["VERSION", "CLASSIFICATION", "FULL_BHSM_COMPLETE", "v18_56_selected_raw_vector", "second_bidirectional_merit_manifold_probe", "completion_payload", "materialize"]
