"""Use the validated v21.27 curvature once at the promoted v21.27 state."""
...
from bhsm.interface.aether_n3_third_expanded_predictive_continuation_v21_26 import v21_26_selected_raw_vector
from bhsm.interface.aether_n3_fourth_expanded_radius_refreshed_continuation_v21_27 import v21_27_selected_raw_vector
...
VERSION = "v21.28"
CLASSIFICATION = "BHSM_N3_FOURTH_EXPANDED_PREDICTIVE_CONTINUATION"
FULL_BHSM_COMPLETE = False


def v21_28_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_N3_FOURTH_EXPANDED_PREDICTIVE_CONTINUATION_V21_28.json"
    ).read_text(encoding="utf-8"))["fourth_expanded_predictive_continuation"]
    if not payload["promotion"]["promoted"]:
        raise ValueError("v21.28 has no physically promoted state")
...
def completion_payload() -> dict[str, Any]:
    refresh_artifact = json.loads(Path(
        "artifacts/BHSM_N3_FOURTH_EXPANDED_RADIUS_REFRESHED_CONTINUATION_V21_27.json"
    ).read_text(encoding="utf-8"))
    if not refresh_artifact["validation_passed"]:
        raise ValueError("v21.27 curvature refresh is not validated")
    curvature_raw = v21_26_selected_raw_vector(); scales = kkt_variable_scales()
    ...
    result = dual_metric_range_space_proposal(
        v21_27_selected_raw_vector(), source_label="v21.27",
        curvature_override=curvature, radius_schedule_override=schedule,
    )
    result["dual_metric_model"]["curvature_artifact"] = (
        "SINGLE_PREDICTIVE_REUSE_OF_VALIDATED_V21_27_EIGENPAIR_CURVATURE"
    )
    best = result["exact_search"]["best"]
    validation = {
        "source_v21_27_reproduced": abs(
            result["source"]["exact_rayleigh_f376_l2"] - 0.780634951423550
        ) < 5.0e-12,
    ...
    return {
        "artifact": "BHSM_N3_FOURTH_EXPANDED_PREDICTIVE_CONTINUATION_V21_28",
        "version": VERSION, "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "fourth_expanded_predictive_continuation": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_N3_FOURTH_EXPANDED_PREDICTIVE_CONTINUATION_V21_28.json"
    path.write_text(deterministic_json(completion_payload()), encoding="utf-8"); return path


__all__ = ["VERSION", "CLASSIFICATION", "FULL_BHSM_COMPLETE", "v21_28_selected_raw_vector", "completion_payload", "materialize"]
