"""Scan both orientations of a bounded geometric probe from v18.95."""
...
from bhsm.interface.aether_n3_tenth_bidirectional_probe_promotion_v18_95 import v18_95_selected_raw_vector


VERSION = "v18.97"
CLASSIFICATION = "BHSM_N3_ELEVENTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.801684037952532
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v18_97_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_eleventh_bidirectional_merit_manifold_probe_v18_97.json"
    ).read_text(encoding="utf-8"))
    selected = payload["eleventh_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v18.97 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def eleventh_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_fourteenth_direct_residual_scale_audit_v18_96.json"
    ).read_text(encoding="utf-8"))
    measured = audit["fourteenth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v18.96 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v18_95_selected_raw_vector()
    ...
    return {
        "source_state": "v18.95_tenth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = eleventh_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v18_95": result["source_state"].startswith("v18.95"),
    ...
    return {
        "artifact": "BHSM_aether_n3_eleventh_bidirectional_merit_manifold_probe_v18_97",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "eleventh_bidirectional_merit_manifold_probe": result,
        ...
        "real_physical_property_explained": (
            "A_FRESH_BOUNDED_LOCAL_GEOMETRIC_PROBE_FROM_V18_83_LOCATES_"
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_eleventh_bidirectional_merit_manifold_probe_v18_97.json"
...
__all__ = [
    ...
    "v18_97_selected_raw_vector", "eleventh_bidirectional_merit_manifold_probe",
