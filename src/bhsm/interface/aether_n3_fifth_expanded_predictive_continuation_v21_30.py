"""Use the validated v21.29 curvature once at the promoted v21.29 state."""
...
from bhsm.interface.aether_n3_fifth_expanded_radius_refreshed_continuation_v21_29 import v21_29_selected_raw_vector
...
VERSION = "v21.30"
CLASSIFICATION = "BHSM_N3_FIFTH_EXPANDED_PREDICTIVE_CONTINUATION"
FULL_BHSM_COMPLETE = False


def v21_30_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_N3_FIFTH_EXPANDED_PREDICTIVE_CONTINUATION_V21_30.json"
    ).read_text(encoding="utf-8"))["fifth_expanded_predictive_continuation"]
    if not payload["promotion"]["promoted"]:
        raise ValueError("v21.30 has no physically promoted state")
...
def completion_payload() -> dict[str, Any]:
    ...
    if not refresh_artifact["validation_passed"]:
        raise ValueError("v21.29 curvature refresh is not validated")
    curvature_raw = v21_28_selected_raw_vector(); scales = kkt_variable_scales()
    ...
    result = dual_metric_range_space_proposal(
        v21_29_selected_raw_vector(), source_label="v21.29",
        curvature_override=curvature, radius_schedule_override=schedule,
    )
    result["dual_metric_model"]["curvature_artifact"] = (
        "SINGLE_PREDICTIVE_REUSE_OF_VALIDATED_V21_29_EIGENPAIR_CURVATURE"
    )
    best = result["exact_search"]["best"]
    validation = {
        "source_v21_29_reproduced": abs(
            result["source"]["exact_rayleigh_f376_l2"] - 0.780394860938150
        ) < 5.0e-12,
    ...
    return {
        "artifact": "BHSM_N3_FIFTH_EXPANDED_PREDICTIVE_CONTINUATION_V21_30",
        "version": VERSION, "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "fifth_expanded_predictive_continuation": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_N3_FIFTH_EXPANDED_PREDICTIVE_CONTINUATION_V21_30.json"
    path.write_text(deterministic_json(completion_payload()), encoding="utf-8"); return path


__all__ = ["VERSION", "CLASSIFICATION", "FULL_BHSM_COMPLETE", "v21_30_selected_raw_vector", "completion_payload", "materialize"]
