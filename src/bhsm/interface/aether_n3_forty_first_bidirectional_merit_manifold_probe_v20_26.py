"""Scan both orientations of a bounded geometric probe from v20.24."""
...
VERSION = "v20.26"
CLASSIFICATION = "BHSM_N3_FORTY_FIRST_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.767729672729035
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v20_26_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_forty_first_bidirectional_merit_manifold_probe_v20_26.json"
    ).read_text(encoding="utf-8"))
    selected = payload["forty_first_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v20.26 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def forty_first_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_forty_fourth_direct_residual_scale_audit_v20_25.json"
    ).read_text(encoding="utf-8"))
    measured = audit["forty_fourth_direct_residual_scale_audit"]
    ...
    if pair is None:
        ...
        if not bounded_pairs:
            raise ValueError("v20.25 has no bounded direct-response pair")
        pair = min(
            bounded_pairs,
    ...
    raw = v20_24_selected_raw_vector()
    ...
    return {
        "source_state": "v20.24_fortieth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = forty_first_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v20_24": result["source_state"].startswith("v20.24"),
    ...
    return {
        "artifact": "BHSM_aether_n3_forty_first_bidirectional_merit_manifold_probe_v20_26",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "forty_first_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_forty_first_bidirectional_merit_manifold_probe_v20_26.json"
...
__all__ = [
    ...
    "v20_26_selected_raw_vector", "forty_first_bidirectional_merit_manifold_probe",
