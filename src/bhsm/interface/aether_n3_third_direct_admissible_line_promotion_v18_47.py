"""Promote the next v18.43 line state only after the unchanged child gate."""
...
VERSION = "v18.47"
CLASSIFICATION = "BHSM_N3_THIRD_DIRECT_ADMISSIBLE_LINE_PROMOTION"
FULL_BHSM_COMPLETE = False


def v18_47_selected_raw_vector() -> np.ndarray:
    return v18_46_selected_raw_vector().copy()


def third_direct_admissible_line_promotion() -> dict[str, Any]:
    """Recompute exact merit, eta, flux, and persistence before promotion."""
    reconstruction = json.loads(Path(
        "artifacts/BHSM_aether_n3_third_direct_admissible_line_child_v18_46.json"
    ).read_text(encoding="utf-8"))
    child = reconstruction["third_direct_admissible_line_child"]
    ...
    return {
        "global_step": {
            ...
            "line_backtrack": child["line_selection"]["selected_backtrack"],
...
def completion_payload() -> dict[str, Any]:
    result = third_direct_admissible_line_promotion()
    g = result["global_step"]
    c = result["event_to_complete_child"]
    p = result["persistence"]
    validation = {
        "invalidated_solver_model_not_reasserted": g["source_solver_model_status"] == "INVALIDATED" and not g["source_solver_model_reasserted"],
        "next_line_candidate_used": g["line_backtrack"] == 3,
    ...
    return {
        "artifact": "BHSM_aether_n3_third_direct_admissible_line_promotion_v18_47",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "third_direct_admissible_line_promotion": result,
        "status": "VALIDATED" if passed else "INVALIDATED",
        "real_physical_property_explained": (
            "THE_NEXT_V18_43_LINE_STATE_PASSES_THE_UNCHANGED_EXACT_MERIT_ETA_"
            "COMPLETE_CHILD_FLUX_AND_PERSISTENCE_GATES"
        ),
        "dependency_advanced": "NONLINEAR_N3_EVENT_SADDLE_CLOSURE",
        "active_calculation": (
            "REMEASURE_THE_DIRECT_RESIDUAL_RESPONSE_AND_CONTINUE_FROM_V18_47"
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_third_direct_admissible_line_promotion_v18_47.json"
...
__all__ = [
    ...
    "CLASSIFICATION",
    "FULL_BHSM_COMPLETE",
    "v18_47_selected_raw_vector",
    "third_direct_admissible_line_promotion",
    "completion_payload",
    "materialize",
]
