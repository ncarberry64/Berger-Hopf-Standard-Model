"""Resume exact N=3 continuation from promoted v20.73 using the recovered proposal mechanism."""
...
def v20_74_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_N3_STRUCTURED_PROPOSAL_CONTINUATION_V20_74.json"
    ).read_text(encoding="utf-8"))
    result = payload["structured_proposal_continuation"]
    if not result["promotion"]["promoted"]:
        raise ValueError("v20.74 candidate was not physically promoted")
    return np.asarray([
        float.fromhex(value)
...
def structured_proposal_continuation() -> dict[str, Any]:
    source_raw = v20_73_selected_raw_vector()
    ...
    for orientation in (-1.0, 1.0):
        for backtrack in range(BACKTRACKS):
            ...
            # The temporary excitation is absent: this starts at the exact v20.73 state.
            ...
            try:
                ...
                continue
    if best is not None:
        bracketed_alpha = float(best["alpha"])
        for factor in (0.625, 0.75, 0.875, 1.125, 1.25, 1.5):
            alpha = factor * bracketed_alpha
            candidate_y = source_y + alpha * (transform @ direction_x)
            try:
                candidate_raw = candidate_y / scales
    ...
    return {
        "source_frontier": {"version": "v20.73", "exact_f376_l2": source_norm},
        "proposal": {
        ...
        "exact_line_search": {
            ...
            "local_exact_scalar_refinement": True,
        ...
        "next_action": (
            "CONTINUE_EXACT_N3_CLOSURE_FROM_V20_74"
            if promotion["promoted"]
            else "REVISIT_EXACT_RESIDUAL_OWNERSHIP_AT_V20_73"
        ),
    }
...
def completion_payload() -> dict[str, Any]:
    ...
    validation = {
        "source_v20_73_reproduced": abs(
            result["source_frontier"]["exact_f376_l2"] - 0.758723504671207
        ) < 5.0e-12,
    ...
    return {
        "artifact": "BHSM_N3_STRUCTURED_PROPOSAL_CONTINUATION_V20_74",
        "version": VERSION,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_N3_STRUCTURED_PROPOSAL_CONTINUATION_V20_74.json"
...
__all__ = [
    ...
    "v20_74_selected_raw_vector", "structured_proposal_continuation",
