"""Use the validated v21.23 curvature once at the promoted v21.23 state."""
...
from bhsm.interface.aether_n3_expanded_radius_predictive_continuation_v21_22 import v21_22_selected_raw_vector
from bhsm.interface.aether_n3_expanded_radius_refreshed_continuation_v21_23 import v21_23_selected_raw_vector
...
VERSION = "v21.24"
CLASSIFICATION = "BHSM_N3_SECOND_EXPANDED_PREDICTIVE_CONTINUATION"
FULL_BHSM_COMPLETE = False


def v21_24_selected_raw_vector() -> np.ndarray:
    ...
    if not payload["promotion"]["promoted"]:
        raise ValueError("v21.24 has no physically promoted state")
...
def completion_payload() -> dict[str, Any]:
    ...
    if not refresh_artifact["validation_passed"]:
        raise ValueError("v21.23 curvature refresh is not validated")
    curvature_raw = v21_22_selected_raw_vector(); scales = kkt_variable_scales()
    ...
    result = dual_metric_range_space_proposal(
        v21_23_selected_raw_vector(), source_label="v21.23",
        curvature_override=curvature, radius_schedule_override=schedule,
    )
    result["dual_metric_model"]["curvature_artifact"] = (
        "SINGLE_PREDICTIVE_REUSE_OF_VALIDATED_V21_23_EIGENPAIR_CURVATURE"
    )
    best = result["exact_search"]["best"]
    validation = {
        "source_v21_23_reproduced": abs(
            result["source"]["exact_rayleigh_f376_l2"] - 0.781148984940364
        ) < 5.0e-12,
    ...
    return {
        "artifact": "BHSM_N3_SECOND_EXPANDED_PREDICTIVE_CONTINUATION_V21_24",
        "version": VERSION, "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "second_expanded_predictive_continuation": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_N3_SECOND_EXPANDED_PREDICTIVE_CONTINUATION_V21_24.json"
    path.write_text(deterministic_json(completion_payload()), encoding="utf-8"); return path


__all__ = ["VERSION", "CLASSIFICATION", "FULL_BHSM_COMPLETE", "v21_24_selected_raw_vector", "completion_payload", "materialize"]
