"""Scan both orientations of a bounded geometric probe from v18.77."""
...
from bhsm.interface.aether_n3_sixth_bidirectional_probe_promotion_v18_77 import (
    v18_77_selected_raw_vector,
)


VERSION = "v18.79"
CLASSIFICATION = "BHSM_N3_SEVENTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.806818034168188
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v18_79_selected_raw_vector() -> np.ndarray:
    payload = json.loads(
        Path(
            "artifacts/"
    ...
    selected = payload["seventh_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v18.79 has no exact-merit candidate")
    return np.asarray(
        [float.fromhex(value) for value in selected["raw_vector_hex"]]
    )


def seventh_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(
        Path(
            "artifacts/BHSM_aether_n3_tenth_direct_residual_scale_audit_v18_78.json"
        ).read_text(encoding="utf-8")
    )
    measured = audit["tenth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v18.78 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v18_77_selected_raw_vector()
    ...
    def direct_response(
        direction_y: np.ndarray, step: float = response_step
    ) -> np.ndarray:
        ...
        for backtrack in range(BACKTRACKS):
            ...
            try:
                ...
                row = {
                    ...
                    "physical_scaled_coordinate_step_norm": (
                        unsigned_fraction * direction_norm
                    ),
    ...
    selected = (
        min(eligible, key=lambda row: row["metrics"]["complete"])
        if eligible
        else None
    )
    best_by_orientation = {}
    for orientation in ("positive", "negative"):
        rows = [
            row for row in eligible if row["orientation"] == orientation
        ]
        best_by_orientation[orientation] = (
            None
            if not rows
            else min(rows, key=lambda row: row["metrics"]["complete"])
        )
    return {
        "source_state": "v18.77_sixth_bidirectional_probe_promoted_state",
        ...
        "direct_response": {
            ...
            "source_maximum_relative_change": pair[
...
def completion_payload() -> dict[str, Any]:
    result = seventh_bidirectional_merit_manifold_probe()
    selected = result[
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    ...
    validation = {
        "source_is_v18_77": result["source_state"].startswith("v18.77"),
        "source_norm_reproduced": abs(
            result["source_complete_norm"] - SOURCE_NORM
        )
        < 5.0e-12,
        "validated_response_scale_source": (
            response["source_status"] == "VALIDATED"
        ),
        ...
        "krylov_convergence_not_physical_gate": probe[
            "convergence_not_required_to_legitimize_proposal"
        ],
        "prior_failed_models_not_reused": not response[
            "prior_failed_direction_models_reused"
        ],
        "right_coordinate_map_invertible": result["coordinate_map"][
            "invertible"
        ],
        "both_orientations_scanned": result["line_scan"][
            "both_orientations_scanned"
        ],
        "exact_nonlinear_merit_authoritative": result["line_scan"][
            "exact_nonlinear_residual_authoritative"
        ],
        ...
        "selected_reduces_true_merit": bool(
            selected is None
            or selected["complete_norm_reduction"] > MARGIN
        ),
    ...
    return {
        "artifact": (
            "BHSM_aether_n3_seventh_bidirectional_merit_manifold_probe_v18_79"
        ),
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "seventh_bidirectional_merit_manifold_probe": result,
        "status": "VALIDATED" if passed else "INVALIDATED",
        "solver_interpretation": (
            "VALIDATED" if solver_interpretation_valid else "INVALIDATED"
        ),
        "physical_candidate_status": (
            "PENDING_COMPLETE_CHILD_GATE" if selected is not None else "NONE"
        ),
        "real_physical_property_explained": (
            "A_FRESH_BOUNDED_LOCAL_GEOMETRIC_PROBE_FROM_V18_77_LOCATES_"
