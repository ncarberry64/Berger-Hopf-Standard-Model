"""Promote only the exact physical state proposed by invalidated v18.31."""
...
from bhsm.interface.aether_n3_congruent_matrix_free_square_kkt_v18_31 import (
...
VERSION = "v18.33"
CLASSIFICATION = "BHSM_N3_INVALID_MODEL_EXACT_MERIT_PROMOTION"
FULL_BHSM_COMPLETE = False


def v18_33_selected_raw_vector() -> np.ndarray:
    return v18_31_selected_raw_vector().copy()


def invalid_model_exact_merit_promotion() -> dict[str, Any]:
    reconstruction = json.loads(Path(
        "artifacts/BHSM_aether_n3_congruent_proposal_child_reconstruction_v18_32.json"
    ).read_text(encoding="utf-8"))
    child_state = reconstruction[
        "congruent_proposal_child_reconstruction"
    ]["child_state"]
    ...
    source_raw = v18_29_selected_raw_vector()
    raw = v18_33_selected_raw_vector()
    source_residual = _square_physical_residual(source_raw * scales)
    candidate_residual = _square_physical_residual(raw * scales)
    source_metrics = _metrics(source_residual)
    ...
    return {
        "global_step": {
            "source_solver_model_status": proposal["status"],
            "source_solver_model_reasserted": False,
        ...
        "event_to_complete_child": {
            ...
            "local_chart_rank": reconstruction[
                "congruent_proposal_child_reconstruction"
            ]["chart"]["full_chart_rank"],
        ...
        "persistence": {
            ...
            "nonzero_relative_evolution_retained": bool(
                np.linalg.norm(q - qc) > 0.0 and np.linalg.norm(v) > 0.0
            ),
...
def completion_payload() -> dict[str, Any]:
    result = invalid_model_exact_merit_promotion()
    g = result["global_step"]
    c = result["event_to_complete_child"]
    p = result["persistence"]
    validation = {
        "invalidated_solver_model_not_reasserted": (
            g["source_solver_model_status"] == "INVALIDATED"
            and not g["source_solver_model_reasserted"]
        ),
        "independent_exact_residual_recomputed": g[
            "independent_exact_residual_recomputed"
        ],
        "validated_recomputed_child": (
            c["source_reconstruction_status"] == "VALIDATED"
            and c["chart_recomputed_from_all_child_variables"]
        ),
        "square_explicit_multiplier_solve": (
            g["physical_solve_dimension"] == [376, 376]
            and g["event_multiplier_explicit"]
        ),
        "no_componentwise_filter": not g["componentwise_monotonicity_required"],
        "previous_path_not_a_constraint": not g[
        ...
        "attachment_momentum_closed": c[
            "attachment_momentum_residual_norm"
        ] < 1.0e-7,
        "resolved_dynamic_flux_closed": c[
            "resolved_dynamic_flux_envelope"
        ] < 2.0e-5,
        "child_eta_hyperregular": c["eta_Legendre_minimum"]["minimum"] > 0.0,
        "positive_interval_persists": (
            p["all_steps_valid"]
            and p["maximum_constraint_residual"] < 1.0e-8
            and p["minimum_eta"] > 0.0
        ),
        "nonzero_relative_evolution_retained": p[
    ...
    return {
        "artifact": "BHSM_aether_n3_invalid_model_exact_merit_promotion_v18_33",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "invalid_model_exact_merit_promotion": result,
        "status": "VALIDATED" if passed else "INVALIDATED",
        "real_physical_property_explained": (
            "AN_INVALIDATED_SOLVER_MODEL_MAY_PROPOSE_A_STATE_BUT_ONLY_"
            "INDEPENDENT_EXACT_MERIT_ETA_AND_COMPLETE_CHILD_PERSISTENCE_PROMOTE_IT"
        ),
        "dependency_advanced": "NONLINEAR_N3_EVENT_SADDLE_CLOSURE",
        "active_calculation": (
            "RESOLVE_THE_EXACT_SQUARE_KKT_LINEAR_RESPONSE_FROM_V18_33"
            if passed else "RESOLVE_THE_COMPLETE_CHILD_PERSISTENCE_GATE"
        ),
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_invalid_model_exact_merit_promotion_v18_33.json"
...
__all__ = [
    ...
    "invalid_model_exact_merit_promotion",
    "completion_payload",
    "materialize",
]
