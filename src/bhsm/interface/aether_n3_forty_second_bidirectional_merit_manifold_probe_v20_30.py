"""Scan both orientations of a bounded geometric probe from v20.28."""
...
VERSION = "v20.30"
CLASSIFICATION = "BHSM_N3_FORTY_SECOND_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.767158069837684
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v20_30_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_forty_second_bidirectional_merit_manifold_probe_v20_30.json"
    ).read_text(encoding="utf-8"))
    selected = payload["forty_second_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v20.30 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def forty_second_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_forty_fifth_direct_residual_scale_audit_v20_29.json"
    ).read_text(encoding="utf-8"))
    measured = audit["forty_fifth_direct_residual_scale_audit"]
    ...
    if pair is None:
        ...
        if not bounded_pairs:
            raise ValueError("v20.29 has no bounded direct-response pair")
        pair = min(
            bounded_pairs,
    ...
    raw = v20_28_selected_raw_vector()
    ...
    return {
        "source_state": "v20.28_forty_first_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = forty_second_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v20_28": result["source_state"].startswith("v20.28"),
    ...
    return {
        "artifact": "BHSM_aether_n3_forty_second_bidirectional_merit_manifold_probe_v20_30",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "forty_second_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_forty_second_bidirectional_merit_manifold_probe_v20_30.json"
...
__all__ = [
    ...
    "v20_30_selected_raw_vector", "forty_second_bidirectional_merit_manifold_probe",
