"""Scan both orientations of a bounded geometric probe from v19.03."""
...
VERSION = "v19.05"
CLASSIFICATION = "BHSM_N3_THIRTEENTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.797947455518253
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v19_05_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_thirteenth_bidirectional_merit_manifold_probe_v19_05.json"
    ).read_text(encoding="utf-8"))
    selected = payload["thirteenth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v19.05 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def thirteenth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_sixteenth_direct_residual_scale_audit_v19_04.json"
    ).read_text(encoding="utf-8"))
    measured = audit["sixteenth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v19.04 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v19_03_selected_raw_vector()
    ...
    return {
        "source_state": "v19.03_twelfth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = thirteenth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v19_03": result["source_state"].startswith("v19.03"),
    ...
    return {
        "artifact": "BHSM_aether_n3_thirteenth_bidirectional_merit_manifold_probe_v19_05",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "thirteenth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_thirteenth_bidirectional_merit_manifold_probe_v19_05.json"
...
__all__ = [
    ...
    "v19_05_selected_raw_vector", "thirteenth_bidirectional_merit_manifold_probe",
