"""Fresh period-priority continuation from the validated v17.36 state."""
...
VERSION = "v17.37"
CLASSIFICATION = "BHSM_N3_FRESH_SBP_THIRD_PERIOD_PRIORITY_MEASURED_TANGENT_FAMILY"
FULL_BHSM_COMPLETE = False


def v17_36_selected_raw_vector() -> np.ndarray:
    payload = json.loads(
        Path(
            "artifacts/BHSM_aether_n3_fresh_sbp_fifth_v0_priority_v17_36.json"
        ).read_text(encoding="utf-8")
    )
    values = payload["fresh_sbp_fifth_v0_priority"][
        "selected_v0_priority_maximin"
    ]["raw_vector_hex"]
    raw = np.asarray([float.fromhex(value) for value in values])
    if raw.shape != (376,):
        raise ValueError("v17.36 selected vector has wrong dimension")
    return raw


def third_period_priority() -> dict[str, Any]:
    return period_priority_family_from(
        v17_36_selected_raw_vector(),
        source_state="v17.36_selected_fifth_v0_priority_state",
...
def completion_payload() -> dict[str, Any]:
    result = third_period_priority()
    best = result["selected_period_priority_maximin"]
    validation = {
        "v17_36_residual_reproduced": math.isclose(
            result["initial_metrics"]["complete"],
            0.982211944328571,
            rel_tol=0,
            abs_tol=2e-8,
        ),
        "v17_36_event_reproduced": math.isclose(
            result["initial_metrics"]["event"],
            0.086699384225563,
            rel_tol=0,
            abs_tol=2e-8,
        ),
        "v17_36_period_reproduced": math.isclose(
        ...
        "source_state_owned": (
            result["source_state"] == "v17.36_selected_fifth_v0_priority_state"
        ),
    ...
    return {
        "artifact": "BHSM_aether_n3_fresh_sbp_third_period_priority_v17_37",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": False,
        "fresh_sbp_third_period_priority": result,
        ...
        "real_physical_property_explained": (
            ...
            "V17_36_PERIOD_LIMITER_TRANSITION"
        ),
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_fresh_sbp_third_period_priority_v17_37.json"
