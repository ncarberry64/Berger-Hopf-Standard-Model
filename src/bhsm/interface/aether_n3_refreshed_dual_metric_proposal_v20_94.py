"""Test one exact proposal using the Rayleigh curvature refreshed at v20.92."""
...
from bhsm.interface.aether_n3_dual_metric_range_space_continuation_v20_92 import (
    v20_92_selected_raw_vector,
)
...
VERSION = "v20.94"
CLASSIFICATION = "BHSM_N3_REFRESHED_CURVATURE_DUAL_METRIC_PROPOSAL"
FULL_BHSM_COMPLETE = False


def v20_94_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_N3_REFRESHED_DUAL_METRIC_PROPOSAL_V20_94.json"
    ).read_text(encoding="utf-8"))["refreshed_dual_metric_proposal"]
    if not payload["promotion"]["promoted"]:
        raise ValueError("v20.94 has no physically promoted state")
    return np.asarray([
...
def completion_payload() -> dict[str, Any]:
    ...
    validation = {
        "source_v20_92_reproduced": abs(
            result["source"]["exact_rayleigh_f376_l2"] - 0.785778751174718
        ) < 5.0e-12,
        "refreshed_curvature_used": result["dual_metric_model"][
            "curvature_artifact"
        ].endswith("V20_93.json"),
        "both_bhsm_metrics_used": result["dual_metric_model"][
            "physical_metric"
        ].startswith("EXISTING_BHSM") and result["dual_metric_model"][
            "action_metric"
        ].startswith("VALIDATED_V18_15"),
    ...
    return {
        "artifact": "BHSM_N3_REFRESHED_DUAL_METRIC_PROPOSAL_V20_94",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "refreshed_dual_metric_proposal": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_N3_REFRESHED_DUAL_METRIC_PROPOSAL_V20_94.json"
