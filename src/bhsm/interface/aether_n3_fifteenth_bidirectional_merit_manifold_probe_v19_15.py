"""Scan both orientations of a bounded geometric probe from v19.13."""
...
VERSION = "v19.15"
CLASSIFICATION = "BHSM_N3_FIFTEENTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.795953277613514
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v19_15_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_fifteenth_bidirectional_merit_manifold_probe_v19_15.json"
    ).read_text(encoding="utf-8"))
    selected = payload["fifteenth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v19.15 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def fifteenth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_eighteenth_direct_residual_scale_audit_v19_14.json"
    ).read_text(encoding="utf-8"))
    measured = audit["eighteenth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v19.14 has no common stable direct-response scale")
    ...
    return {
        "source_state": "v19.13_fourteenth_bidirectional_fallback_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = fifteenth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v19_13": result["source_state"].startswith("v19.13"),
    ...
    return {
        "artifact": "BHSM_aether_n3_fifteenth_bidirectional_merit_manifold_probe_v19_15",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "fifteenth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_fifteenth_bidirectional_merit_manifold_probe_v19_15.json"
...
__all__ = [
    ...
    "v19_15_selected_raw_vector", "fifteenth_bidirectional_merit_manifold_probe",
