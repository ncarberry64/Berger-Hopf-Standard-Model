"""Scan both orientations of a bounded geometric probe from v19.98."""
...
from bhsm.interface.aether_n3_thirty_fourth_bidirectional_probe_promotion_v19_98 import v19_98_selected_raw_vector


VERSION = "v20.00"
CLASSIFICATION = "BHSM_N3_THIRTY_FIFTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.770113042159652
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v20_00_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_thirty_fifth_bidirectional_merit_manifold_probe_v20_00.json"
    ).read_text(encoding="utf-8"))
    selected = payload["thirty_fifth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v20.00 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def thirty_fifth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_thirty_eighth_direct_residual_scale_audit_v19_99.json"
    ).read_text(encoding="utf-8"))
    measured = audit["thirty_eighth_direct_residual_scale_audit"]
    ...
    if pair is None:
        ...
        if not bounded_pairs:
            raise ValueError("v19.99 has no bounded direct-response pair")
        pair = min(
            bounded_pairs,
    ...
    raw = v19_98_selected_raw_vector()
    ...
    return {
        "source_state": "v19.98_thirty_fourth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = thirty_fifth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v19_98": result["source_state"].startswith("v19.98"),
    ...
    return {
        "artifact": "BHSM_aether_n3_thirty_fifth_bidirectional_merit_manifold_probe_v20_00",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "thirty_fifth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_thirty_fifth_bidirectional_merit_manifold_probe_v20_00.json"
...
__all__ = [
    ...
    "v20_00_selected_raw_vector", "thirty_fifth_bidirectional_merit_manifold_probe",
