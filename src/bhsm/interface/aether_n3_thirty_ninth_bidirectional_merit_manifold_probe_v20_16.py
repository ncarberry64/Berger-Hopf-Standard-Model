"""Scan both orientations of a bounded geometric probe from v20.14."""
...
from bhsm.interface.aether_n3_thirty_eighth_bidirectional_probe_promotion_v20_14 import v20_14_selected_raw_vector


VERSION = "v20.16"
CLASSIFICATION = "BHSM_N3_THIRTY_NINTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.768704442333321
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v20_16_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_thirty_ninth_bidirectional_merit_manifold_probe_v20_16.json"
    ).read_text(encoding="utf-8"))
    selected = payload["thirty_ninth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v20.16 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def thirty_ninth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_forty_second_direct_residual_scale_audit_v20_15.json"
    ).read_text(encoding="utf-8"))
    measured = audit["forty_second_direct_residual_scale_audit"]
    ...
    if pair is None:
        ...
        if not bounded_pairs:
            raise ValueError("v20.15 has no bounded direct-response pair")
        pair = min(
            bounded_pairs,
    ...
    raw = v20_14_selected_raw_vector()
    ...
    return {
        "source_state": "v20.14_thirty_eighth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = thirty_ninth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v20_14": result["source_state"].startswith("v20.14"),
    ...
    return {
        "artifact": "BHSM_aether_n3_thirty_ninth_bidirectional_merit_manifold_probe_v20_16",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "thirty_ninth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_thirty_ninth_bidirectional_merit_manifold_probe_v20_16.json"
...
__all__ = [
    ...
    "v20_16_selected_raw_vector", "thirty_ninth_bidirectional_merit_manifold_probe",
