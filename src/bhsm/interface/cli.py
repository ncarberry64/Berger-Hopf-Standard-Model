...
from .boundary_matter_dynamics_neutral_response import (
...
def build_parser() -> argparse.ArgumentParser:
    ...
    m5_m4 = commands.add_parser("m5-m4-boundary-reduction-status", help="Render the BHSM v6.1.1 parent-M5 to equatorial-M4 boundary reduction")
    ...
    integrated_status_commands = (
        ...
        ("primitive-support-ledger-status-v11-2", "Render the BHSM v11.2 primitive support-character ledger"),
        ("support-current-status-v11-2", "Render the BHSM v11.2 support-current classification"),
        ("support-connection-couplings-status-v11-2", "Render the BHSM v11.2 linear/quadratic connection ledger"),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command in {
        ...
        "primitive-support-ledger-status-v11-2",
        "support-current-status-v11-2",
        "support-connection-couplings-status-v11-2",
    ...
    if args.command == "b8-parent-action-status":
        ...
        return 0
    if args.command == "energy-geometry-confinement-status":
        payload = confinement_status_report()
        if args.format == "json":
            print(json.dumps(payload, indent=2, sort_keys=True))
        else:
    ...
    if args.command == "b8-s7-berger-s3-reduction-status":
        ...
        return 0
    if args.command == "twistor-berger-associated-bundle-status":
        payload = twistor_berger_status_report()
        if args.format == "json":
            print(json.dumps(payload, indent=2, sort_keys=True))
        else:
    ...
    if args.command == "round-background-gauge-scalar-status":
        ...
        return 0
    if args.command == "m5-m4-boundary-reduction-status":
        payload = m5_m4_status_report()
        if args.format == "json":
            print(json.dumps(payload, indent=2, sort_keys=True))
        else:
            _print_unicode(m5_m4_status_to_markdown(payload))
        return 0
    if args.command == "m4-lorentz-localization-status":
        payload = localization_status_report()
        if args.format == "json":
            print(json.dumps(payload, indent=2, sort_keys=True))
        else:
    ...
    if args.command == "minimal-equatorial-boundary-action-status":
        ...
        return 0
    if args.command == "intrinsic-m4-junction-background-status":
        payload = junction_background_status_report()
        if args.format == "json":
            print(json.dumps(payload, indent=2, sort_keys=True))
        else:
    ...
    if args.command == "scalar-wall-junction-audit-status":
        ...
        return 0
    if args.command == "scalar-wall-bifurcation-status":
        payload = bifurcation_status_report()
        if args.format == "json":
            print(json.dumps(payload, indent=2, sort_keys=True))
        else:
    ...
    if args.command == "topological-fr-neutral-dispersion-status":
        ...
        return 0
    if args.command == "boundary-matter-neutral-response-status":
        payload = boundary_matter_status_report()
        if args.format == "json":
            print(json.dumps(payload, indent=2, sort_keys=True))
        else:
            _print_unicode(boundary_matter_status_to_markdown(payload))
        return 0
    if args.command in {
        "primitive-charged-incidence",
        "rho-ch-gcd-selection",
