"""Refresh the eigenpair curvature at v21.31 for consecutive fresh continuation."""
...
from bhsm.interface.aether_n3_sixth_expanded_radius_refreshed_continuation_v21_31 import v21_31_selected_raw_vector
...
VERSION = "v21.32"
CLASSIFICATION = "BHSM_N3_SEVENTH_EXPANDED_RADIUS_REFRESHED_CONTINUATION"
FULL_BHSM_COMPLETE = False


def v21_32_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_N3_SEVENTH_EXPANDED_RADIUS_REFRESHED_CONTINUATION_V21_32.json"
    ).read_text(encoding="utf-8"))["seventh_expanded_radius_refreshed_continuation"]
    if not payload["promotion"]["promoted"]:
        raise ValueError("v21.32 has no physically promoted state")
    return np.asarray([float.fromhex(v) for v in payload["exact_search"]["best"]["raw_vector_hex"]])


def completion_payload() -> dict[str, Any]:
    raw = v21_31_selected_raw_vector(); scales = kkt_variable_scales(); y = raw * scales
    ...
    if not curvature_valid:
        raise ValueError("v21.32 refreshed curvature did not validate")
    ...
    result = dual_metric_range_space_proposal(
        raw, source_label="v21.31", curvature_override=curvature,
        radius_schedule_override=schedule,
    )
    result["dual_metric_model"]["curvature_artifact"] = "IN_MEMORY_VALIDATED_V21_32_EIGENPAIR_REFRESH"
    ...
    validation = {
        "source_v21_31_reproduced": abs(source_norm - 0.780212770711283) < 5.0e-12,
    ...
    return {
        "artifact": "BHSM_N3_SEVENTH_EXPANDED_RADIUS_REFRESHED_CONTINUATION_V21_32", "version": VERSION,
        "classification": CLASSIFICATION, "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "seventh_expanded_radius_refreshed_continuation": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_N3_SEVENTH_EXPANDED_RADIUS_REFRESHED_CONTINUATION_V21_32.json"
    path.write_text(deterministic_json(completion_payload()), encoding="utf-8"); return path


__all__ = ["VERSION", "CLASSIFICATION", "FULL_BHSM_COMPLETE", "v21_32_selected_raw_vector", "completion_payload", "materialize"]
