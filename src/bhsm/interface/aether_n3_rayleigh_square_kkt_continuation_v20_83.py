"""Continue the validated Rayleigh square-KKT solve from v20.82."""
...
def v20_83_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_N3_RAYLEIGH_SQUARE_KKT_CONTINUATION_V20_83.json"
    ).read_text(encoding="utf-8"))["rayleigh_square_kkt_continuation"]
    if not payload["promotion"]["promoted"]:
        raise ValueError("v20.83 has no physically promoted state")
    return np.asarray([float.fromhex(value) for value in payload["exact_line_search"]["best"]["raw_vector_hex"]])


def completion_payload() -> dict[str, Any]:
    result = rayleigh_square_kkt_proposal(v20_82_selected_raw_vector(), source_label="v20.82")
    best = result["exact_line_search"]["best"]
    validation = {
        "source_v20_82_reproduced": abs(result["source"]["exact_rayleigh_f376_l2"] - 0.792168779696871) < 5.0e-12,
    ...
    return {
        "artifact": "BHSM_N3_RAYLEIGH_SQUARE_KKT_CONTINUATION_V20_83", "version": VERSION,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_N3_RAYLEIGH_SQUARE_KKT_CONTINUATION_V20_83.json"
    path.write_text(deterministic_json(completion_payload()), encoding="utf-8")
    return path


__all__ = ["VERSION", "CLASSIFICATION", "FULL_BHSM_COMPLETE", "v20_83_selected_raw_vector", "completion_payload", "materialize"]
