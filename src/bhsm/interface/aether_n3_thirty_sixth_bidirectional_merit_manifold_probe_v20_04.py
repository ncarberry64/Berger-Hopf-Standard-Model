"""Scan both orientations of a bounded geometric probe from v20.02."""
...
VERSION = "v20.04"
CLASSIFICATION = "BHSM_N3_THIRTY_SIXTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
...
def v20_04_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_thirty_sixth_bidirectional_merit_manifold_probe_v20_04.json"
    ).read_text(encoding="utf-8"))
    selected = payload["thirty_sixth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v20.04 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def thirty_sixth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_thirty_ninth_direct_residual_scale_audit_v20_03.json"
    ).read_text(encoding="utf-8"))
    measured = audit["thirty_ninth_direct_residual_scale_audit"]
    ...
    if pair is None:
        ...
        if not bounded_pairs:
            raise ValueError("v20.03 has no bounded direct-response pair")
        pair = min(
            bounded_pairs,
...
def completion_payload() -> dict[str, Any]:
    result = thirty_sixth_bidirectional_merit_manifold_probe()
    ...
    return {
        "artifact": "BHSM_aether_n3_thirty_sixth_bidirectional_merit_manifold_probe_v20_04",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "thirty_sixth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_thirty_sixth_bidirectional_merit_manifold_probe_v20_04.json"
...
__all__ = [
    ...
    "v20_04_selected_raw_vector", "thirty_sixth_bidirectional_merit_manifold_probe",
