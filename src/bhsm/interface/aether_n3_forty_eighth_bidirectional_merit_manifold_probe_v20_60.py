"""Scan both orientations of a bounded geometric probe from v20.58."""
...
VERSION = "v20.60"
CLASSIFICATION = "BHSM_N3_FORTY_EIGHTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.767014925748291
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v20_60_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_forty_eighth_bidirectional_merit_manifold_probe_v20_60.json"
    ).read_text(encoding="utf-8"))
    selected = payload["forty_eighth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v20.60 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def forty_eighth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_fifty_first_direct_residual_scale_audit_v20_59.json"
    ).read_text(encoding="utf-8"))
    measured = audit["fifty_first_direct_residual_scale_audit"]
    ...
    if pair is None:
        ...
        if not bounded_pairs:
            raise ValueError("v20.59 has no bounded direct-response pair")
        pair = min(
            bounded_pairs,
    ...
    return {
        "source_state": "v20.58_forty_seventh_bidirectional_fallback_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = forty_eighth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v20_58": result["source_state"].startswith("v20.58"),
    ...
    return {
        "artifact": "BHSM_aether_n3_forty_eighth_bidirectional_merit_manifold_probe_v20_60",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "forty_eighth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_forty_eighth_bidirectional_merit_manifold_probe_v20_60.json"
...
__all__ = [
    ...
    "v20_60_selected_raw_vector", "forty_eighth_bidirectional_merit_manifold_probe",
