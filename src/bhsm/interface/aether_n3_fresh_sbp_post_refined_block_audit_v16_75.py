"""Residual-block and soft-spectrum audit after the v16.74 refined cone."""
...
VERSION="v16.75";CLASSIFICATION="BHSM_N3_FRESH_SBP_POST_REFINED_CONE_BLOCK_SOFT_AUDIT";FULL_BHSM_COMPLETE=False

def v16_74_selected_raw_vector()->np.ndarray:
    p=json.loads(Path("artifacts/BHSM_aether_n3_fresh_sbp_second_refined_damped_cone_v16_74.json").read_text(encoding="utf-8"))
    values=p["fresh_sbp_second_refined_damped_common_cone"]["selected_best_accepted"]["raw_vector_hex"]
    raw=np.asarray([float.fromhex(v) for v in values])
    if raw.shape!=(376,):raise ValueError("v16.74 selected vector has wrong dimension")
    return raw

def post_refined_block_soft_audit()->dict[str,Any]:
    raw=v16_74_selected_raw_vector();y=raw*kkt_variable_scales();_,res=sbp_projected_residual_and_vector(y)
    qres=res[:230].reshape(23,10);mres=res[230:374].reshape(24,6)
    u=unpack_reduced(raw);q=np.asarray(u["coordinates"]);m=np.asarray(u["multipliers"]);period=float(u["period"])
    ...
    return {"source_state":"v16.74_selected_refined_damped_common_cone_state","complete_residual_norm":float(np.linalg.norm(res)),
        ...
        "interpretation":"THE_REFINED_CONE_REDUCED_THE_COMPLETE_KKT_DEFECT_WHILE_PRESERVING_AN_ISOLATED_PHYSICAL_SOFT_BRANCH;THE_REMAINING_BLOCK_RANKING_SELECTS_THE_NEXT_PARENT_GEOMETRY_CORRECTION"}

def completion_payload()->dict[str,Any]:
    r=post_refined_block_soft_audit();s=r["terminal_soft_mode"]
    validation={"v16_74_residual_reproduced":math.isclose(r["complete_residual_norm"],14.803880474835667,rel_tol=0,abs_tol=2e-8),
    ...
    return {"artifact":"BHSM_aether_n3_fresh_sbp_post_refined_block_audit_v16_75","version":VERSION,"classification":CLASSIFICATION,"FULL_BHSM_COMPLETE":False,
        "fresh_sbp_post_refined_block_soft_audit":r,"status":"RECLASSIFIED" if all(validation.values()) else "INVALIDATED",
        "real_physical_property_explained":"CURRENT_PARENT_GEOMETRY_OWNER_AND_NORMALIZED_SOFT_MODE_AFTER_REFINED_COMMON_DESCENT",
        "dependency_advanced":"MEASURES_THE_POST_V16_74_N3_BLOCKS_REQUIRED_FOR_SIMULTANEOUS_SADDLE_CLOSURE",
        "active_calculation":"TARGET_THE_DOMINANT_REPORTED_EXISTING_GEOMETRY_BLOCK_WITHIN_THE_SAME_CONSTRAINED_SBP_ORBIT",
...
def materialize(d:str|Path)->Path:
    t=Path(d);t.mkdir(parents=True,exist_ok=True);p=t/"BHSM_aether_n3_fresh_sbp_post_refined_block_audit_v16_75.json";p.write_text(deterministic_json(completion_payload()),encoding="utf-8");return p
__all__=["VERSION","CLASSIFICATION","FULL_BHSM_COMPLETE","v16_74_selected_raw_vector","post_refined_block_soft_audit","completion_payload","deterministic_json","materialize"]
