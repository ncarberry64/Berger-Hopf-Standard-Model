"""Scan both orientations of a bounded geometric probe from v20.20."""
...
VERSION = "v20.22"
CLASSIFICATION = "BHSM_N3_FORTIETH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.768614585778829
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v20_22_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_fortieth_bidirectional_merit_manifold_probe_v20_22.json"
    ).read_text(encoding="utf-8"))
    selected = payload["fortieth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v20.22 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def fortieth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_forty_third_direct_residual_scale_audit_v20_21.json"
    ).read_text(encoding="utf-8"))
    measured = audit["forty_third_direct_residual_scale_audit"]
    ...
    if pair is None:
        ...
        if not bounded_pairs:
            raise ValueError("v20.21 has no bounded direct-response pair")
        pair = min(
            bounded_pairs,
    ...
    return {
        "source_state": "v20.20_thirty_ninth_bidirectional_fallback_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = fortieth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v20_20": result["source_state"].startswith("v20.20"),
    ...
    return {
        "artifact": "BHSM_aether_n3_fortieth_bidirectional_merit_manifold_probe_v20_22",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "fortieth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_fortieth_bidirectional_merit_manifold_probe_v20_22.json"
...
__all__ = [
    ...
    "v20_22_selected_raw_vector", "fortieth_bidirectional_merit_manifold_probe",
