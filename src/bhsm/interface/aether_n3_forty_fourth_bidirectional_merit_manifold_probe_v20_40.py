"""Scan both orientations of a bounded geometric probe from v20.38."""
...
VERSION = "v20.40"
CLASSIFICATION = "BHSM_N3_FORTY_FOURTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.767070408740601
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v20_40_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_forty_fourth_bidirectional_merit_manifold_probe_v20_40.json"
    ).read_text(encoding="utf-8"))
    selected = payload["forty_fourth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v20.40 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def forty_fourth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_forty_seventh_direct_residual_scale_audit_v20_39.json"
    ).read_text(encoding="utf-8"))
    measured = audit["forty_seventh_direct_residual_scale_audit"]
    ...
    if pair is None:
        ...
        if not bounded_pairs:
            raise ValueError("v20.39 has no bounded direct-response pair")
        pair = min(
            bounded_pairs,
    ...
    raw = v20_38_selected_raw_vector()
    ...
    return {
        "source_state": "v20.38_forty_third_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = forty_fourth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v20_38": result["source_state"].startswith("v20.38"),
    ...
    return {
        "artifact": "BHSM_aether_n3_forty_fourth_bidirectional_merit_manifold_probe_v20_40",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "forty_fourth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_forty_fourth_bidirectional_merit_manifold_probe_v20_40.json"
...
__all__ = [
    ...
    "v20_40_selected_raw_vector", "forty_fourth_bidirectional_merit_manifold_probe",
