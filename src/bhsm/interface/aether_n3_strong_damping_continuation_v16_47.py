"""Strong-damping merit continuation from the accepted v16.46 N=3 state."""
...
VERSION = "v16.47"
CLASSIFICATION = "BHSM_N3_STRONGLY_DAMPED_MERIT_CONTINUATION"
FULL_BHSM_COMPLETE = False
FILTER_RELATIVE_SCALES = (1e-8, 3e-8, 1e-7, 3e-7, 1e-6)
STEP_FRACTIONS = (1.0, 0.5, 0.25, 0.125, 0.0625)


def v16_46_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_targeted_filter_continuation_v16_46.json"
    ).read_text(encoding="utf-8"))
    values = payload["targeted_filter_continuation"]["targeted_filter_trial_bank"]["best_accepted"]["raw_vector_hex"]
    raw = np.asarray([float.fromhex(value) for value in values])
    if raw.shape != (376,):
        raise ValueError("v16.46 vector has wrong dimension")
    return raw


def strong_damping_continuation() -> dict[str, Any]:
    matrix, residual, raw = refreshed_system_at(v16_46_raw_vector())
    return {
        "fresh_physical_audit": spectral_and_block_audit_from_system(matrix, residual, raw),
        "strong_damping_trial_bank": refined_filter_trial_bank(
...
def completion_payload() -> dict[str, Any]:
    result = strong_damping_continuation()
    audit = result["fresh_physical_audit"]
    bank = result["strong_damping_trial_bank"]
    best = bank["best_accepted"]
    validation = {
        "accepted_v16_46_residual_reproduced": math.isclose(
            audit["residual_norm"], 6.45252689885623,
            rel_tol=0.0, abs_tol=2e-9,
        ),
        "fresh_physical_jacobian_symmetric": audit["symmetric_relative_residual"] < 1e-14,
        "strong_damping_grid_probed": bank["trial_count"] == 25,
    ...
    return {
        "artifact": "BHSM_aether_n3_strong_damping_continuation_v16_47",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "strong_damping_continuation": result,
        "status": "VALIDATED" if all(validation.values()) else "RECLASSIFIED",
        "real_physical_property_explained": (
            "JOINT_STATIONARITY_OF_THE_EXISTING_HOPF_ANISOTROPY_FIBER_"
            "LOCALIZATION_AND_COMMON_SCALE_EVENT_BACKGROUND"
        ),
        "dependency_advanced": "SIMULTANEOUS_N3_PHYSICAL_EVENT_SADDLE_CLOSURE",
        "active_calculation": (
            "IF_STRONG_DAMPING_DESCENDS_PROMOTE_ITS_STATE;_OTHERWISE_CLASSIFY_"
            "THE_V0_W0_LOG_SCALE_DEFECT_AS_THE_NEXT_UPSTREAM_OBJECT"
        ),
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_strong_damping_continuation_v16_47.json"
...
__all__ = [
    ...
    "FILTER_RELATIVE_SCALES", "STEP_FRACTIONS", "v16_46_raw_vector",
    "strong_damping_continuation", "completion_payload", "deterministic_json",
    "materialize",
]
