"""Promote the v18.11 square-KKT trial after complete-child persistence."""
...
from bhsm.interface.aether_n3_reclassified_chain_proposal_v18_11 import v18_11_selected_raw_vector
...
VERSION = "v18.12"
CLASSIFICATION = "BHSM_N3_SQUARE_KKT_COMPLETE_CHILD_PROMOTION"
FULL_BHSM_COMPLETE = False

_CHILD_Q = np.asarray([
    -0.0004996471139327818, 0.021842643593756556,
    -0.013170601320518656, -0.027252906511162675,
    0.05741315599094109, -0.24544753936142075,
    0.00029371600174913216, -0.08277219914685884,
    0.027889798326213827, 0.13834257285410106,
])
_CHILD_V = np.asarray([
    -0.31647940738400443, -0.14920509766979628,
    0.17270327262661553, -0.024016196871881793,
    -4.0689088873718315, -8.074375896635235,
    -2.0294225343960623, 0.3612980351520471,
    0.6009119691786203, 0.19656937790258203,
])
_CHILD_M = np.asarray([
    1.0487196310646674, 0.058893892680203556,
    0.8875377307258338, -0.1520233309177915,
    -0.1275751722843579, 0.8035925308237062,
])


def v18_12_selected_raw_vector() -> np.ndarray:
    return v18_11_selected_raw_vector().copy()


def square_kkt_complete_child_promotion() -> dict[str, Any]:
    ...
    selected = source["reclassified_chain_proposal"][
    ...
    return {
        "global_step": {
            ...
            "source_proposal_status": source["status"],
...
def completion_payload() -> dict[str, Any]:
    result = square_kkt_complete_child_promotion()
    ...
    validation = {
        ...
        "previous_path_not_a_constraint": not g["must_remain_on_previous_iterate_path"] and c["chart_recomputed_from_all_child_variables"],
    ...
    return {
        "artifact": "BHSM_aether_n3_square_kkt_complete_child_promotion_v18_12",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "square_kkt_complete_child_promotion": result,
        "status": "VALIDATED" if passed else "INVALIDATED",
        "real_physical_property_explained": (
            "A_TRUE_MERIT_DESCENT_STEP_OF_THE_SQUARE_EXPLICIT_MULTIPLIER_"
            "KKT_RECONSTRUCTS_A_COMPLETE_PERSISTENT_MOVING_CHILD"
        ),
        "dependency_advanced": "NONLINEAR_N3_EVENT_SADDLE_CLOSURE",
        "active_calculation": (
            "CONTINUE_THE_SQUARE_376_VARIABLE_KKT_WITH_TOTAL_MERIT_ETA_"
            "AND_COMPLETE_CHILD_ACCEPTANCE_ONLY"
        ),
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_square_kkt_complete_child_promotion_v18_12.json"
...
__all__ = [
    ...
    "v18_12_selected_raw_vector", "square_kkt_complete_child_promotion",
