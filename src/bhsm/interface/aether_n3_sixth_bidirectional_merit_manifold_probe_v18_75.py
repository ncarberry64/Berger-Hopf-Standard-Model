"""Scan both orientations of a bounded geometric probe from v18.73."""
...
from bhsm.interface.aether_n3_fifth_bidirectional_probe_promotion_v18_73 import (
    v18_73_selected_raw_vector,
)
...
VERSION = "v18.75"
CLASSIFICATION = "BHSM_N3_SIXTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.807144219141348
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v18_75_selected_raw_vector() -> np.ndarray:
    payload = json.loads(
        Path(
            "artifacts/"
    ...
    selected = payload["sixth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v18.75 has no exact-merit candidate")
    return np.asarray(
        [float.fromhex(value) for value in selected["raw_vector_hex"]]
    )


def sixth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(
        Path(
            "artifacts/BHSM_aether_n3_ninth_direct_residual_scale_audit_v18_74.json"
        ).read_text(encoding="utf-8")
    )
    measured = audit["ninth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v18.74 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v18_73_selected_raw_vector()
    ...
    return {
        "source_state": "v18.73_fifth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = sixth_bidirectional_merit_manifold_probe()
    selected = result[
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    ...
    validation = {
        "source_is_v18_73": result["source_state"].startswith("v18.73"),
    ...
    return {
        "artifact": (
            "BHSM_aether_n3_sixth_bidirectional_merit_manifold_probe_v18_75"
        ),
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "sixth_bidirectional_merit_manifold_probe": result,
        ...
        "real_physical_property_explained": (
            "A_FRESH_BOUNDED_LOCAL_GEOMETRIC_PROBE_FROM_V18_73_LOCATES_"
