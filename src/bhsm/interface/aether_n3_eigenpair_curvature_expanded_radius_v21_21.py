"""Refresh at v21.20 and test the full BHSM-owned plateau-to-large radius interval."""
...
from bhsm.interface.aether_n3_eigenpair_curvature_predictive_continuation_v21_20 import v21_20_selected_raw_vector
...
from bhsm.interface.aether_n3_isolated_eigenpair_event_hessian_v21_17 import (
    EXACT_RESPONSE_STEP, _local_eigenpair_hessian, _terminal_pullback,
)
from bhsm.interface.aether_n3_rayleigh_event_covector_v20_80 import (
    _terminal_data, rayleigh_sbp_event_covector, rayleigh_square_physical_residual,
)
...
VERSION = "v21.21"
CLASSIFICATION = "BHSM_N3_EIGENPAIR_CURVATURE_EXPANDED_NATURAL_RADIUS"
FULL_BHSM_COMPLETE = False
POINTS = 25


def v21_21_selected_raw_vector() -> np.ndarray:
    ...
    if not payload["promotion"]["promoted"]:
        raise ValueError("v21.21 has no physically promoted state")
    return np.asarray([float.fromhex(v) for v in payload["exact_search"]["best"]["raw_vector_hex"]])


def completion_payload() -> dict[str, Any]:
    raw = v21_20_selected_raw_vector(); scales = kkt_variable_scales(); y = raw * scales
    ...
    if not curvature_valid:
        raise ValueError("v21.20 refreshed eigenpair curvature did not validate")
    ...
    action_radii = prior["bhsm_owned_action_coordinate_radii"]
    physical_radii = _physical_history_radii(scales)
    schedule = []
    for index, fraction in enumerate(np.linspace(0.0, 1.0, POINTS)):
        schedule.append({
            "label": f"PLATEAU_TO_LARGE_{index:02d}",
            "action_radius": float(np.exp(
                (1.0 - fraction) * np.log(action_radii["PLATEAU_DESCENT"])
                + fraction * np.log(action_radii["LARGE_DESCENT"])
            )),
            "physical_radius": float(np.exp(
                (1.0 - fraction) * np.log(physical_radii["PLATEAU_DESCENT"])
                + fraction * np.log(physical_radii["LARGE_DESCENT"])
            )),
        })
    curvature = {
    ...
    result = dual_metric_range_space_proposal(
        raw, source_label="v21.20", curvature_override=curvature,
        radius_schedule_override=schedule,
    )
    result["dual_metric_model"]["curvature_artifact"] = "VALIDATED_V21_21_EIGENPAIR_REFRESH"
    ...
    result["expanded_radius_audit"] = {
        "reason": "THREE_CONSECUTIVE_CORRECTED_PROMOTIONS_SELECTED_THE_PREVIOUS_UPPER_RADIUS_BOUNDARY",
        "uses_only_existing_accepted_history_radii": True,
        "points": POINTS,
    }
    best = result["exact_search"]["best"]
    validation = {
        "source_v21_20_reproduced": abs(source_norm - 0.782142227982532) < 5.0e-12,
        "curvature_validated": curvature_valid,
        "restriction_audit_justified": result["expanded_radius_audit"]["reason"].startswith("THREE_CONSECUTIVE"),
        "history_owned_radii_only": result["expanded_radius_audit"]["uses_only_existing_accepted_history_radii"],
    ...
    return {
        "artifact": "BHSM_N3_EIGENPAIR_CURVATURE_EXPANDED_RADIUS_V21_21", "version": VERSION,
        "classification": CLASSIFICATION, "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "eigenpair_curvature_expanded_radius": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_N3_EIGENPAIR_CURVATURE_EXPANDED_RADIUS_V21_21.json"
    path.write_text(deterministic_json(completion_payload()), encoding="utf-8"); return path


__all__ = ["VERSION", "CLASSIFICATION", "FULL_BHSM_COMPLETE", "v21_21_selected_raw_vector", "completion_payload", "materialize"]
