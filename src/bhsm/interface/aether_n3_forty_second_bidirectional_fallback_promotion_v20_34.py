"""Promote the v20.33 fallback child only after the complete physical gate."""
...
from bhsm.interface.aether_n3_forty_second_bidirectional_fallback_child_v20_33 import (
    v20_33_selected_raw_vector,
)
from bhsm.interface.aether_n3_forty_first_bidirectional_probe_promotion_v20_28 import (
    v20_28_selected_raw_vector,
)


VERSION = "v20.34"
CLASSIFICATION = "BHSM_N3_FORTY_SECOND_BIDIRECTIONAL_FALLBACK_PROMOTION"
FULL_BHSM_COMPLETE = False


def v20_34_selected_raw_vector() -> np.ndarray:
    return v20_33_selected_raw_vector().copy()


def forty_second_bidirectional_fallback_promotion() -> dict[str, Any]:
    reconstruction = json.loads(
        Path(
            "artifacts/"
            "BHSM_aether_n3_forty_second_bidirectional_fallback_child_v20_33.json"
        ).read_text(encoding="utf-8")
    )
    child = reconstruction["forty_second_bidirectional_fallback_child"]
    ...
    source_raw = v20_28_selected_raw_vector()
    raw = v20_34_selected_raw_vector()
        ...
        flux_rows.append(
            {
                ...
                "norm": float(
                    np.linalg.norm(
                        child_flux - (-p_dot + force - event_flux)
                    )
                ),
            }
        )
        ...
        persistence.append(
            {
                ...
                "eta_minimum": eta_legendre_minimum(
                    q, multipliers, points=3000
                )["minimum"],
                "finite": bool(
                    np.all(np.isfinite(np.r_[q, velocity, multipliers]))
                ),
            }
        )
    proposal = json.loads(
        Path(
            "artifacts/"
            "BHSM_aether_n3_forty_second_bidirectional_merit_manifold_probe_v20_30.json"
        ).read_text(encoding="utf-8")
    )
    ...
    return {
        "global_step": {
            "source_solver_interpretation": proposal[
            ...
            "primary_candidate_flux_rejection_preserved": True,
            ...
            "complete_norm_reduction": (
                source_metrics["complete"] - candidate_metrics["complete"]
            ),
        ...
        "persistence": {
            ...
            "all_steps_valid": all(
                row["projection_success"] and row["finite"]
                for row in persistence
            ),
            "nonzero_relative_evolution_retained": bool(
                np.linalg.norm(q - qc) > 0.0
                and np.linalg.norm(velocity) > 0.0
            ),
...
def completion_payload() -> dict[str, Any]:
    result = forty_second_bidirectional_fallback_promotion()
    ...
    validation = {
        ...
        "primary_flux_rejection_preserved": global_step[
            "primary_candidate_flux_rejection_preserved"
        ],
        "bidirectional_line_scan_used": global_step[
            "both_orientations_were_scanned"
        ],
        "independent_exact_residual_recomputed": global_step[
            "independent_exact_residual_recomputed"
        ],
        ...
        "no_componentwise_filter": not global_step[
            "componentwise_monotonicity_required"
        ],
        "previous_path_not_a_constraint": not global_step[
            "must_remain_on_previous_iterate_path"
        ],
        "true_376_merit_reduced": (
            global_step["complete_norm_reduction"] > 0.0
        ),
        "global_eta_preserved": global_step["eta_minimum"] > 1.0e-5,
        "trace_closed": child["maximum_trace_residual"] < 1.0e-9,
        "seven_constraints_closed": (
            child["maximum_seven_constraint_residual"] < 1.0e-9
        ),
        "attachment_momentum_closed": (
            child["attachment_momentum_residual_norm"] < 1.0e-7
        ),
        "resolved_dynamic_flux_closed": (
            child["resolved_dynamic_flux_envelope"] < 2.0e-5
        ),
        "child_eta_hyperregular": (
            child["eta_Legendre_minimum"]["minimum"] > 0.0
        ),
    ...
    return {
        "artifact": (
            "BHSM_aether_n3_forty_second_bidirectional_fallback_promotion_v20_34"
        ),
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "forty_second_bidirectional_fallback_promotion": result,
        "status": "VALIDATED" if passed else "INVALIDATED",
        "real_physical_property_explained": (
            "THE_NEXT_V19_78_EXACT_MERIT_STATE_PASSES_THE_UNCHANGED_"
            "COMPLETE_CHILD_FLUX_AND_PERSISTENCE_GATE_AFTER_THE_PRIMARY_"
            "STATE_IS_REJECTED"
        ),
        ...
        "active_calculation": (
            ...
            if passed
            else "TEST_THE_NEXT_COMPETITIVE_EXACT_MERIT_STATE"
        ),
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / (
        "BHSM_aether_n3_forty_second_bidirectional_fallback_promotion_v20_34.json"
    )
...
__all__ = [
    ...
    "CLASSIFICATION",
    "FULL_BHSM_COMPLETE",
    "v20_34_selected_raw_vector",
    "forty_second_bidirectional_fallback_promotion",
    "completion_payload",
    "materialize",
]
