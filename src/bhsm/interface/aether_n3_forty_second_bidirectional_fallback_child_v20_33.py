"""Reconstruct the next v20.30 child after the v19.80 flux rejection."""
...
VERSION = "v20.33"
CLASSIFICATION = "BHSM_N3_FORTY_SECOND_BIDIRECTIONAL_FALLBACK_CHILD"
FULL_BHSM_COMPLETE = False
FALLBACK_ORIENTATION = "positive"
FALLBACK_BACKTRACK = 14
...
def _fallback_candidate() -> dict[str, Any]:
    ...
    result = payload["forty_second_bidirectional_merit_manifold_probe"]
    return next(
        row
        for row in result["trials"]
        if row.get("true_merit_eligible")
        and row["orientation"] == FALLBACK_ORIENTATION
        and row["backtrack"] == FALLBACK_BACKTRACK
    )


def v20_33_selected_raw_vector() -> np.ndarray:
    selected = _fallback_candidate()
    return np.asarray(
        [float.fromhex(value) for value in selected["raw_vector_hex"]]
    )


def forty_second_bidirectional_fallback_child() -> dict[str, Any]:
    selected = _fallback_candidate()
    raw = v20_33_selected_raw_vector()
    ...
    prior = json.loads(
        Path(
            "artifacts/BHSM_aether_n3_forty_first_bidirectional_probe_child_v20_27.json"
        ).read_text(encoding="utf-8")
    )["forty_first_bidirectional_probe_child"]["child_state"]
    germ = _pack_child(
    ...
    return {
        "line_selection": {
            ...
            "selected_by": (
                "NEXT_LOWEST_INDEPENDENT_EXACT_MERIT_AFTER_V20_32_"
                "FLUX_REJECTION"
            ),
        },
        "rejected_predecessor": {
            "state": "v20.30_primary_candidate",
            "rejection_gate": "INDEPENDENT_TWO_SCALE_FLUX_ENVELOPE",
            "measured_envelope": 2.547828065e-5,
            "retained_limit": 2.0e-5,
            "acceptance_gate_changed": False,
        },
        "source_event": "v20.30_next_exact_merit_candidate",
        "source_solver_interpretation": "INVALIDATED_NOT_REASSERTED",
        "source_germ": "v20.27_child_of_accepted_v20.28_state",
        ...
        "chart": {
            ...
            "row_scaling": (
                "EXISTING_PHYSICAL_ROW_TOLERANCES_NUMERICAL_ONLY_SAME_ZERO_SET"
            ),
...
def completion_payload() -> dict[str, Any]:
    result = forty_second_bidirectional_fallback_child()
    chart = result["chart"]
    rows = result["physical_residuals"]
    validation = {
        "next_exact_merit_candidate_selected": result["line_selection"][
            "selected_by"
        ].startswith("NEXT_LOWEST_INDEPENDENT"),
        "failed_primary_not_promoted": (
            result["rejected_predecessor"]["measured_envelope"]
            > result["rejected_predecessor"]["retained_limit"]
            and not result["rejected_predecessor"]["acceptance_gate_changed"]
        ),
        "invalidated_solver_not_reasserted": (
            result["source_solver_interpretation"]
            == "INVALIDATED_NOT_REASSERTED"
        ),
    ...
    return {
        "artifact": "BHSM_aether_n3_forty_second_bidirectional_fallback_child_v20_33",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "forty_second_bidirectional_fallback_child": result,
        "status": "VALIDATED" if passed else "INVALIDATED",
        "real_physical_property_explained": (
            "THE_NEXT_V19_78_EXACT_MERIT_STATE_SELECTS_A_COMPLETE_MOVING_"
            "CHILD_AFTER_THE_PRIMARY_STATE_FAILS_THE_UNCHANGED_FLUX_GATE"
        ),
...
__all__ = [
    ...
    "forty_second_bidirectional_fallback_child",
    "completion_payload",
    "materialize",
]
