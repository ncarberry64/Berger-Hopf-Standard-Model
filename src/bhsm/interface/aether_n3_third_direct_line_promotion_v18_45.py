"""Promote only the exact physical line state proposed by invalidated v18.43."""
...
from bhsm.interface.aether_n3_second_direct_line_promotion_v18_41 import (
    v18_41_selected_raw_vector,
)
from bhsm.interface.aether_n3_third_direct_residual_jfnk_v18_43 import (
    v18_43_selected_raw_vector,
)


VERSION = "v18.45"
CLASSIFICATION = "BHSM_N3_THIRD_DIRECT_LINE_PROMOTION"
FULL_BHSM_COMPLETE = False


def v18_45_selected_raw_vector() -> np.ndarray:
    return v18_43_selected_raw_vector().copy()


def third_direct_line_promotion() -> dict[str, Any]:
    """Recompute merit, flux, eta, and persistence before physical promotion."""
    reconstruction = json.loads(Path(
        "artifacts/BHSM_aether_n3_third_direct_line_child_v18_44.json"
    ).read_text(encoding="utf-8"))
    child_state = reconstruction["third_direct_line_child"]["child_state"]
    qc = np.asarray(child_state["coordinates"])
    vc = np.asarray(child_state["velocities"])
    mc = np.asarray(child_state["multipliers"])
    scales = kkt_variable_scales()
    source_raw = v18_41_selected_raw_vector()
    raw = v18_45_selected_raw_vector()
    ...
    return {
        ...
        "event_to_complete_child": {
            ...
            "local_chart_rank": reconstruction["third_direct_line_child"]["chart"]["full_chart_rank"],
...
def completion_payload() -> dict[str, Any]:
    result = third_direct_line_promotion()
    ...
    return {
        "artifact": "BHSM_aether_n3_third_direct_line_promotion_v18_45",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "third_direct_line_promotion": result,
        "status": "VALIDATED" if passed else "INVALIDATED",
        "real_physical_property_explained": (
            "THE_V18_43_LINE_STATE_IS_PROMOTED_ONLY_BY_RECOMPUTED_EXACT_MERIT_"
            "ETA_AND_COMPLETE_MOVING_CHILD_PERSISTENCE"
        ),
        "dependency_advanced": "NONLINEAR_N3_EVENT_SADDLE_CLOSURE",
        "active_calculation": (
            "REMEASURE_THE_DIRECT_RESIDUAL_RESPONSE_AND_CONTINUE_FROM_V18_45"
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_third_direct_line_promotion_v18_45.json"
...
__all__ = [
    ...
    "CLASSIFICATION",
    "FULL_BHSM_COMPLETE",
    "v18_45_selected_raw_vector",
    "third_direct_line_promotion",
    "completion_payload",
    "materialize",
]
