"""Scan both orientations of a bounded geometric probe from v18.47."""
...
VERSION = "v18.50"
CLASSIFICATION = "BHSM_N3_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
...
def v18_50_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_bidirectional_merit_manifold_probe_v18_50.json"
    ).read_text(encoding="utf-8"))
    selected = payload["bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v18.50 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_fourth_direct_residual_scale_audit_v18_49.json"
    ).read_text(encoding="utf-8"))
    measured = audit["fourth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v18.49 has no common stable direct-response scale")
...
def completion_payload() -> dict[str, Any]:
    result = bidirectional_merit_manifold_probe()
    ...
    solver_interpretation_valid = bool(
        probe["info"] == 0 and response["resulting_direction_relative_change"] < 5.0e-3
    )
    return {
        "artifact": "BHSM_aether_n3_bidirectional_merit_manifold_probe_v18_50",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "bidirectional_merit_manifold_probe": result,
        ...
        "real_physical_property_explained": "A_BOUNDED_LOCAL_GEOMETRIC_PROBE_CAN_LOCATE_EXACT_NONLINEAR_MERIT_STATES_IN_EITHER_ORIENTATION_WITHOUT_A_NEWTON_CLAIM",
        "dependency_advanced": "NONLINEAR_N3_EVENT_SADDLE_CLOSURE",
        "active_calculation": (
            "RECONSTRUCT_AND_TEST_COMPETITIVE_SELECTED_COMPLETE_CHILD_STATES"
            if selected is not None else "REMEASURE_OR_GENERATE_THE_NEXT_BOUNDED_GEOMETRIC_PROBE"
        ),
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_bidirectional_merit_manifold_probe_v18_50.json"
    path.write_text(deterministic_json(completion_payload()), encoding="utf-8")
    return path


__all__ = ["VERSION", "CLASSIFICATION", "FULL_BHSM_COMPLETE", "v18_50_selected_raw_vector", "bidirectional_merit_manifold_probe", "completion_payload", "materialize"]
