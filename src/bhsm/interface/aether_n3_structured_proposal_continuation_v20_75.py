"""Resume exact N=3 continuation from promoted v20.74 using the recovered proposal mechanism."""
...
def v20_75_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_N3_STRUCTURED_PROPOSAL_CONTINUATION_V20_75.json"
    ).read_text(encoding="utf-8"))
    result = payload["structured_proposal_continuation"]
    if not result["promotion"]["promoted"]:
        raise ValueError("v20.75 candidate was not physically promoted")
    return np.asarray([
        float.fromhex(value)
...
def structured_proposal_continuation() -> dict[str, Any]:
    source_raw = v20_74_selected_raw_vector()
    ...
    for orientation in (-1.0, 1.0):
        for backtrack in range(BACKTRACKS):
            ...
            # The temporary excitation is absent: this starts at the exact v20.74 state.
    ...
    return {
        "source_frontier": {"version": "v20.74", "exact_f376_l2": source_norm},
        "proposal": {
        ...
        "next_action": (
            "CONTINUE_EXACT_N3_CLOSURE_FROM_V20_75"
            if promotion["promoted"]
            else "REVISIT_EXACT_RESIDUAL_OWNERSHIP_AT_V20_74"
        ),
    }
...
def completion_payload() -> dict[str, Any]:
    ...
    validation = {
        "source_v20_74_reproduced": abs(
            result["source_frontier"]["exact_f376_l2"] - 0.758674388483069
        ) < 5.0e-12,
    ...
    return {
        "artifact": "BHSM_N3_STRUCTURED_PROPOSAL_CONTINUATION_V20_75",
        "version": VERSION,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_N3_STRUCTURED_PROPOSAL_CONTINUATION_V20_75.json"
...
__all__ = [
    ...
    "v20_75_selected_raw_vector", "structured_proposal_continuation",
