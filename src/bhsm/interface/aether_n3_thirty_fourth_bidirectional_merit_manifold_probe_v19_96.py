"""Scan both orientations of a bounded geometric probe from v19.94."""
...
from bhsm.interface.aether_n3_thirty_third_bidirectional_probe_promotion_v19_94 import v19_94_selected_raw_vector


VERSION = "v19.96"
CLASSIFICATION = "BHSM_N3_THIRTY_FOURTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.774048801461998
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v19_96_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_thirty_fourth_bidirectional_merit_manifold_probe_v19_96.json"
    ).read_text(encoding="utf-8"))
    selected = payload["thirty_fourth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v19.96 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def thirty_fourth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_thirty_seventh_direct_residual_scale_audit_v19_95.json"
    ).read_text(encoding="utf-8"))
    measured = audit["thirty_seventh_direct_residual_scale_audit"]
    ...
    if pair is None:
        ...
        if not bounded_pairs:
            raise ValueError("v19.95 has no bounded direct-response pair")
        pair = min(
            bounded_pairs,
    ...
    raw = v19_94_selected_raw_vector()
    ...
    return {
        "source_state": "v19.94_thirty_third_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = thirty_fourth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v19_94": result["source_state"].startswith("v19.94"),
    ...
    return {
        "artifact": "BHSM_aether_n3_thirty_fourth_bidirectional_merit_manifold_probe_v19_96",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "thirty_fourth_bidirectional_merit_manifold_probe": result,
        ...
        "real_physical_property_explained": (
            "A_FRESH_BOUNDED_LOCAL_GEOMETRIC_PROBE_FROM_V19_82_LOCATES_"
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_thirty_fourth_bidirectional_merit_manifold_probe_v19_96.json"
...
__all__ = [
    ...
    "v19_96_selected_raw_vector", "thirty_fourth_bidirectional_merit_manifold_probe",
