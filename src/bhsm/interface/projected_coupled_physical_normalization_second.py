"""Fixed output projection before the scalar arithmetic of mixed physical normalization."""
import numpy as np
...
def normalized_mixed(configuration,weights,psi,hard,border,descriptor,cpsi,remainder,
                     axis,transverse,mixed,output_projection,*,coupled_identities_and_variations=False):
    ...
    projection=_balls(output_projection)
    if projection.ndim!=2 or projection.shape[0]<1 or projection.shape[1]!=c.size+p.size+1:
        raise ValueError('finite fixed output projection with complete physical input rows required')
    matrix=arb_mat(*projection.shape,list(projection.flat))
    def project(vector):
        value=matrix*arb_mat(len(vector),1,list(vector))
        return np.array(value.entries(),dtype=object)
    def pieces(configuration,line,response,cpsi,remainder):
        W=project(np.concatenate((np.full(c.size,arb(0),dtype=object),w*line,np.array([cpsi],dtype=object))))
        V=project(np.concatenate((configuration,w*response,np.array([remainder],dtype=object))))
        return W,V
    W,V=pieces(c,p,h,cp,rem)
    U=W+t*V
    result=np.empty((projection.shape[0],count),dtype=object)
    for k in range(count):
        ...
        Wu,Vu=pieces(du['configuration'],du['psi'],du['hard'],du['cpsi'],du['remainder'])
        Wv,Vv=pieces(dv['configuration'],dv['psi'],dv['hard'],dv['cpsi'],dv['remainder'])
        Wx,Vx=pieces(dx['configuration'],dx['psi'],dx['hard'],dx['cpsi'],dx['remainder'])
        Uu=Wu+tu*V+t*Vu
        Uv=Wv+tv*V+t*Vv
        Ux=Wx+tx*V+tu*Vv+tv*Vu+t*Vx
    ...
    return result,dict(border_sign=sign,coupled_identities_through_mixed_second_order_required=True,
        ...
        fixed_output_map_before_scalar_products=True, original_unprojected_physical_norm_retained=True,
