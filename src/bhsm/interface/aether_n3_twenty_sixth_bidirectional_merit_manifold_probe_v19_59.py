"""Scan both orientations of a bounded geometric probe from v19.57."""
...
VERSION = "v19.59"
CLASSIFICATION = "BHSM_N3_TWENTY_SIXTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.788966669806045
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v19_59_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_twenty_sixth_bidirectional_merit_manifold_probe_v19_59.json"
    ).read_text(encoding="utf-8"))
    selected = payload["twenty_sixth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v19.59 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def twenty_sixth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_twenty_ninth_direct_residual_scale_audit_v19_58.json"
    ).read_text(encoding="utf-8"))
    measured = audit["twenty_ninth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v19.58 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v19_57_selected_raw_vector()
    ...
    return {
        "source_state": "v19.57_twenty_fifth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = twenty_sixth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v19_57": result["source_state"].startswith("v19.57"),
    ...
    return {
        "artifact": "BHSM_aether_n3_twenty_sixth_bidirectional_merit_manifold_probe_v19_59",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "twenty_sixth_bidirectional_merit_manifold_probe": result,
        ...
        "real_physical_property_explained": (
            "A_FRESH_BOUNDED_LOCAL_GEOMETRIC_PROBE_FROM_V19_57_LOCATES_"
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_twenty_sixth_bidirectional_merit_manifold_probe_v19_59.json"
...
__all__ = [
    ...
    "v19_59_selected_raw_vector", "twenty_sixth_bidirectional_merit_manifold_probe",
