...
def default_prediction_registry() -> PredictionRegistry:
    ...
    entries = [
        ...
        _entry("neutral_operator_kernel_BH", "Neutral propagation-response kernel", "neutral", "propagation_conditioned_effective_mass", PredictionStatus.CONDITIONAL_THEOREM,
               "The neutral boundary seed now has a conditional action-normalized spectral-gap theorem shape. Numeric sqrt(A_nu/Z_nu), physical K_neutral,eff, and admissible positivity remain open, so no physical mass is produced.", theorem="CONDITIONAL_NEUTRAL_SPECTRAL_MASS_CANDIDATE",
