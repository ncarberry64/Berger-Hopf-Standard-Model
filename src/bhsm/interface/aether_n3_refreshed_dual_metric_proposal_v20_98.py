"""Test one exact proposal using Rayleigh curvature refreshed at v20.95."""
...
def v20_98_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_N3_REFRESHED_DUAL_METRIC_PROPOSAL_V20_98.json"
    ).read_text(encoding="utf-8"))["refreshed_dual_metric_proposal"]
    if not payload["promotion"]["promoted"]:
        raise ValueError("v20.98 has no physically promoted state")
    return np.asarray([
...
def completion_payload() -> dict[str, Any]:
    ...
    validation = {
        ...
        "v20_97_curvature_used": result["dual_metric_model"][
            "curvature_artifact"
        ].endswith("V20_97.json"),
    ...
    return {
        "artifact": "BHSM_N3_REFRESHED_DUAL_METRIC_PROPOSAL_V20_98",
        "version": VERSION,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_N3_REFRESHED_DUAL_METRIC_PROPOSAL_V20_98.json"
    path.write_text(deterministic_json(completion_payload()), encoding="utf-8")
    return path


__all__ = [
    "VERSION", "CLASSIFICATION", "FULL_BHSM_COMPLETE", "v20_98_selected_raw_vector",
