"""Apply the remeasured v18.42 direct residual response at v18.41."""
...
VERSION = "v18.43"
CLASSIFICATION = "BHSM_N3_THIRD_DIRECT_RESIDUAL_JFNK"
...
def v18_43_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_third_direct_residual_jfnk_v18_43.json"
    ).read_text(encoding="utf-8"))
    selected = payload["third_direct_residual_jfnk"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v18.43 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def third_direct_residual_jfnk() -> dict[str, Any]:
    """Propose a line using only the measured exact-residual JVP."""
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_third_direct_residual_scale_audit_v18_42.json"
    ).read_text(encoding="utf-8"))
    measured = audit["third_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v18.42 has no common stable direct-response scale")
    ...
    return {
        "source_state": "v18.41_second_direct_line_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = third_direct_residual_jfnk()
    ...
    validation = {
        ...
        "source_norm_reproduced": abs(
            result["source_complete_norm"] - 0.824262386198657
        ) < 5.0e-12,
    ...
    return {
        "artifact": "BHSM_aether_n3_third_direct_residual_jfnk_v18_43",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "third_direct_residual_jfnk": result,
        "status": (
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_third_direct_residual_jfnk_v18_43.json"
