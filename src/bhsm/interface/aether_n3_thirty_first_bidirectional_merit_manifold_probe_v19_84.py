"""Scan both orientations of a bounded geometric probe from v19.82."""
...
VERSION = "v19.84"
CLASSIFICATION = "BHSM_N3_THIRTY_FIRST_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.781663574515915
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v19_84_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_thirty_first_bidirectional_merit_manifold_probe_v19_84.json"
    ).read_text(encoding="utf-8"))
    selected = payload["thirty_first_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v19.84 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def thirty_first_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_thirty_fourth_direct_residual_scale_audit_v19_83.json"
    ).read_text(encoding="utf-8"))
    measured = audit["thirty_fourth_direct_residual_scale_audit"]
    ...
    if pair is None:
        ...
        if not bounded_pairs:
            raise ValueError("v19.83 has no bounded direct-response pair")
        pair = min(
            bounded_pairs,
    ...
    return {
        "source_state": "v19.82_thirtieth_bidirectional_fallback_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = thirty_first_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v19_82": result["source_state"].startswith("v19.82"),
    ...
    return {
        "artifact": "BHSM_aether_n3_thirty_first_bidirectional_merit_manifold_probe_v19_84",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "thirty_first_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_thirty_first_bidirectional_merit_manifold_probe_v19_84.json"
...
__all__ = [
    ...
    "v19_84_selected_raw_vector", "thirty_first_bidirectional_merit_manifold_probe",
