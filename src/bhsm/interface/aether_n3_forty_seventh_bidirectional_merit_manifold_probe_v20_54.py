"""Scan both orientations of a bounded geometric probe from v20.52."""
...
VERSION = "v20.54"
CLASSIFICATION = "BHSM_N3_FORTY_SEVENTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.76702869003898
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v20_54_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_forty_seventh_bidirectional_merit_manifold_probe_v20_54.json"
    ).read_text(encoding="utf-8"))
    selected = payload["forty_seventh_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v20.54 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def forty_seventh_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_fiftieth_direct_residual_scale_audit_v20_53.json"
    ).read_text(encoding="utf-8"))
    measured = audit["fiftieth_direct_residual_scale_audit"]
    ...
    if pair is None:
        ...
        if not bounded_pairs:
            raise ValueError("v20.53 has no bounded direct-response pair")
        pair = min(
            bounded_pairs,
    ...
    return {
        "source_state": "v20.52_forty_sixth_bidirectional_fallback_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = forty_seventh_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v20_52": result["source_state"].startswith("v20.52"),
    ...
    return {
        "artifact": "BHSM_aether_n3_forty_seventh_bidirectional_merit_manifold_probe_v20_54",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "forty_seventh_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_forty_seventh_bidirectional_merit_manifold_probe_v20_54.json"
...
__all__ = [
    ...
    "v20_54_selected_raw_vector", "forty_seventh_bidirectional_merit_manifold_probe",
