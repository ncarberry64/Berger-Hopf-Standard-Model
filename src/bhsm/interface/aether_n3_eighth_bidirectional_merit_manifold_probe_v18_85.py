"""Scan both orientations of a bounded geometric probe from v18.83."""
...
VERSION = "v18.85"
CLASSIFICATION = "BHSM_N3_EIGHTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.80554785212226
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v18_85_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_eighth_bidirectional_merit_manifold_probe_v18_85.json"
    ).read_text(encoding="utf-8"))
    selected = payload["eighth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v18.85 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def eighth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_eleventh_direct_residual_scale_audit_v18_84.json"
    ).read_text(encoding="utf-8"))
    measured = audit["eleventh_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v18.84 has no common stable direct-response scale")
    ...
    return {
        "source_state": "v18.83_seventh_bidirectional_fallback_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = eighth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v18_83": result["source_state"].startswith("v18.83"),
    ...
    return {
        "artifact": "BHSM_aether_n3_eighth_bidirectional_merit_manifold_probe_v18_85",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "eighth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_eighth_bidirectional_merit_manifold_probe_v18_85.json"
...
__all__ = [
    ...
    "v18_85_selected_raw_vector", "eighth_bidirectional_merit_manifold_probe",
