...
def _basis() -> list[dict[str, Any]]:
    ...
    return [
        ...
        record(
            ...
            "Delta=Dlambda[b_Psi*Psi+s*V_hard]; abs(D_i*Delta_exact)>=abs(D_i*Delta_center)-r_seed-sup_row_i*(D2Delta)*r_tube",
            "CANCELLATION_PRESERVING_ONE_ROW_REDUCTION",
            "MATHEMATICAL_OBJECT",
            "The signed denominator recombines exactly before norms, and zero exclusion needs only the dominant action-coordinate Hessian row. Two direct meshes place that diagnostic row about 8.6e5 below the rigorous resolving ceiling, but mesh agreement is not an interval remainder theorem.",
            "first matrix-Lohner chart around stored proof node 1214",
            [p_ddelta_row],
            current_status="DIRECT_IDENTITY_AND_ONE_ROW_SUFFICIENCY_DERIVED_RIGOROUS_ROW_86_REMAINDER_OPEN",
            downstream_consumers=["C2_SIGNED_DURATION_INCIDENCE_OWNER", "G7_08_FORCE"],
            forbidden_interpretations=["two-mesh agreement is an interval certificate", "the diagnostic row closes signed D_Y Delta", "a full 98 by 98 D2Delta norm remains necessary", "proof node 1214 is an exact physical history"],
        ),
        record(
