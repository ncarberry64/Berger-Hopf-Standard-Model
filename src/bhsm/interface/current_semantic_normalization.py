...
GATE_CHAIN = [
    ...
    ("G7_08_FORCE", "heat-minus-zeta functional, physical quotient criterion, fixed-stratum regularity, quotient-robust radius variation, moving-endpoint chain rules, inverse-free Weyl solver, maximal Friedrichs value exhaustion, and nested channel/Euler-Dirac adjoint pullback are derived; fixed-channel source-Dini and high-energy trace control remain closed; every history captured by the proved H4-to-H0-positive asymptotic NHIM has finite optical length and therefore fails the required absolute graded angular source sum, so that infinite NHIM route cannot supply the physical force through the currently proved absolute source class even if reset entry were proved; the analytic full 57 by 196 reset Jacobian and intrinsic continuation produced a 58-row child terminal-event candidate, and the directed Y/Z0 audit plus retained-action Z2 bound now close a contractive terminal root ball; selected-line simplicity, Legendre, canonical-lift, reset regularity, and the cancellation-preserving c_psi<0<b_psi orientation all transfer to the exact root, proving an incoming local finite-positive-time event-reaching germ without recurrence or universal reachability; the compatibility audit shows that this root is a T=0 reset/event incidence rather than a T>0 history from the physical positive-duration reset image to a later endpoint; the current owner is therefore certification of that positive-duration connecting BVP, followed by compact operator/first reset-quotient jet realization and evaluation of the heat-minus-zeta covector; no physical child is selected", "OPEN_CURRENT_OWNER"),
    ("G7_09_SADDLE", "same-action finite-endpoint forward-adjoint quotient KKT equations are derived but unsolved; the 58-row finite terminal root ball and its incoming orientation remain certified, while the positive-duration reset-to-later-event connecting BVP is open, so the saddle route awaits that connection and the actual compact-endpoint heat-minus-zeta force evaluation and remains mathematically coupled to G7_08 without adding a gate", "PENDING_COUPLED_TO_G7_08"),
