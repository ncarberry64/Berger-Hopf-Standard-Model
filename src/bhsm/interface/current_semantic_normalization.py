...
def _basis() -> list[dict[str, Any]]:
    ...
    return [
        ...
        record(
            ...
            "b_Psi=<Psi,f>; V_hard=sum_(a!=selected) Psi_a<Psi_a,f>/(lambda_a-lambda); Delta=Dlambda[b_Psi*Psi+s*V_hard]",
            "CANCELLATION_PRESERVING_ONE_ROW_REDUCTION",
            "MATHEMATICAL_OBJECT",
            "The selected-line coefficient is the inverse-free local contraction <Psi,f>, the hard response is confined to the spectral complement, and the signed denominator recombines exactly before norms. Zero exclusion needs only the dominant action-coordinate Hessian row. Two direct meshes place that diagnostic row about 8.6e5 below the rigorous resolving ceiling, but mesh agreement is not an interval remainder theorem.",
            "first matrix-Lohner chart around stored proof node 1214",
            [p_ddelta_row],
            current_status="INVERSE_FREE_b_AND_DIRECT_DELTA_ONE_ROW_SUFFICIENCY_DERIVED_RIGOROUS_ROW_86_REMAINDER_OPEN",
            downstream_consumers=["C2_SIGNED_DURATION_INCIDENCE_OWNER", "G7_08_FORCE"],
