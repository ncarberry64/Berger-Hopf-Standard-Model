...
def _basis() -> list[dict[str, Any]]:
    ...
    return [
        ...
        record(
            ...
            "b_Psi=<Psi,f>; Delta=Dlambda[b_Psi*Psi+s*V_hard]; z=(lambda-H)_hard^-1 Qg; D_ih(cb)=b*c_ih+b_i*c_h+c_i*b_h+c*b_ih with c_ih and b_ih assembled from finite local jets and hard adjoints",
            "CANCELLATION_PRESERVING_ONE_ROW_REDUCTION",
            "MATHEMATICAL_OBJECT",
            "The selected-line coefficient is the inverse-free local contraction <Psi,f>, the hard response is confined to the spectral complement, and the signed denominator recombines exactly before norms. Zero exclusion needs only the dominant action-coordinate Hessian row. Every moving Psi_h and Psi_ih occurrence in its dominant bc sector is eliminated exactly in favor of finite local action/source jets and fixed hard adjoints; at the reference center the first structured adjoint is over three orders below the gap-only estimate. Two direct meshes place the diagnostic row about 8.6e5 below the rigorous resolving ceiling, but neither center calculation is an interval remainder theorem.",
            "first matrix-Lohner chart around stored proof node 1214",
            [p_ddelta_row],
            current_status="COMPLETE_LOCAL_ADJOINT_bc_ROW_ASSEMBLY_DERIVED_RIGOROUS_ROW_86_REMAINDER_OPEN",
            downstream_consumers=["C2_SIGNED_DURATION_INCIDENCE_OWNER", "G7_08_FORCE"],
            forbidden_interpretations=["two-mesh agreement or the center hard adjoint is an interval certificate", "the diagnostic row closes signed D_Y Delta", "a full 98 by 98 D2Delta norm or a moving eigenline derivative matrix remains necessary for the bc row", "proof node 1214 is an exact physical history"],
        ),
        record(
