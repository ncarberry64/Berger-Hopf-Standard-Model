...
def _basis() -> list[dict[str, Any]]:
    ...
    return [
        ...
        record(
            ...
            "MATHEMATICAL_OBJECT",
            "The signed log-radius and log-lapse covectors and the moving-duration incidence formula are exact; the minimal local missing duration datum is the signed selected-line/hard-complement covector D_Y Delta, because the stored continuation data contain only its norm bound.",
            "regular positive-lapse positive-Delta C2 fixed-s family",
            [p_duration_incidence],
            current_status="DERIVED_SIGNED_D_Y_DELTA_AND_TRANSPOSED_SEGMENT_ACTION_OPEN",
