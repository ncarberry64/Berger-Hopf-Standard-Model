"""Scan both orientations of a bounded geometric probe from v19.29."""
...
VERSION = "v19.31"
CLASSIFICATION = "BHSM_N3_NINETEENTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.793187079982019
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v19_31_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_nineteenth_bidirectional_merit_manifold_probe_v19_31.json"
    ).read_text(encoding="utf-8"))
    selected = payload["nineteenth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v19.31 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def nineteenth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_twenty_second_direct_residual_scale_audit_v19_30.json"
    ).read_text(encoding="utf-8"))
    measured = audit["twenty_second_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v19.30 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v19_29_selected_raw_vector()
    ...
    return {
        "source_state": "v19.29_eighteenth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = nineteenth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v19_29": result["source_state"].startswith("v19.29"),
    ...
    return {
        "artifact": "BHSM_aether_n3_nineteenth_bidirectional_merit_manifold_probe_v19_31",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "nineteenth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_nineteenth_bidirectional_merit_manifold_probe_v19_31.json"
...
__all__ = [
    ...
    "v19_31_selected_raw_vector", "nineteenth_bidirectional_merit_manifold_probe",
