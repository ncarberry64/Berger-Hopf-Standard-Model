"""Scan both orientations of a bounded geometric probe from v19.49."""
...
VERSION = "v19.51"
CLASSIFICATION = "BHSM_N3_TWENTY_FOURTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.790602144149231
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v19_51_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_twenty_fourth_bidirectional_merit_manifold_probe_v19_51.json"
    ).read_text(encoding="utf-8"))
    selected = payload["twenty_fourth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v19.51 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def twenty_fourth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_twenty_seventh_direct_residual_scale_audit_v19_50.json"
    ).read_text(encoding="utf-8"))
    measured = audit["twenty_seventh_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v19.50 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v19_49_selected_raw_vector()
    ...
    return {
        "source_state": "v19.49_twenty_third_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = twenty_fourth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v19_49": result["source_state"].startswith("v19.49"),
    ...
    return {
        "artifact": "BHSM_aether_n3_twenty_fourth_bidirectional_merit_manifold_probe_v19_51",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "twenty_fourth_bidirectional_merit_manifold_probe": result,
        ...
        "real_physical_property_explained": (
            "A_FRESH_BOUNDED_LOCAL_GEOMETRIC_PROBE_FROM_V19_49_LOCATES_"
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_twenty_fourth_bidirectional_merit_manifold_probe_v19_51.json"
...
__all__ = [
    ...
    "v19_51_selected_raw_vector", "twenty_fourth_bidirectional_merit_manifold_probe",
