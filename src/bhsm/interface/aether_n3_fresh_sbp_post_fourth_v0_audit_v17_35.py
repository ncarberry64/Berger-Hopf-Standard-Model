"""Complete block and soft-spectrum audit after v17.34 continuation."""
...
VERSION = "v17.35"
CLASSIFICATION = "BHSM_N3_FRESH_SBP_POST_FOURTH_V0_COMPLETE_BLOCK_SOFT_AUDIT"
FULL_BHSM_COMPLETE = False


def v17_34_selected_raw_vector() -> np.ndarray:
    payload = json.loads(
        Path(
            "artifacts/BHSM_aether_n3_fresh_sbp_fourth_v0_priority_v17_34.json"
        ).read_text(encoding="utf-8")
    )
    values = payload["fresh_sbp_fourth_v0_priority"][
        "selected_v0_priority_maximin"
    ]["raw_vector_hex"]
    raw = np.asarray([float.fromhex(value) for value in values])
    if raw.shape != (376,):
        raise ValueError("v17.34 selected vector has wrong dimension")
    return raw


def post_fourth_v0_audit() -> dict[str, Any]:
    raw = v17_34_selected_raw_vector()
    ...
    return {
        "source_state": "v17.34_selected_fourth_v0_priority_state",
        "block_norms": block,
        ...
        "eta_minimum_provenance": 0.774702253521857,
        "scale_rows_retained": 23,
        "nonlinear_limiter_transition": "V0_REMAINS_LIMITING_AT_V17_34",
        "interpretation": "CONTINUE_BOUNDED_V0_PRIORITY_FROM_CURRENT_FULL_RESIDUAL",
    }


def completion_payload() -> dict[str, Any]:
    result = post_fourth_v0_audit()
    block = result["block_norms"]
    soft = result["terminal_soft_mode"]
    validation = {
        "v17_34_residual_reproduced": math.isclose(
            block["complete"], 1.001308539666789, rel_tol=0, abs_tol=2e-8
        ),
        "v17_34_event_reproduced": math.isclose(
            block["event"], 0.08752101877492, rel_tol=0, abs_tol=2e-8
        ),
        ...
        "limiter_status_recorded": (
            result["nonlinear_limiter_transition"]
            == "V0_REMAINS_LIMITING_AT_V17_34"
        ),
        "simultaneous_closure_not_claimed": block["complete"] > 1e-6,
    }
    return {
        "artifact": "BHSM_aether_n3_fresh_sbp_post_fourth_v0_audit_v17_35",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": False,
        "fresh_sbp_post_fourth_v0_audit": result,
        ...
        "real_physical_property_explained": (
            ...
            "FOURTH_V0_PRIORITY_PASS"
        ),
        "dependency_advanced": (
            "RETARGETS_THE_NEXT_SIMULTANEOUS_N3_EVENT_SADDLE_CORRECTION"
        ),
        "active_calculation": "BUILD_A_FIFTH_BOUNDED_V0_PRIORITY_FAMILY_FROM_V17_34",
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_fresh_sbp_post_fourth_v0_audit_v17_35.json"
