"""Scan both orientations of a bounded geometric probe from v18.58."""
...
VERSION = "v18.62"
CLASSIFICATION = "BHSM_N3_THIRD_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
...
def v18_62_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path("artifacts/BHSM_aether_n3_third_bidirectional_merit_manifold_probe_v18_62.json").read_text(encoding="utf-8"))
    selected = payload["third_bidirectional_merit_manifold_probe"]["selected_true_merit_candidate_pending_child_acceptance"]
    if selected is None:
        raise ValueError("v18.62 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def third_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path("artifacts/BHSM_aether_n3_sixth_direct_residual_scale_audit_v18_61.json").read_text(encoding="utf-8"))
    measured = audit["sixth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v18.61 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v18_58_selected_raw_vector()
    ...
    return {
        "source_state": "v18.58_second_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = third_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v18_58": result["source_state"].startswith("v18.58"),
        "source_norm_reproduced": abs(result["source_complete_norm"] - 0.815925953107132) < 5.0e-12,
    ...
    return {
        "artifact": "BHSM_aether_n3_third_bidirectional_merit_manifold_probe_v18_62",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "third_bidirectional_merit_manifold_probe": result,
        ...
        "real_physical_property_explained": "A_FRESH_BOUNDED_LOCAL_GEOMETRIC_PROBE_FROM_V18_58_LOCATES_EXACT_NONLINEAR_MERIT_STATES_WITHOUT_A_NEWTON_CLAIM",
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_third_bidirectional_merit_manifold_probe_v18_62.json"
    path.write_text(deterministic_json(completion_payload()), encoding="utf-8")
    return path


__all__ = ["VERSION", "CLASSIFICATION", "FULL_BHSM_COMPLETE", "v18_62_selected_raw_vector", "third_bidirectional_merit_manifold_probe", "completion_payload", "materialize"]
