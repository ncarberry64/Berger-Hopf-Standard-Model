"""Promote the v18.22 global step after v18.24 child persistence."""
...
VERSION = "v18.25"
CLASSIFICATION = "BHSM_N3_DIRECTIONAL_COMPLETE_CHILD_PROMOTION"
FULL_BHSM_COMPLETE = False


def v18_25_selected_raw_vector() -> np.ndarray:
    return v18_22_selected_raw_vector().copy()


def directional_complete_child_promotion() -> dict[str, Any]:
    reconstruction = json.loads(Path(
        "artifacts/BHSM_aether_n3_complete_child_chart_reconstruction_v18_24.json"
    ).read_text(encoding="utf-8"))
    child_state = reconstruction["complete_child_chart_reconstruction"]["child_state"]
    qc = np.asarray(child_state["coordinates"]); vc = np.asarray(child_state["velocities"]); mc = np.asarray(child_state["multipliers"])
    raw = v18_25_selected_raw_vector(); state = unpack_reduced(raw)
    ...
    pe, _, le, _ = _canonical_pair(qe, ve, me); pc, force, lc, _ = _canonical_pair(qc, vc, mc)
    ec, _ = _metric_radial_flux_covector(qe, me); cc, _ = _metric_radial_flux_covector(qc, mc)
    ...
    for relative_step in (8.0e-4, 4.0e-4):
        ...
        flux_rows.append({"relative_step": relative_step, "norm": float(np.linalg.norm(child_flux - (-p_dot + force - event_flux)))})
    q, v, m = qc.copy(), vc.copy(), mc.copy(); persistence = []
    ...
    trace = _trace_jacobian() @ (qc - qe); constraints = constraint_residual(3, qc, vc, mc, points=44)
    return {
        "global_step": {
            ...
            "event_magnitude": selected["metrics"]["event"],
        ...
        "event_to_complete_child": {
            ...
            "local_chart_rank": reconstruction["complete_child_chart_reconstruction"]["chart"]["full_chart_rank"],
...
def completion_payload() -> dict[str, Any]:
    result = directional_complete_child_promotion(); g = result["global_step"]; c = result["event_to_complete_child"]; p = result["persistence"]
    validation = {
    ...
    return {
        "artifact": "BHSM_aether_n3_directional_complete_child_promotion_v18_25",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "directional_complete_child_promotion": result,
        "status": "VALIDATED" if passed else "INVALIDATED",
        "real_physical_property_explained": "THE_EXACT_DIRECTIONAL_MERIT_STEP_PRODUCES_A_RECOMPUTED_COMPLETE_PERSISTENT_MOVING_CHILD",
        "dependency_advanced": "NONLINEAR_N3_EVENT_SADDLE_CLOSURE",
        "active_calculation": "CONTINUE_EXACT_ACTION_DIRECTIONAL_EVENT_MERIT_DESCENT_FROM_V18_25",
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_directional_complete_child_promotion_v18_25.json"
    path.write_text(deterministic_json(completion_payload()), encoding="utf-8")
    return path


__all__ = ["VERSION", "CLASSIFICATION", "FULL_BHSM_COMPLETE", "v18_25_selected_raw_vector", "directional_complete_child_promotion", "completion_payload", "materialize"]
