"""Recompute the 14-row complete-child local chart at the v18.22 event."""
...
from bhsm.interface.aether_n3_constraint_solved_orbit_v16_08 import eta_legendre_minimum, exact_euler_dirac_acceleration
...
VERSION = "v18.24"
CLASSIFICATION = "BHSM_N3_COMPLETE_CHILD_CHART_RECONSTRUCTION"
FULL_BHSM_COMPLETE = False
JACOBIAN_STEP = 2.0e-4


def _pack_child(q: np.ndarray, v: np.ndarray, m: np.ndarray) -> np.ndarray:
    return np.concatenate((q, v, m))


def _unpack_child(z: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    value = np.asarray(z, dtype=float)
    return value[:10], value[10:20], value[20:]


def _child_rows(z: np.ndarray, qe: np.ndarray, pe: np.ndarray, event_flux: np.ndarray) -> np.ndarray:
    q, v, m = _unpack_child(z)
    pc, force, lc, _ = _canonical_pair(q, v, m)
    child_covector, _ = _metric_radial_flux_covector(q, m)
    child_flux = lc.T @ child_covector
    ...
    tangent_scale = max(1.0, float(np.max(np.abs(v))), float(np.max(np.abs(acceleration))), float(np.max(np.abs(multiplier_rate))))
    epsilon = 4.0e-4 / tangent_scale
    plus, _, _, _ = _canonical_pair(q + epsilon * v, v + epsilon * acceleration, m + epsilon * multiplier_rate)
    minus, _, _, _ = _canonical_pair(q - epsilon * v, v - epsilon * acceleration, m - epsilon * multiplier_rate)
    momentum_rate = (plus - minus) / (2.0 * epsilon)
    flux = child_flux - (-momentum_rate + force - event_flux)
    return np.concatenate((
        _trace_jacobian() @ (q - qe),
...
def complete_child_chart_reconstruction() -> dict[str, Any]:
    raw = v18_22_selected_raw_vector(); state = unpack_reduced(raw)
    ...
    germ = _pack_child(_CHILD_Q, _CHILD_V, _CHILD_M)
    ...
    _, triangular, pivots = qr(scaled_jacobian, mode="economic", pivoting=True)
    ...
    fixed = germ.copy(); initial_chart = germ[chart].copy()

    def residual(chart_values: np.ndarray) -> np.ndarray:
        value = fixed.copy(); value[chart] = chart_values
        return _child_rows(value, qe, pe, event_flux) / row_scales

    solution = least_squares(
        residual, initial_chart, method="lm",
        ftol=1.0e-13, xtol=1.0e-13, gtol=1.0e-13,
        max_nfev=900,
    )
    ...
    return {
        "source_event": "v18.22_exact_directional_merit_candidate",
...
def completion_payload() -> dict[str, Any]:
    result = complete_child_chart_reconstruction(); chart = result["chart"]; rows = result["physical_residuals"]
    validation = {
        ...
        "numerical_row_scaling_does_not_change_zero_set": "SAME_ZERO_SET" in chart["row_scaling"],
    ...
    return {
        "artifact": "BHSM_aether_n3_complete_child_chart_reconstruction_v18_24",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "complete_child_chart_reconstruction": result,
        "status": "VALIDATED" if passed else "INVALIDATED",
        "real_physical_property_explained": "THE_V18_22_EVENT_SELECTS_A_NEW_COMPLETE_MOVING_CHILD_ON_A_RECOMPUTED_FULL_RANK_LOCAL_CHART",
        "dependency_advanced": "NONLINEAR_N3_EVENT_SADDLE_CLOSURE",
        "active_calculation": "VERIFY_FLUX_ENVELOPE_AND_POSITIVE_DURATION_PERSISTENCE_THEN_PROMOTE_THE_GLOBAL_STEP" if passed else "RESOLVE_THE_LOCAL_COMPLETE_CHILD_BVP",
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_complete_child_chart_reconstruction_v18_24.json"
    path.write_text(deterministic_json(completion_payload()), encoding="utf-8")
    return path


__all__ = ["VERSION", "CLASSIFICATION", "FULL_BHSM_COMPLETE", "complete_child_chart_reconstruction", "completion_payload", "materialize"]
