"""Scan both orientations of a bounded geometric probe from v19.33."""
...
VERSION = "v19.35"
CLASSIFICATION = "BHSM_N3_TWENTIETH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.792728134993666
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v19_35_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_twentieth_bidirectional_merit_manifold_probe_v19_35.json"
    ).read_text(encoding="utf-8"))
    selected = payload["twentieth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v19.35 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def twentieth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_twenty_third_direct_residual_scale_audit_v19_34.json"
    ).read_text(encoding="utf-8"))
    measured = audit["twenty_third_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v19.34 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v19_33_selected_raw_vector()
    ...
    return {
        "source_state": "v19.33_nineteenth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = twentieth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v19_33": result["source_state"].startswith("v19.33"),
    ...
    return {
        "artifact": "BHSM_aether_n3_twentieth_bidirectional_merit_manifold_probe_v19_35",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "twentieth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_twentieth_bidirectional_merit_manifold_probe_v19_35.json"
...
__all__ = [
    ...
    "v19_35_selected_raw_vector", "twentieth_bidirectional_merit_manifold_probe",
