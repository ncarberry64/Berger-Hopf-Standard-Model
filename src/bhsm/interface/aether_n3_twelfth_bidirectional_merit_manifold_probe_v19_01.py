"""Scan both orientations of a bounded geometric probe from v18.99."""
...
from bhsm.interface.aether_n3_eleventh_bidirectional_probe_promotion_v18_99 import v18_99_selected_raw_vector


VERSION = "v19.01"
CLASSIFICATION = "BHSM_N3_TWELFTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.801038620295453
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v19_01_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_twelfth_bidirectional_merit_manifold_probe_v19_01.json"
    ).read_text(encoding="utf-8"))
    selected = payload["twelfth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v19.01 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def twelfth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_fifteenth_direct_residual_scale_audit_v19_00.json"
    ).read_text(encoding="utf-8"))
    measured = audit["fifteenth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v19.00 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v18_99_selected_raw_vector()
    ...
    return {
        "source_state": "v18.99_eleventh_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = twelfth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v18_99": result["source_state"].startswith("v18.99"),
    ...
    return {
        "artifact": "BHSM_aether_n3_twelfth_bidirectional_merit_manifold_probe_v19_01",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "twelfth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_twelfth_bidirectional_merit_manifold_probe_v19_01.json"
...
__all__ = [
    ...
    "v19_01_selected_raw_vector", "twelfth_bidirectional_merit_manifold_probe",
