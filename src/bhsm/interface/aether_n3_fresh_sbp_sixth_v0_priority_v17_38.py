"""Fresh v0-priority continuation from the validated v17.37 state."""
...
VERSION = "v17.38"
CLASSIFICATION = "BHSM_N3_FRESH_SBP_SIXTH_V0_PRIORITY_MEASURED_TANGENT_FAMILY"
FULL_BHSM_COMPLETE = False


def v17_37_selected_raw_vector() -> np.ndarray:
    payload = json.loads(
        Path(
            "artifacts/BHSM_aether_n3_fresh_sbp_third_period_priority_v17_37.json"
        ).read_text(encoding="utf-8")
    )
    values = payload["fresh_sbp_third_period_priority"][
        "selected_period_priority_maximin"
    ]["raw_vector_hex"]
    raw = np.asarray([float.fromhex(value) for value in values])
    if raw.shape != (376,):
        raise ValueError("v17.37 selected vector has wrong dimension")
    return raw


def sixth_v0_priority() -> dict[str, Any]:
    return period_priority_family_from(
        v17_37_selected_raw_vector(),
        source_state="v17.37_selected_third_period_priority_state",
        priority_owner="v0",
...
def completion_payload() -> dict[str, Any]:
    result = sixth_v0_priority()
    best = result["selected_v0_priority_maximin"]
    validation = {
        "v17_37_residual_reproduced": math.isclose(
            result["initial_metrics"]["complete"],
            0.979916849149738,
            rel_tol=0,
            abs_tol=2e-8,
        ),
        "v17_37_event_reproduced": math.isclose(
            result["initial_metrics"]["event"],
            0.086613377720454,
            rel_tol=0,
            abs_tol=2e-8,
        ),
        "v17_37_v0_reproduced": math.isclose(
        ...
        "source_state_owned": (
            result["source_state"] == "v17.37_selected_third_period_priority_state"
        ),
    ...
    return {
        "artifact": "BHSM_aether_n3_fresh_sbp_sixth_v0_priority_v17_38",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": False,
        "fresh_sbp_sixth_v0_priority": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_fresh_sbp_sixth_v0_priority_v17_38.json"
