"""Scan both orientations of a bounded geometric probe from v19.41."""
...
VERSION = "v19.43"
CLASSIFICATION = "BHSM_N3_TWENTY_SECOND_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
...
def v19_43_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_twenty_second_bidirectional_merit_manifold_probe_v19_43.json"
    ).read_text(encoding="utf-8"))
    selected = payload["twenty_second_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v19.43 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def twenty_second_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_twenty_fifth_direct_residual_scale_audit_v19_42.json"
    ).read_text(encoding="utf-8"))
    measured = audit["twenty_fifth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v19.42 has no common stable direct-response scale")
...
def completion_payload() -> dict[str, Any]:
    result = twenty_second_bidirectional_merit_manifold_probe()
    ...
    return {
        "artifact": "BHSM_aether_n3_twenty_second_bidirectional_merit_manifold_probe_v19_43",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "twenty_second_bidirectional_merit_manifold_probe": result,
        ...
        "real_physical_property_explained": (
            "A_FRESH_BOUNDED_LOCAL_GEOMETRIC_PROBE_FROM_V19_41_LOCATES_"
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_twenty_second_bidirectional_merit_manifold_probe_v19_43.json"
...
__all__ = [
    ...
    "v19_43_selected_raw_vector", "twenty_second_bidirectional_merit_manifold_probe",
