"""Scan both orientations of a bounded geometric probe from v18.68."""
...
from bhsm.interface.aether_n3_eighth_direct_residual_scale_audit_v18_70 import eighth_direct_residual_scale_audit
...
VERSION = "v18.71"
CLASSIFICATION = "BHSM_N3_FIFTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
...
def v18_71_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_fifth_bidirectional_merit_manifold_probe_v18_71.json"
    ).read_text(encoding="utf-8"))
    selected = payload["fifth_bidirectional_merit_manifold_probe"]["selected_true_merit_candidate_pending_child_acceptance"]
    if selected is None:
        raise ValueError("v18.71 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def fifth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_eighth_direct_residual_scale_audit_v18_70.json"
    ).read_text(encoding="utf-8"))
    measured = audit["eighth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v18.70 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v18_68_selected_raw_vector()
    ...
    operator = LinearOperator(
        (376, 376), matvec=lambda direction_x: direct_response(transform @ direction_x), dtype=float
    )
    ...
    response_consistency = float(
        np.linalg.norm(direct_response(unit, response_step) - direct_response(unit, comparison_step))
    ...
    return {
        "source_state": "v18.68_fourth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = fifth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v18_68": result["source_state"].startswith("v18.68"),
        "source_norm_reproduced": abs(result["source_complete_norm"] - 0.811248056430707) < 5.0e-12,
    ...
    return {
        "artifact": "BHSM_aether_n3_fifth_bidirectional_merit_manifold_probe_v18_71",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "fifth_bidirectional_merit_manifold_probe": result,
        ...
        "real_physical_property_explained": "A_FRESH_BOUNDED_LOCAL_GEOMETRIC_PROBE_FROM_V18_68_LOCATES_EXACT_NONLINEAR_MERIT_STATES_WITHOUT_A_NEWTON_CLAIM",
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_fifth_bidirectional_merit_manifold_probe_v18_71.json"
    path.write_text(deterministic_json(completion_payload()), encoding="utf-8")
    return path


__all__ = ["VERSION", "CLASSIFICATION", "FULL_BHSM_COMPLETE", "v18_71_selected_raw_vector", "fifth_bidirectional_merit_manifold_probe", "completion_payload", "materialize"]
