"""Scan both orientations of a bounded geometric probe from v19.25."""
...
VERSION = "v19.27"
CLASSIFICATION = "BHSM_N3_EIGHTEENTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.794780177090688
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v19_27_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_eighteenth_bidirectional_merit_manifold_probe_v19_27.json"
    ).read_text(encoding="utf-8"))
    selected = payload["eighteenth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v19.27 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def eighteenth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_twenty_first_direct_residual_scale_audit_v19_26.json"
    ).read_text(encoding="utf-8"))
    measured = audit["twenty_first_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v19.26 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v19_25_selected_raw_vector()
    ...
    return {
        "source_state": "v19.25_seventeenth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = eighteenth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v19_25": result["source_state"].startswith("v19.25"),
    ...
    return {
        "artifact": "BHSM_aether_n3_eighteenth_bidirectional_merit_manifold_probe_v19_27",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "eighteenth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_eighteenth_bidirectional_merit_manifold_probe_v19_27.json"
...
__all__ = [
    ...
    "v19_27_selected_raw_vector", "eighteenth_bidirectional_merit_manifold_probe",
