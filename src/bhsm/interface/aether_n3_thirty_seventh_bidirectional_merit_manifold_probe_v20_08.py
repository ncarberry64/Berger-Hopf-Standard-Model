"""Scan both orientations of a bounded geometric probe from v20.06."""
...
VERSION = "v20.08"
CLASSIFICATION = "BHSM_N3_THIRTY_SEVENTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.768902386537366
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v20_08_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_thirty_seventh_bidirectional_merit_manifold_probe_v20_08.json"
    ).read_text(encoding="utf-8"))
    selected = payload["thirty_seventh_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v20.08 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def thirty_seventh_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_fortieth_direct_residual_scale_audit_v20_07.json"
    ).read_text(encoding="utf-8"))
    measured = audit["fortieth_direct_residual_scale_audit"]
    ...
    if pair is None:
        ...
        if not bounded_pairs:
            raise ValueError("v20.07 has no bounded direct-response pair")
        pair = min(
            bounded_pairs,
    ...
    raw = v20_06_selected_raw_vector()
    ...
    return {
        "source_state": "v20.06_thirty_sixth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = thirty_seventh_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v20_06": result["source_state"].startswith("v20.06"),
    ...
    return {
        "artifact": "BHSM_aether_n3_thirty_seventh_bidirectional_merit_manifold_probe_v20_08",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "thirty_seventh_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_thirty_seventh_bidirectional_merit_manifold_probe_v20_08.json"
...
__all__ = [
    ...
    "v20_08_selected_raw_vector", "thirty_seventh_bidirectional_merit_manifold_probe",
