"""Promote only the exact physical line state proposed by invalidated v18.35."""
...
VERSION = "v18.37"
CLASSIFICATION = "BHSM_N3_DIRECT_LINE_EXACT_MERIT_PROMOTION"
FULL_BHSM_COMPLETE = False


def v18_37_selected_raw_vector() -> np.ndarray:
    return v18_35_selected_raw_vector().copy()


def direct_line_exact_merit_promotion() -> dict[str, Any]:
    reconstruction = json.loads(Path(
        "artifacts/BHSM_aether_n3_direct_jfnk_proposal_child_v18_36.json"
    ).read_text(encoding="utf-8"))
    child_state = reconstruction["direct_jfnk_proposal_child"]["child_state"]
    qc = np.asarray(child_state["coordinates"])
    vc = np.asarray(child_state["velocities"])
    mc = np.asarray(child_state["multipliers"])

    scales = kkt_variable_scales()
    source_raw = v18_33_selected_raw_vector()
    raw = v18_37_selected_raw_vector()
    ...
    return {
        ...
        "event_to_complete_child": {
            ...
            "local_chart_rank": reconstruction["direct_jfnk_proposal_child"][
                "chart"
            ]["full_chart_rank"],
...
def completion_payload() -> dict[str, Any]:
    result = direct_line_exact_merit_promotion()
    ...
    return {
        "artifact": "BHSM_aether_n3_direct_line_exact_merit_promotion_v18_37",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "direct_line_exact_merit_promotion": result,
        "status": "VALIDATED" if passed else "INVALIDATED",
        "real_physical_property_explained": (
            "THE_V18_35_LINE_STATE_IS_PROMOTED_ONLY_BY_RECOMPUTED_EXACT_MERIT_"
            "ETA_AND_COMPLETE_MOVING_CHILD_PERSISTENCE"
        ),
        "dependency_advanced": "NONLINEAR_N3_EVENT_SADDLE_CLOSURE",
        "active_calculation": (
            "REMEASURE_THE_DIRECT_RESIDUAL_RESPONSE_AND_CONTINUE_FROM_V18_37"
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_direct_line_exact_merit_promotion_v18_37.json"
...
__all__ = [
    ...
    "direct_line_exact_merit_promotion",
    "completion_payload",
    "materialize",
]
