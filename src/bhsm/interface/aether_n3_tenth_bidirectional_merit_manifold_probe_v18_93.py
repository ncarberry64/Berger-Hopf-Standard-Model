"""Scan both orientations of a bounded geometric probe from v18.91."""
...
from bhsm.interface.aether_n3_ninth_bidirectional_probe_promotion_v18_91 import v18_91_selected_raw_vector


VERSION = "v18.93"
CLASSIFICATION = "BHSM_N3_TENTH_BIDIRECTIONAL_MERIT_MANIFOLD_PROBE"
FULL_BHSM_COMPLETE = False
SOURCE_NORM = 0.801699023846746
BACKTRACKS = 41
GMRES_RESTART = 40
...
def v18_93_selected_raw_vector() -> np.ndarray:
    payload = json.loads(Path(
        "artifacts/BHSM_aether_n3_tenth_bidirectional_merit_manifold_probe_v18_93.json"
    ).read_text(encoding="utf-8"))
    selected = payload["tenth_bidirectional_merit_manifold_probe"][
        "selected_true_merit_candidate_pending_child_acceptance"
    ]
    if selected is None:
        raise ValueError("v18.93 has no exact-merit candidate")
    return np.asarray([float.fromhex(value) for value in selected["raw_vector_hex"]])


def tenth_bidirectional_merit_manifold_probe() -> dict[str, Any]:
    audit = json.loads(Path(
        "artifacts/BHSM_aether_n3_thirteenth_direct_residual_scale_audit_v18_92.json"
    ).read_text(encoding="utf-8"))
    measured = audit["thirteenth_direct_residual_scale_audit"]
    pair = measured["selected_finest_common_stable_pair"]
    if pair is None:
        raise ValueError("v18.92 has no common stable direct-response scale")
    response_step = float(pair["fine_step"])
    comparison_step = float(pair["coarse_step"])
    raw = v18_91_selected_raw_vector()
    ...
    return {
        "source_state": "v18.91_ninth_bidirectional_probe_promoted_state",
...
def completion_payload() -> dict[str, Any]:
    result = tenth_bidirectional_merit_manifold_probe()
    ...
    validation = {
        "source_is_v18_91": result["source_state"].startswith("v18.91"),
    ...
    return {
        "artifact": "BHSM_aether_n3_tenth_bidirectional_merit_manifold_probe_v18_93",
        "version": VERSION,
        "classification": CLASSIFICATION,
        "FULL_BHSM_COMPLETE": FULL_BHSM_COMPLETE,
        "tenth_bidirectional_merit_manifold_probe": result,
...
def materialize(directory: str | Path) -> Path:
    ...
    path = target / "BHSM_aether_n3_tenth_bidirectional_merit_manifold_probe_v18_93.json"
...
__all__ = [
    ...
    "v18_93_selected_raw_vector", "tenth_bidirectional_merit_manifold_probe",
