...
## Current v15.8 critical path

Do not rebuild the already-owned conditional Hessian, gauge quotient,
self-adjoint theorem classes, nonlinear normal forms, persistence contract, or
cycle-mass readout. First derive a localized, nonhomogeneous, constraint-solved
physical parent configuration whose negative mode lives on the attached
domain. Then test same-action continuation into any legitimate nonlinear
endpoint. Cavitation, pair capture, bifurcation locking, and localization are
candidate mechanisms; none is currently universal or action selected.
