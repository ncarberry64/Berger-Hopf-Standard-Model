...
PHASE_THREE_INPUTS = {
    ...
    "vector_fermion_normalization_status": "artifacts/BHSM_vector_fermion_normalization_status_v0_6.json",
    "chiral_current_attachment_map": "artifacts/BHSM_chiral_current_attachment_map_v0_6.json",
    "mass_width_renormalization_open_gates": "artifacts/BHSM_mass_width_renormalization_open_gates_v0_6.json",
...
def load_required(relative: str) -> dict[str, Any]:
    path = ROOT / relative
    if not path.exists():
        raise FileNotFoundError(f"Missing required Phase Three-C/D artifact: {relative}")
    return json.loads(path.read_text(encoding="utf-8"))


def load_phase_three_inputs() -> dict[str, dict[str, Any]]:
    return {name: load_required(relative) for name, relative in PHASE_THREE_INPUTS.items()}
...
def parameter_entries() -> dict[str, dict[str, Any]]:
    payload = load_required("artifacts/BHSM_candidate_parameter_card_v0_5.json")
    return {entry["parameter"]: entry for entry in payload.get("entries", [])}


def source_artifact_list() -> list[str]:
    return list(PHASE_THREE_INPUTS.values())
