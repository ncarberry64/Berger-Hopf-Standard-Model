...
def load_required(relative: str) -> dict[str, Any]:
    path = ROOT / relative
    if not path.exists():
        raise FileNotFoundError(f"Missing required Phase Three-C/D/E/F/G artifact: {relative}")
    return json.loads(path.read_text(encoding="utf-8"))


def load_phase_three_h_inputs() -> dict[str, dict[str, Any]]:
    return {name: load_required(relative) for name, relative in PHASE_THREE_H_INPUTS.items()}
...
def source_artifact_list() -> list[str]:
    return list(PHASE_THREE_H_INPUTS.values())
...
def vertex_entries(inputs: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    return {entry["vertex_id"]: entry for entry in inputs["production_vertex_table_candidate"]["entries"]}
