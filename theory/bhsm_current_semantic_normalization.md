...
On the event-child constraint surface `C(y)=0`, the exact physical replacement transfer condition is `N_phys^dagger q_rep=0`, where `range(N_phys)=ker D_C/G_exact` and `q_rep=D Gamma_heat-D Gamma_SM^zeta`; a constraint-normal force only shifts the KKT multiplier. The certified fixed-event child Jacobian has a 67-dimensional raw kernel, while the already-retained whole-system time quotient has dimension 66. The local child Euler--Dirac vector is not the coupled time generator: its fixed-event reset residual is stably about `1.136e-2` relatively. The boundary radius Cauchy-jet map `(delta log R4,delta D_tau log R4)` has rank two on the raw reset tangent, so any one-dimensional time quotient leaves at least one fixed-channel coefficient-history direction. The common-scale center is retained as a physical force/modulation direction because lower action weights and the boundary Casimir break full scale invariance. Therefore the raw nullspace and bordered KKT cross-checks are retained as algebraic witnesses but are not relabelled as the final physical quotient. The coupled hybrid generator or an intrinsic quotient formulation remains part of the open realization.

A reproducible data inventory shows that the original endpoint checkpoint contains the state and first constraint Jacobian but no complete coefficient path, evaluated endpoint load, geometry operator jet, projected force, or geometry/reset Hessian. Proper-time differentiation and its form Laplacian are action-owned kinematics rather than missing independent coefficient data. The positive-duration child persistence witness ends at a validation time and cannot be promoted to an action-owned force endpoint. The two tracked certified chords likewise end at `2e-8` without an event or canonical stop and cannot be promoted because the zeta extension force is strictly additive. On each fixed regular finite endpoint or canonical-stop stratum, common-domain C^2 form dependence and a uniform negative-probe coercivity margin imply C^2 dependence of the resolvent, Poisson operator, and Weyl--Calderón map. An inverse-free, block-covariant solver evaluates the Weyl value and its geometry jets from any supplied stratum pencil. One stored reset representative still cannot determine the physical force or Hessian unless the action proves reset-fiber invariance. The rank-two radius Cauchy jet and non-gauge common scale exclude reset kinematics, time translation, and scale symmetry as sources of such invariance; only a separate quantum trace-cancellation theorem could supply that shortcut. The heat-minus-zeta force is a first variation. At each reset parameter its complete quotient covector can be evaluated without 66 forward Jacobi columns: pull the operator cotangent backward through the fixed-channel assembly and the transposed implicit Euler--Dirac linearization, apply the exact moving-endpoint projection, and finally pull through the reset jet and physical quotient. Because the reset is set-valued, one such evaluation is not a selector. The exact no-selector owner is existence and certification of a nonempty regular finite-endpoint solution of the coupled forward-state, operator, backward-adjoint, endpoint, and reset-quotient KKT equations, or the equivalent parametric-oracle covector root. The full analytic reset Jacobian removes the old recentering bottleneck and intrinsic continuation found a high-precision child terminal-event candidate. Independent directed center accumulation closes `Y<=6.0881e-13` and `Z0<=2.2919e-6`; the retained nonlinear bound `Z2<=2.29881e10` gives a negative radii polynomial and contraction `0.229884` on a nested `1e-11` root ball. The exact solution lies within `6.1313e-13`. Selected-line simplicity, Legendre positivity, all four canonical lifts, and reset regularity transfer. A coarse global `D4 L` norm is inconclusive for the small cubic, but the cancellation-preserving selected-line graph expansion and a directed Decimal symmetric difference with retained `D5` remainder prove `c_psi<=-9.5310e-12` and `b_psi>=9.98085e-5` at the exact root. Thus an incoming local finite-positive-time event-reaching germ exists. This is neither recurrence nor universal reachability, and the numerical continuation is not a physical child selector or observable readout. The compatibility audit further distinguishes this germ from a complete forward-component history: the 58-row equation sets the child event function to zero at the reset incidence itself, so the root supplies `T=0`, not a `T>0` orbit from the physical positive-duration reset image to a later event. The current owner is certification of that positive-duration connecting BVP; only then can the compact finite-endpoint operator, its first physical reset-quotient jet, and the existing heat-minus-zeta force covector be evaluated. Same-action homotopy continuation of the replacement-force saddle still awaits that force; its positive tangent Hessian is synthetic, the historical zero Hessian belongs to a constant reconstruction map, and the actual complete-history physical KKT Hessian and replacement force are absent. The remaining gap is the connecting history and downstream operator evaluation, not a missing boundary condition, force formula, solver, or proved action inconsistency. The adjoint residual uses `D3 L`; certifying a nonzero-force Newton/KKT correction retains `D4 L`, second operator jets, reset curvature, and the geometry KKT Hessian. Arbitrary infinite nonencapsulating formation tails and terminal recurrence are not reopened. The later pair-plus-contact source Hessian remains distinct. The dependency chain then proceeds through the certified same-action saddle, that response Hessian, Ward/BRST and relative-trace control, and a basis-independent scalar observable map.
