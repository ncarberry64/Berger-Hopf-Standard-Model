...
The cotangent factorization sharpens that scope. If `Z` spans the 139-dimensional reset tangent, `B=P_C2 Z` is the outgoing seed map, and `K` spans `ker B`, then every downstream C2 adjoint covector satisfies `K^dagger B^dagger p_0=0`. The complete restricted force splits as `Z^dagger d_seam+B^dagger p_0`. Hence stationarity on the 67 fixed-seed lift directions is exactly the direct two-sided seam condition `(ZK)^dagger d_seam=0`, whereas one downstream adjoint supplies the 73-coordinate outgoing launch force `(Q^dagger p_0,<F_0,p_0>)`. This eliminates 73 forward-Jacobi solves for a scalar force, but it neither discards the 67 seam directions nor evaluates either action covector.
