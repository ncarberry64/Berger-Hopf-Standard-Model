...
The moving-duration incidence is now exact up to one sharply localized action derivative. The BHSM boundary attachment supplies signed `D log R4` and `D log N_boundary`, and `q_tau=N_boundary s/Delta` obeys `D q_tau=q_tau(D log N_boundary-D Delta/Delta)`. The committed continuation data supply a finite norm bound for `D Delta`, but no signed covector on the exact family. Thus the minimal local duration datum is signed `D_Y Delta` from the selected-line/hard-complement first variation, followed by the transposed exact segment-map action. A norm bound or proof-center derivative cannot replace it.
