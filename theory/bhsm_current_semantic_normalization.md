...
The chronology is `pre-event formation -> singular encapsulation event -> complete-child reset -> decay/evolution`. The reset image is post-encapsulation data and is not required to hit the event again. The desingularized event normal form, with the selected eigenvalue as independent variable, proves existence of at least one local pre-event constrained branch reaching the event in finite positive time. This is an action-derived local existence theorem, not universal reachability and not a child selector. The current Gate-7 owner is therefore G7_08. Its exact basis-independent heat-minus-zeta force functional is derived. The event-normal Riccati identity `D_s M=L-zI-M^2` and geometry jet `D_s delta M=delta L-M delta M-delta M M` are valid arm-transfer identities, but the physical AE2 seam is two-sided: `S_AE2=M_event+U_R^dagger M_child U_R+W_phys`. The earlier initialization `M(0,z)=W_phys` is superseded because it omits the child-arm Calderon response. The reset lift is covariantly parallel as the transition map of the one AE2 global bundle, so its frame derivative is absorbed into the covariant child-response jet rather than counted as a new physical source. The retained comparison theorems now give broad two-sided child-load and compact first/mixed jet enclosures for every neutral negative probe `z=-kappa^2`, `kappa>0`; an optimized inner-core trial improves the product-Dirac high-probe bound to `O(kappa)`. Low-energy source-Dini and independent high-energy trace control remain closed. On the event-child constraint surface `C(y)=0`, the exact replacement transfer condition is now normalized as `N^dagger q_rep=0`, where `range(N)=ker D_C` and `q_rep=D Gamma_heat-D Gamma_SM^zeta`; an ambient normal force only shifts the KKT multiplier. Nullspace and bordered KKT formulations agree without inverting the ambient Hessian. The certified fixed-event child relation has a 67-dimensional tangent fiber, so G7_08 and the G7_09 joint saddle are coupled and no reset-fiber representative may be selected by hand. A reproducible data inventory shows that the endpoint checkpoint contains the state and first constraint Jacobian but no complete coefficient path, temporal form, endpoint load, geometry operator jet, projected force, or geometry/reset Hessian. The positive-duration child persistence witness ends at a validation time and cannot be promoted to an action-owned force endpoint. Thus the single current owner is an action-derived complete event-reaching physical-history operator realization on the finite-encapsulation domain or an equivalent actual exterior Weyl--Calderón oracle with its geometry jet. Arbitrary infinite nonencapsulating formation tails are not reopened. The later pair-plus-contact source Hessian is a distinct downstream object. The dependency chain then proceeds through that response Hessian, Ward/BRST and relative-trace control, and a basis-independent scalar observable map.
