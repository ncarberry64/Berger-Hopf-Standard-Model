...
The launch chart and exact fixed-`s` field also remove the earlier base-history wording. On the compact 1222-segment cover the selected line remains simple, the hard complement has a uniform gap, `c`, `Delta`, radii, and durations remain positive, and the complete fixed-`s` growth is at most `1.0121455013371734`. Finite-time smooth ODE dependence therefore gives a nonempty local 73-parameter family of exact C2 histories and first Jacobi fields through every finite-core prefix. The proof centers remain enclosure data and the 1222 edge remains a nonphysical form-core truncation. What remains open is the numerical parametric or interval realization of the signed joint adjoint, the actual graded heat-minus-zeta contraction, and the maximal projected force tail.

The signed finite-core adjoint equation is now assembled on that family. If `C_x` and `C_h` are the inverse-free node-radius and moving-duration coefficient cotangents, then `p_N=C_x,N x_Y,N+g_T` and `p_j=C_x,j x_Y,j+C_h,j h_Y,j+Phi_Y,j^dagger p_(j+1)`. Its seed value composes as `g_reset=Z^dagger d_upstream_interface+B^dagger p_0`. This uses one reverse sweep, zero forward Jacobi columns, and no full Euler--Dirac inverse. It still requires a numerical parametric or interval realization on exact family members, the complete upstream signed covector, and the graded heat-minus-zeta source; proof centers do not supply those values.

The moving-duration incidence is now exact up to transport of one sharply localized action derivative. The BHSM boundary attachment supplies signed `D log R4` and `D log N_boundary`, and `q_tau=N_boundary s/Delta` obeys `D q_tau=q_tau(D log N_boundary-D Delta/Delta)`. At the retained reference center the selected-line/hard-complement calculation supplies a signed `D_Y Delta` partial of norm about `1.603e-9` plus a controlled remainder at most `4.467e-12`, a relative ball below `2.79e-3`. This is a certified local seed, not a physical-history value. The minimal local duration task is to transport that ball onto the exact parametric family and apply the transposed exact segment map. A global norm bound or untransported proof-center derivative cannot replace it.
