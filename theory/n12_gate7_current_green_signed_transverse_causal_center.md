...
For interval `i`, let `B_i`, `B_{i+1}`, and `B_{i+1/2}` be the recovered
endpoint and midpoint transverse Hessians.  The midpoint input map is the
same Hermite--Simpson first variation used by the certified mixed operator.
...
itself close Gate 7.  A failed screen records a representation or action
obstruction and does not authorize a fitted coefficient, a changed norm, or
retuning of the frozen background.
