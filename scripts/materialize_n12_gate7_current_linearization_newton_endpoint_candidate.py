"""Take a second endpoint Newton step using the rebuilt current linearization."""
...
RESIDUAL = BASE / "BHSM_N12_GATE7_SIGNED_GREEN_HERMITE_COLLOCATION_REPLAY.json"
RESIDUAL_DATA = RESIDUAL.with_suffix(".npz")
JACOBIAN = BASE / "BHSM_N12_GATE7_SIGNED_GREEN_CURRENT_CENTER_GRAPH_JACOBIAN.json"
JACOBIAN_DATA = JACOBIAN.with_suffix(".npz")
TANGENT = BASE / "BHSM_N12_GATE7_SIGNED_GREEN_CURRENT_CENTER_MACRO_TANGENT.json"
TANGENT_DATA = TANGENT.with_suffix(".npz")
THEORY = ROOT / "theory" / "n12_gate7_current_linearization_newton_endpoint_candidate.md"
RESULT = BASE / "BHSM_N12_GATE7_CURRENT_LINEARIZATION_NEWTON_ENDPOINT_CANDIDATE.json"
...
def main() -> None:
    center = _load(CENTER)
    residual_record = _load(RESIDUAL)
    jacobian_record = _load(JACOBIAN)
    tangent_record = _load(TANGENT)
    if not all(record.get("validation_passed") is True for record in (
        center, jacobian_record, tangent_record,
    )):
        raise RuntimeError("validated current center, Jacobian, and tangents required")
    if residual_record["status"] != "SIGNED_GREEN_NEWTON_STEP_DOES_NOT_CLOSE_DENSE_FLOW_DEFECT":
        raise RuntimeError("the measured current-center residual is required")
    ...
    for interval in range(times.size - 1):
        ...
        source = np.zeros(98)
        for unit, weight, residual in zip(units, gauss_weights, residuals[mask, :-1], strict=True):
    ...
    workers = min(
        int(os.environ.get("BHSM_N12_CURRENT_NEWTON_WORKERS", "8")),
        os.cpu_count() or 1,
    )
    tasks = [
    ...
    if eigenvalues[-2] > 0.0 > eigenvalues[-1]:
        ...
        raise RuntimeError("second Newton step moved the stop before the retained final segment")
    ...
    validation = {
        "all_371_second_Newton_endpoints_replayed": output_states.shape == (371, 98),
        "current_center_graph_Jacobian_consumed": jacobians.shape == (371, 98, 98),
        "current_center_macro_tangents_consumed": tangents.shape == (48, 98, 73),
    ...
    payload = {
        "artifact": "BHSM_N12_GATE7_CURRENT_LINEARIZATION_NEWTON_ENDPOINT_CANDIDATE",
        "status": "SECOND_CURRENT_LINEARIZATION_NEWTON_ENDPOINTS_MATERIALIZED" if passed else "CURRENT_LINEARIZATION_NEWTON_ENDPOINT_CANDIDATE_INVALID",
        "authority": "NUMERICAL_CURRENT_LINEARIZATION_NEWTON_CANDIDATE_NOT_INTERVAL_AUTHORITY",
        "summary": {
        ...
        "inputs": {
            ...
            for path in (CENTER, CENTER_DATA, RESIDUAL, RESIDUAL_DATA, JACOBIAN, JACOBIAN_DATA, TANGENT, TANGENT_DATA, THEORY, THIS_SCRIPT)
        },
        "adjudication": {
            "current_center_linearization": "USED_FOR_SECOND_SIGNED_GREEN_NEWTON_STEP",
            "continuous_center": "OPEN_PENDING_SECOND_GAUSS3_REPLAY",
        ...
        "exact_next_dependency": "JOIN_THE_SECOND_NEWTON_ENDPOINTS_WITH_ENDPOINT_FIELD_MATCHED_CUBICS_AND_REPLAY_ALL_1110_GAUSS3_RESIDUALS",
