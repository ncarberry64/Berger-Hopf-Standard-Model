"""All 75 scaled tube directions against an actual frozen trial column.

Retains the complete original mixed graph and uses independently paired
first-variation enclosures on the identical physical family.
"""
import os
...
def evaluate(source,column):
    ...
    if not 0<=column<74:raise ValueError('frozen trial column in 0..73 required')
    axis=np.array([arb(float(v)) for v in frame[:,column]],dtype=object)
    ...
    print(json.dumps(dict(phase='VERIFIED_POINT_AFFINE_BASIS',columns=75,frozen_trial_column=column)),flush=True)
    ...
    report=dict(point_eigenpair_checks=checks,point_derivative_checks=df_checks,solve_proofs=proofs,normalization_proof=norm_proof,
        all_75_scaled_affine_tube_directions_enclosed=True,frozen_trial_column=column,full_physical_input_basis=False,
