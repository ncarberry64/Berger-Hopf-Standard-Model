"""Propose all input covectors at once, retaining a single linear input leg.
...
def evaluate(root, family, adjoint_path, out):
    ...
    if baseline.get('family') != family or any(baseline['source_hashes'].get(k) != v
            for k, v in verified['paired_source_hashes'].items()):
        raise ValueError('matching input-independent last adjoint blocks required')
    ...
    paths = {}
    full = {}
    for stage, name in (('endpoint', 'endpoint_014'), ('midpoint', 'interval_013')):
    ...
    result = dict(algorithm='FULL_INPUT_ANCHOR_BASE_ADJOINT_V1', family=family,
        ...
        column14_base_covector_difference=max(float(abs(beta[14, j]-arb(baseline['exact_covectors']['eigenline' if j<62 else 'response'][j%62])).upper()) for j in range(dim)),
    ...
    print(json.dumps({k:result[k] for k in ('maximum_base_adjoint_defect_approximate','column14_base_covector_difference')}), flush=True)
