...
def evaluate(root,midpoint_path,endpoint_path,adjoint_path,refined_path,out,midpoint_adjoint_path,midpoint_refined_path,midpoint_corrected,endpoint_corrected,export_models=False,pullback_state_errors=False,midpoint_base_path=None,endpoint_base_path=None):
    ...
    residual_blocks=[
        ...
        base_models.load_block(endpoint_base_path,endpoint_path,refined_path,common,endpoint_state,verified)]
    ...
    with ExitStack() as stack:
        ...
        for i in [73]:
            ...
            value,residual_weights=cancel_base(value)
            ...
            diagnostic=dict(component=i,constant=upper(abs(longitudinal.c)),linear=upper(longitudinal.linear_bound()),remainder=upper(longitudinal.r),groups=[dict(start=a,stop=b,kind=k,bound=upper(linear_support(longitudinal.a.entries()[a:b],[(0,b-a,k)]))) for a,b,k in common.groups],midpoint=upper(midpoint_model.at_input(arb_mat(198,1,longitudinal_direction.entries()[:198])).support()),endpoint=upper(endpoint_part.at_input(arb_mat(198,1,axis+longitudinal_direction.entries()[198:])).support()),fixed=upper(fixed_model.at_input(longitudinal_direction).support()))
            (out/'diagnostic.json').write_bytes(saved.encoded(diagnostic))
    ...
    long_value,long_residual_weights=cancel_base(long_value)
    ...
    result=dict(algorithm='FULL_INPUT_BASE_RESIDUAL_COUPLED_HS_TRANSPORT_V2',physical_input_columns=74,projected_output_rows=74,
        proof_assumptions=[
        ...
        all_248_base_residuals_used=True,
