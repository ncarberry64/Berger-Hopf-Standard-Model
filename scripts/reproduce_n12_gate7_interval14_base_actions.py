"""Independently regenerate all scalar terms for a fresh base proof.
...
def prefetch(root,operands,predictors,template,out,workers):
    ...
    # The template supplies input identities only. No Taylor coefficients,
    # residuals or error bounds are read from the first computation.
    sources=json.loads(template.read_bytes())['input_source_SHA256']
    if any(sha(Path(path))!=digest for path,digest in sources.items()):raise ValueError('Active producer source changed')
    out.mkdir(parents=True,exist_ok=False)
    (out/'sources.json').write_bytes(p.geometry.encoded(sources))
    ...
    legs=dict(psi=pad(psi),hard=pad(hard),d=[configuration[i]/weights[i] for i in range(37)]+[arb(0)]*61)
    tasks=[('rayleigh',['psi','psi'])]
    for i in range(62):
        ...
        tasks.extend([(f'eigen_{i:02d}',[f'v{i}','psi']),
            (f'response_{i:02d}_gradient',[f'g{i}']),
    ...
    with (out/'independent_action_receipt.json').open('xb') as stream:stream.write(p.geometry.encoded(receipt))
    print(json.dumps(dict(prefetched_terms=len(tasks),complete=True)),flush=True)


if __name__=='__main__':
    parser=argparse.ArgumentParser()
    for key in ('evidence-root','operands','predictors','template','out'):parser.add_argument('--'+key,type=Path,required=True)
    parser.add_argument('--workers',type=int,default=8)
    args=parser.parse_args()
    prefetch(args.evidence_root.resolve(),args.operands.resolve(),args.predictors.resolve(),args.template.resolve(),args.out.resolve(),args.workers)
