"""Apply the damped second block correction without imposing a stop adapter."""
...
PREDICTOR = BASE / "BHSM_N12_GATE7_SECOND_HS_NEWTON_LINE_SEARCH_PREDICTOR.json"
THEORY = ROOT / "theory" / "n12_gate7_damped_second_hs_newton_endpoint_candidate.md"
RESULT = BASE / "BHSM_N12_GATE7_DAMPED_SECOND_HS_NEWTON_ENDPOINT_CANDIDATE.json"
...
def main() -> None:
    ...
    if center.get("validation_passed") is not True or predictor.get("validation_passed") is not True:
        raise RuntimeError("validated parent center and damped predictor required")
    ...
    with np.load(PREDICTOR.with_suffix(".npz")) as source:
        ...
        alpha = float(source["line_search_alpha"])
    trial_action_states = states * weights[None, :] + correction
    workers = min(int(os.environ.get("BHSM_N12_DAMPED_HS_WORKERS", "8")), os.cpu_count() or 1)
    ...
    validation = {
        "all_371_damped_endpoints_replayed": output_states.shape == (371, 98),
    ...
    payload = {
        "artifact": "BHSM_N12_GATE7_DAMPED_SECOND_HS_NEWTON_ENDPOINT_CANDIDATE",
        "status": "DAMPED_SECOND_HERMITE_SIMPSON_ENDPOINTS_MATERIALIZED" if passed else "DAMPED_SECOND_ENDPOINTS_INVALID",
        "authority": "NUMERICAL_DAMPED_CONSTRAINT_PROJECTED_ENDPOINTS_NOT_INTERVAL_AUTHORITY",
        "summary": {
        ...
        "exact_next_dependency": "REPLAY_ALL_370_DAMPED_SECOND_ITERATION_COLLOCATION_MIDPOINTS",
