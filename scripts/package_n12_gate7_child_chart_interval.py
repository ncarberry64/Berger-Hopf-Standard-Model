...
def verify(payload):
    ...
    if (payload.get('interval') not in range(14, 19) or len(payload['links']) != 2
    ...
    if [(v['from_domain'], v['to_domain']) for v in payload['links']] != list(zip(names, names[1:])):
        raise ValueError('link endpoints do not match the stated interval')
...
def run(first, repeat, out):
    ...
    receipt = dict(algorithm='SHARED_EIGENBRANCH_LINK_REPRODUCTION_V1',
        ...
        scope='TWO_CENTERLINE_LINKS_AND_CONNECTED_EIGENBRANCH_EVALUATION_COVER_ONLY')
