"""Replay all exact midpoints after the intrinsic 73D tangent Newton step."""
...
ENDPOINT = BASE / "BHSM_N12_GATE7_FIRST_HS_TANGENT_NEWTON_ENDPOINT_CANDIDATE.json"
THEORY = ROOT / "theory" / "n12_gate7_first_hs_tangent_newton_midpoint_replay.md"
RESULT = BASE / "BHSM_N12_GATE7_FIRST_HS_TANGENT_NEWTON_MIDPOINT_REPLAY.json"
...
def main() -> None:
    ...
    if parent.get("validation_passed") is not True or endpoint.get("validation_passed") is not True:
        raise RuntimeError("validated first-HS source and tangent endpoints required")
    ...
    tasks = [
        (interval, midpoint_augmented[interval, :-1] / weights, max(float(midpoint_augmented[interval, -1]), 0.0))
    ...
    validation = {
        ...
        "intrinsic_tangent_step_reduces_nonlinear_Hermite_Simpson_residual": reduction > 1.0,
    ...
    payload = {
        "artifact": "BHSM_N12_GATE7_FIRST_HS_TANGENT_NEWTON_MIDPOINT_REPLAY",
        "status": "INTRINSIC_TANGENT_STEP_REDUCES_NONLINEAR_HERMITE_SIMPSON_RESIDUAL" if passed else "INTRINSIC_TANGENT_STEP_DOES_NOT_REDUCE_NONLINEAR_RESIDUAL",
        "authority": "NUMERICAL_EXACT_MIDPOINT_REPLAY_NOT_INTERVAL_AUTHORITY",
        "mesh": {"shooting_intervals": 370, "augmented_dimension": 99, "endpoint_tangent_dimension": 73, "workers": workers},
        "summary": {
        ...
        "adjudication": {
            "ambient_98D_post_projected_step": "REJECTED_BY_PRIOR_EXACT_REPLAY",
            "intrinsic_73D_constraint_tangent_step": "ACCEPTED_FOR_NEXT_ITERATION" if passed else "REJECTED_BY_EXACT_NONLINEAR_REPLAY",
        },
        "claim_boundary": {
        ...
        "exact_next_dependency": (
            "IF_REDUCED,_REBUILD_THE_COMPLETE_PROJECTED_RECENTERED_JVP_ON_THIS_CENTER;_"
            "OTHERWISE_ROUTE_TO_AN_AUGMENTED_KKT_BLOCK_LINEARIZATION"
        ),
