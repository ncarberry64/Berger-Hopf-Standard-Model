"""Cancel directional implicit residuals in every unnormalized velocity component.
...
def evaluate(root, family, adjoint_path, refined_path, out, components, reference=None, fresh_terms=()):
    reuse=None
    ...
    if reference is not None:
        ...
        fast_producer=ROOT/'scripts/certify_n12_gate7_directional_numerator_fast.py'
        allowed_sources={saved.sha(full_producer),saved.sha(directional_producer),saved.sha(fast_producer),saved.sha(Path(__file__))}
        ...
        import bhsm.interface.local_input_taylor_action_fast as fast_implementation
        if reference_sources.get('implementation') not in {saved.sha(Path(previous_implementation.__file__)),saved.sha(Path(fast_implementation.__file__)),saved.sha(Path(implementation.__file__))}:
    ...
    def action(name, legs):
        ...
        # Completed terms are immutable. Full gzip integrity, source binding,
        # matrix shape, and exact dyadic restoration are checked below.
        # A requested fresh term bypasses only the reference checkpoint.
        if name in fresh_terms: reference_term=None
        origin=None
    ...
    result=dict(algorithm='FULL_INPUT_DIRECTIONAL_NUMERATOR_SHARED_RESIDUAL_V1',family=family,components=list(components),
...
def main():
    ...
    parser.add_argument('--fresh-term',action='append',default=[],help='recompute a named action term instead of adopting its reference checkpoint')
    ...
    import bhsm.interface.local_input_taylor_action_top as implementation
    input_linear_taylor_action=implementation.input_linear_taylor_action
    evaluate(args.evidence_root.resolve(),args.family,args.adjoint.resolve(),args.refined.resolve(),args.out.resolve(),range(start,stop),args.reference.resolve() if args.reference is not None else None,args.fresh_term)
