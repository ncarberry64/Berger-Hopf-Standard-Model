"""Original-domain Taylor enclosure of DF times one shared direction.
...
def evaluate(root,operands,predictors,base_path,out,endpoint_path=None,workers=1):
    ...
    with cache.cache_hashes(targets,excluded_roots=[out]):
        ...
        def decode(data,target=domain):
            ...
            return Taylor(target,values[0],arb_mat(1,target.dimension,values[1:-1]),values[-1])
        ...
        def predictor(c,derivative,errors=None):
            return [domain.affine(v.mid(),[derivative[i,j].mid() for j in range(n)],
        ...
        if pre['stage']=='midpoint':
            ...
            def embed(value):
                coefficients=[arb(0)]*249
                coefficients[1]=value.a[0,0]
                coefficients[76:150]=value.a.entries()[1:]
                return domain.affine(value.c,coefficients,value.r)
            end_models=[embed(decode(value,endpoint_domain)) for value in endpoint['physical_derivative_models']]
        ...
        parameter_s=domain.affine(s['point_descriptor'][0],[directions[98,j] for j in range(n)])
        su=axis[98]
