"""The retained batched Hessian graph with interval state/direction inputs.

Only three input conversions differ from the parent. Physical formulas and
all contraction grouping are unchanged. This module does not certify inputs,
eigenpair inclusion, branch continuation, or a physical neighborhood.
"""
import hashlib
import inspect
import numpy as np
...
cert = parent.cert
_solve, _pair_columns, OUTPUTS = parent._solve, parent._pair_columns, parent.OUTPUTS
_batch_scalar, _batch_fixed = parent._batch_scalar, parent._batch_fixed
_batch_first, _affine_contraction = parent._batch_first, parent._affine_contraction
PARENT_FUNCTION_SHA256 = '465C94EFF0CA2635F0DAE155BCBAD2CF89518170958C7BBCA685BB147883004E'
if hashlib.sha256(inspect.getsource(parent.batched_axis_map).encode()).hexdigest().upper() != PARENT_FUNCTION_SHA256:
    raise RuntimeError('parent physical Hessian graph changed; reconciliation required')
...
def batched_axis_map(state: np.ndarray, descriptor: float, weights: np.ndarray, reference: np.ndarray, axis_direction: np.ndarray, transverse_directions: np.ndarray) -> np.ndarray:
    ...
    u = np.asarray([preserve_ball(value) for value in axis_direction], dtype=object)
    V = np.asarray([[preserve_ball(value) for value in row] for row in transverse_directions], dtype=object)
    ...
    configuration = np.asarray([arb(float(q_weights[i])) * preserve_ball(state[cert.QDIM + i]) for i in range(cert.QDIM)], dtype=object)
