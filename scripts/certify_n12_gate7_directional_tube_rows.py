...
def rows_for_cell(state,left,eta,directions,progress):
    ...
    with sparse.use_optimized_mixed(action),factored.use_ball_factored_integrand(action,state):
        ...
        for start in range(0,62,4):
            ...
            for row in values:
                support=sum((abs(v).upper() for v in row[:4]),arb(0))
                support+=sum((sum((abs(v).upper()**2 for v in row[a:b]),arb(0)).sqrt() for a,b in [(4,78),(78,152)]),arb(0))
                support+=sum((abs(v).upper() for v in row[152:]),arb(0))
...
def run(evidence,interval,cells,selected,out):
    ...
    for cell in selected:
        ...
        # Retain the signed longitudinal and transverse input directions.
        # Only component rate-error symbols and explicit Taylor tails are boxed.
        delta=np.empty((98,250),dtype=object)
        for i,v in enumerate(ops['displacement']):
            co=v.a.entries()
            error=(sum((abs(x).upper() for x in co[152:348]),arb(0))+v.r).upper()
            delta[i]=[v.c,co[0],co[2],co[77]]+co[3:77]+co[78:152]+[error if i==j else arb(0) for j in range(98)]
    ...
    payload=dict(algorithm='SIGNED_DIRECTIONAL_LOCAL_HERMITE_TUBE_ROWS_V1',interval=interval,partition_cells=cells,
