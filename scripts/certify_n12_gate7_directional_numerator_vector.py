...
def evaluate(root, family, adjoint_path, refined_path, out, components, reference=None):
    ...
    if reference is not None:
        ...
        if reference_sources.get('evaluator')!=saved.sha(full_producer):
            raise ValueError('unchanged reviewed full numerator producer required')
        if any(reference_sources.get(k)!=v for k,v in sources.items() if k!='evaluator'):
        ...
        sources['reference_full_producer']=saved.sha(full_producer)
    ...
    def action(name, legs):
        ...
        # A completed component record is published after all of its terms.
        if active_component is not None and reference is not None and not (reference/f'component_{active_component:02d}.json').exists():
            reference_term=None
        origin=None
