"""Complete anchor adjoint for the interval-14 endpoint descriptor DF entry."""
import os
...
def evaluate(root,operands,predictors,direction,out):
    ...
    with hashes.cache_hashes([(p.values,'sha'),(r.center,'_sha'),(r.foundation.coordinate.center,'_sha')]):
        ...
        if pre['stage']!='endpoint' or pre['interval']!=14:raise ValueError('Endpoint 15 only')
        p.verify_sources(ore['binding'])
        old=json.loads(direction.read_bytes())
        if old['stage']!='endpoint' or any(sha(Path(path))!=digest for path,digest in old['input_source_SHA256'].items()):
            raise ValueError('Complete original endpoint directional enclosure required')
        eigen=root/'artifacts/flagship_integration/.affine_eigenpair_pilot_work/endpoint_015/eigenpair.npz'
        ...
        J,gradient,value=adjoint.linearize(H,Hu,configuration,configuration_u,rw,s['point_descriptor'][0],axis[98],
            [arb(i==98) for i in range(99)],centers,descriptor)
        ...
        paths=[operands/'record.json',operands/'operands.npz',predictors/'record.json',predictors/'predictors.npz',
               direction,eigen,Path(__file__),Path(cert.__file__),Path(adjoint.__file__),Path(prescribed.__file__),Path(factored.__file__)]
        result=dict(algorithm='INTERVAL14_ENDPOINT_DESCRIPTOR_COMPLETE_ANCHOR_ADJOINT_V1',interval=14,stage='endpoint',
            output_coordinate=98,unknowns=248,
