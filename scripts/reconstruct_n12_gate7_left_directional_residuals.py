...
"""Reconstruct contracted directional implicit residuals in every unnormalized velocity component.
...
def evaluate(root, family, adjoint_path, refined_path, out, components, reference=None):
    ...
    if reference is not None:
        ...
        allowed_sources={saved.sha(full_producer),saved.sha(directional_producer),saved.sha(ROOT/'scripts/certify_n12_gate7_left_directional_numerators.py'),saved.sha(Path(__file__))}
    ...
    def action(name, legs):
        ...
        if path.exists() or (reference_term is not None and reference_term.exists()):
            ...
            raise ValueError('All action derivatives must come from the verified completed cache')
        ...
        # Read-only action caches stay in their original source-bound directory.
        terms[name]['source_path']=str(selected.resolve())
        terms[name]['source_SHA256']=saved.sha(selected)
    ...
    for component in components:
        ...
        if reference is None:
            raise ValueError('a completed source numerator is required for exact reconstruction replay')
        original_model=reference/f'component_{component:02d}.json.gz'
        original=json.loads(gzip.decompress(original_model.read_bytes()))
        original_record=json.loads((reference/f'component_{component:02d}.json').read_bytes())
        covectors=[[str(v.fmpq()) for v in block] for block in (v2,v3)]
        if (saved.sha(original_model)!=original_record['models_SHA256']
                or original['component']!=component or original['source_binding']!=reference_binding
                or original['coefficients']!=encode(W) or original['axis_covectors']!=covectors):
            raise ArithmeticError('cached action equations must reproduce every original velocity coefficient exactly')
        payload=dict(component=component,G2_coefficients=encode(G2),G3_coefficients=encode(G3),
            original_velocity_coefficients_reproduced_exactly=True,
            original_velocity_model_SHA256=saved.sha(original_model),axis_covectors=covectors,
    ...
    normals=[dot(psi,psi_u[:61]),dot(psi,hard_u[:61])+dot(psi_u[:61],hard[:61]),psi_u[61]]
    normalization=out/'normalizations.json.gz'
    normalization.write_bytes(gzip.compress(saved.encoded([encode(v) for v in normals]),mtime=0))
    result=dict(side='left',normalizations_SHA256=saved.sha(normalization),normalization_equations=['psi_dot_psi_u','psi_dot_h_u_plus_psi_u_dot_h','derived_psi_u_multiplier_zero'],algorithm='FULL_INPUT_CONTRACTED_DIRECTIONAL_RESIDUALS_V1',family=family,components=list(components),
