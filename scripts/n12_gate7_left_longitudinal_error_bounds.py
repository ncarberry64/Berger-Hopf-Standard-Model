...
def load_direction(path, parent_path, adjoint_path, refined_path, axis, error_map, verified):
    ...
    if (record['algorithm']!='LONGITUDINAL_DIRECTIONAL_ERROR_RESIDUAL_INTERSECTION_V2'
            ...
            or guard['evaluator']!=saved.sha(ROOT/'scripts/refine_n12_gate7_left_axis_directional_errors.py')
            or guard['residual_producer']!=saved.sha(ROOT/'scripts/reconstruct_n12_gate7_left_directional_residuals.py')
