"""Enclose one frozen trial derivative using all 75 uniform mixed directions."""
import os
...
p = base.p
ALGORITHM = 'UNIFORM_FROZEN_TRIAL_MEAN_VALUE_DERIVATIVE_ARB512_V1'
THEORY = base.ROOT/'theory/n12_gate7_trial_mean_value_derivative.md'
...
def evaluate(args):
    ...
    previous = directional['selected']
    selected = previous.copy()
    changed = base.select_first_variation(selected[:, 0], arrays['derivative_candidate'][:, 0])
    arrays.update(uniform_derivative=selected, previous_direct_uniform_derivative=previous)
