"""Integrate paired bootstrapped endpoint actions with the paired midpoint mean-value actions."""
import os
...
p = base.p
THEORY = base.ROOT/'theory/n12_gate7_bootstrap_endpoint_midpoint_local_integration.md'
ALGORITHM = 'BOOTSTRAP_ENDPOINT_MIDPOINT_MEAN_VALUE_LOCAL_COLUMN_ARB512_V1'


def evaluate(args):
    source = base.reader.load_inputs(args.interval)
    previous_record, previous, previous_files = base.read_pair(args.previous_pair)
    if (previous_record.get('algorithm') != 'MEAN_VALUE_ENDPOINT_AND_MIDPOINT_CENTER_LOCAL_COLUMN_ARB512_V1'
    ...
    for side, name in (('left', 'DL'), ('right', 'DR')):
        ...
        for stage in ('endpoint', 'midpoint'):
            if stage == 'endpoint':
                record, values, files = base.read_pair(args.endpoint_left_pair if side=='left' else args.endpoint_right_pair)
                report = record.get('report', {})
                if (record.get('algorithm') != 'BOOTSTRAP_UNIFORM_FROZEN_TRIAL_MEAN_VALUE_DERIVATIVE_ARB512_V1'
                        or record.get('interval') != args.interval or record.get('trial_column') != args.column
                        or record.get('side') != side or report.get('paired_mean_value_seed_bootstrap') is not True
    ...
    return source, arrays, dict(columns=reports, paired_center_split_operands_used=True,
        new_action_derivative_evaluations=0, paired_uniform_mean_value_endpoint_actions_used=True, paired_endpoint_seed_bootstrap_actions_used=True,
...
def main():
    ...
    parser.add_argument('--endpoint-left-pair', type=Path, required=True)
    parser.add_argument('--endpoint-right-pair', type=Path, required=True)
    ...
    args.endpoint_left_pair, args.endpoint_right_pair = args.endpoint_left_pair.resolve(), args.endpoint_right_pair.resolve()
    args.previous_pair = args.previous_pair.resolve()
