...
def evaluate(root,parent_path,refined_path,out,components,reference=None):
    ...
    if reference is not None:
        ...
        if (reference_sources.get('evaluator') not in {saved.sha(ROOT/'scripts/certify_n12_gate7_left_base_residual_vector.py')}
    ...
    def action(name,legs):
        ...
        if cached is not None and cached.exists():
            ...
            raise ValueError('cache-only base replay requires every completed source-bound action')
    ...
    result=dict(algorithm='ORIGINAL_DOMAIN_BASE_IMPLICIT_RESIDUAL_VECTOR_V1',family=family,side='left',
