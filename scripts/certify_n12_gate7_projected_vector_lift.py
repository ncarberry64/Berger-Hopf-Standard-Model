...
def evaluate(root, scalar_path, coefficients_path):
    ...
    radii = [arb(v) for v in scalar['correction_radii_exact']]
    ...
    return dict(algorithm='EXACT_DESCRIPTOR_ELIMINATION_SHARED_VECTOR_LIFT_ARB512_V1',
        ...
        components=rows,vector_norm=dict(anchor_deviation=upper(total),linear=upper(linear_row_norm),nonlinear=upper(nonlinear_norm),constant=upper(constant_norm)),
