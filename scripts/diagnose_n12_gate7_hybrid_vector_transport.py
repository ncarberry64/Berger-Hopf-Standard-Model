...
def evaluate(root,midpoint_path,endpoint_path,adjoint_path,refined_path,out,midpoint_adjoint_path,midpoint_refined_path,midpoint_corrected,endpoint_corrected,export_models=False):
    ...
    result=dict(algorithm='FULL_INPUT_HYBRID_NUMERATOR_HS_TRANSPORT_V1',physical_input_columns=74,projected_output_rows=74,
        proof_assumptions=[
        ...
        all_61_velocity_residuals_cancelled_before_normalization_and_transport=all(
            json.loads((folder/'record.json').read_bytes())['all_61_velocity_residuals_cancelled']
            for folder in (midpoint_corrected,endpoint_corrected)),
        cancelled_velocity_components={name:json.loads((folder/'record.json').read_bytes())['cancelled_components']
            for name,folder in (('midpoint',midpoint_corrected),('endpoint',endpoint_corrected))},
...
def main():
    ...
    evaluate(args.evidence_root.resolve(),args.midpoint.resolve(),args.endpoint.resolve(),
             ...
             args.midpoint_refined.resolve(),args.midpoint_corrected.resolve(),args.endpoint_corrected.resolve(),args.export_models)
