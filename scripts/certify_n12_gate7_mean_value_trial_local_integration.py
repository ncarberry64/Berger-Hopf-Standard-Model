"""Integrate paired uniform mean-value trial actions with the original midpoint evidence."""
import os
...
p = base.p
THEORY = base.ROOT/'theory/n12_gate7_mean_value_trial_local_integration.md'
ALGORITHM = 'MEAN_VALUE_ENDPOINT_TRIAL_FIXED_FRAME_LOCAL_COLUMN_ARB512_V1'


def evaluate(args):
    source = base.reader.load_inputs(args.interval)
    previous_record, previous, previous_files = base.read_pair(args.previous_pair)
    if (previous_record.get('algorithm') != 'PRECONDITIONED_CENTER_SPLIT_FROZEN_TRIAL_COLUMN_ARB512_V1'
            or previous_record.get('interval') != args.interval
            or previous_record.get('trial_column') != args.column
            or previous_record['report'].get('paired_center_split_operands_used') is not True):
    ...
    return source, arrays, dict(columns=reports, paired_center_split_operands_used=True,
        ...
        original_paired_midpoint_center_action_reused=True, center_rounding_remainder_included=True,
