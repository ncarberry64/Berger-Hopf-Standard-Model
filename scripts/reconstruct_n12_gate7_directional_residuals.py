...
def evaluate(root, family, adjoint_path, refined_path, out, components, reference=None):
    ...
    if reference is not None:
        ...
        allowed_sources={saved.sha(full_producer),saved.sha(directional_producer),saved.sha(ROOT/'scripts/certify_n12_gate7_directional_numerator_fast.py'),saved.sha(Path(__file__))}
    ...
    result=dict(normalizations_SHA256=saved.sha(normalization),normalization_equations=['psi_dot_psi_u','psi_dot_h_u_plus_psi_u_dot_h','derived_psi_u_multiplier_zero'],algorithm='FULL_INPUT_CONTRACTED_DIRECTIONAL_RESIDUALS_V1',family=family,components=list(components),
