"""Refine only endpoint-15 DF descriptor tail using the identity Y-beta G.
...
def evaluate(root,operands,predictors,base_path,direction_path,adjoint_path,out,workers):
    ...
    with cache.cache_hashes([(p.values,'sha'),(r.center,'_sha'),(r.foundation.coordinate.center,'_sha')],excluded_roots=[out]):
        ...
        if any(v['stage']!='endpoint' or v['interval']!=14 for v in (base,old,adj)):
            raise ValueError('Matching interval-14 endpoint graph required')
        ...
        n=323;domain=TaylorDomain(pre['parameter_groups']+[(75,n,'box')],n)
        radii=[arb(v) for key,record in (('eigenpair_error_upper_exact',base),('response_error_upper_exact',base),
                  ('axis_line_error_upper_exact',old),('axis_response_error_upper_exact',old)) for v in record[key]]
        ep=[arb(v).mid() for v in pre['point_checks'][0]['target_midpoints_rational']]
        def model(centers,derivative,offset):
            result=[]
            for i,c in enumerate(centers):
                coefficients=[derivative[i,j].mid() for j in range(75)]+[arb(0)]*248
                coefficients[75+offset+i]=radii[offset+i]
                result.append(domain.affine(c.mid(),coefficients))
            return result
        psi=model(ep[:61],s['point_solve_3'][:61],0)
        lam_data=base['eigenvalue_predictor'];lam_values=[arb(m)+arb(0,arb(r)) for m,r in lam_data]
        lc=lam_values[1:-1]+[arb(0)]*248;lc[75+61]=radii[61]
        lam=domain.affine(lam_values[0],lc)
        hard=model(s['point_solve_0'][:,0],s['point_solve_4'],62)
        pu=model(s['point_solve_1'][:,0],s['point_solve_5'],124)
        hu=model(s['point_solve_2'][:,0],s['point_solve_6'],186)
        ...
        state=[domain.affine(s['center_state'][i],[s['weighted_tube_directions'][i,j]/weights[i] for j in range(75)]+[arb(0)]*248) for i in range(98)]
        ...
        axis=s['input_axis_ball'];u=[axis[i]/weights[i] for i in range(98)]
        ...
        parameter_s=domain.affine(s['point_descriptor'][0],[s['weighted_tube_directions'][98,j] for j in range(75)]+[arb(0)]*248)
        ...
        Y=delta_u/norm-delta*dot(N,Nu)/norm**3
        W=Y-sum(G,domain.affine(0))
        original_values=[arb(m)+arb(0,arb(r)) for m,r in old['physical_derivative_models'][98]]
        original_affine=domain.affine(original_values[0],original_values[1:-1]+[arb(0)]*248)
        difference=W-original_affine;new_radius=difference.support()
        refinement=dict(algorithm='INTERVAL14_ENDPOINT_DESCRIPTOR_RESIDUAL_CANCELLED_TAYLOR_V1',
            interval=14,stage='endpoint',original_domain_unchanged=True,common_correction_parameters=248,
            ...
            refined_descriptor_remainder_upper_exact=str(new_radius.fmpq()),
            previous_descriptor_remainder_upper_exact=str(original_values[-1].upper().fmpq()),
            strict_improvement=bool(new_radius<original_values[-1]),
        ...
        if not refinement['strict_improvement']:raise ArithmeticError('Descriptor refinement did not improve the enclosure')
        updated=dict(old);updated['physical_derivative_models']=list(old['physical_derivative_models'])
        updated['physical_derivative_models'][98]=old['physical_derivative_models'][98][:-1]+[[str(new_radius.fmpq()),'0']]
        updated['descriptor_refinement_only']=True
        updated['input_source_SHA256']={**sources,str((out/'refinement.json').resolve()):sha(out/'refinement.json')}
        (out/'record.json').write_bytes(p.geometry.encoded(updated))
        print(json.dumps(dict(completed=True,old_descriptor_tail=float(original_values[-1]),new_descriptor_tail=float(new_radius))),flush=True)
