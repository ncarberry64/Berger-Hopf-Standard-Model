"""Apply paired primal value bounds to all 99 endpoint derivative columns."""
import argparse
import json
...
import certify_n12_gate7_coupled_endpoint_uniform_derivatives as baseline
...
def main():
    ...
    engine.WORK = ROOT/'artifacts/flagship_integration/.primal_mean_value_component_centered_endpoint_uniform_df_work'
    engine.ALGORITHM = 'PRIMAL_MEAN_VALUE_COMPONENT_CENTERED_ENDPOINT_UNIFORM_DF_ARB512_V1'
    p = engine.p
        ...
        for key, value in reference['binding'].items():
            if key == 'files':
                if any(record['binding']['files'].get(name) != digest for name, digest in value.items()):
                    raise ValueError('primal and derivative endpoint families differ')
