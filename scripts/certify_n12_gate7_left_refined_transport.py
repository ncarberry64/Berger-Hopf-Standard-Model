"""Assemble the left 74-input HS block from shared endpoint/midpoint models.
...
74-dimensional physical input and left endpoint state coordinates are shared.
...
import bhsm.interface.base_residual_cancellation as base_cancellation
import n12_gate7_base_residual_models as base_models
import n12_gate7_left_longitudinal_error_bounds as longitudinal_errors
...
def transport_coefficients(QP,Q,E,M,h,A0,U,frozen_left):
    from bhsm.interface.two_sided_input_transport import transport_coefficients as both
    return both(QP,Q,E,M,h,A0,U,side='left',frozen_left=frozen_left)
...
def evaluate(root,midpoint_path,endpoint_path,adjoint_path,refined_path,out,midpoint_adjoint_path,midpoint_refined_path,midpoint_corrected,endpoint_corrected,export_models=False,pullback_state_errors=False,midpoint_base_path=None,endpoint_base_path=None,midpoint_error_refinement=None,endpoint_error_refinement=None):
    ...
    import n12_gate7_left_corrected_numerator_models as corrected_models
    guarded_paths={
        ...
        'two_sided_transport':ROOT/'src/bhsm/interface/two_sided_input_transport.py',
    ...
    guarded_paths['base_cancellation_arithmetic']=Path(base_cancellation.__file__)
    guarded_paths['base_residual_loader']=Path(base_models.__file__)
    for name,folder in (('midpoint',midpoint_base_path),('endpoint',endpoint_base_path)):
        guarded_paths[name+'_base_residual_record']=folder/'record.json'
        guarded_paths[name+'_base_residual_models']=folder/'models.json.gz'
    guarded_paths['midpoint_error_refinement']=midpoint_error_refinement
    guarded_paths['endpoint_error_refinement']=endpoint_error_refinement
    guarded_paths['longitudinal_error_loader']=Path(longitudinal_errors.__file__)
    ...
    input_axis=[arb(float(v)) for v in axes[13]]
    ...
    fullfile=root/'artifacts/flagship_integration/.primal_mean_value_component_centered_endpoint_uniform_df_work/endpoint_013/derivative.npz'
    ...
    # Cancel the endpoint base graph before the interval midpoint matrix
    # acts. Its actual coefficients multiply the same zero residual; choosing
    # separate interval occurrences after transport would lose this identity.
    original_endpoint_W,original_endpoint_numerator=W,numerator
    local_endpoint_block=base_models.load_block(endpoint_base_path,endpoint_path,refined_path,
        domain,list(range(domain.dimension)),verified)
    W,endpoint_scalar_weights=local_endpoint_block.cancel(W)
    cancelled=[];endpoint_weights=[]
    for j,v in enumerate(numerator):
        polynomial,weighted=local_endpoint_block.cancel(v)
        cancelled.append(polynomial);endpoint_weights.append(weighted)
        print(json.dumps(dict(stage='endpoint_base_cancelled',component=j)),flush=True)
    numerator=cancelled
    endpoint_weight_matrix=arb_mat(98,124*198,[v for w in endpoint_weights for v in w.entries()])
    del endpoint_weights
    ...
    fixed,coefficient=transport_coefficients(QP,Q,E,M,h,A0,U_mid,base.matrix(local['frozen_left']))
    ...
    residual_blocks=[
        base_models.load_block(midpoint_base_path,midpoint_path,midpoint_refined_path,common,midpoint_state,verified),
]
    common_error_maps=[(74,midpoint_error_map),(198,endpoint_error_map)]
    def cancel_base(value):
        tails=[]
        for block in residual_blocks:
            value,weighted=block.cancel(value)
            tails.append(weighted)
        return value,arb_mat(sum(m.nrows() for m in tails),322,[v for m in tails for v in m.entries()])
    def boxed_tail(weighted):
        return sum((linear_support([weighted[i,j] for j in range(322)],input_groups)
                    for i in range(weighted.nrows())),arb(0)).upper()
    ...
    if any(not v.contains(0) for v in parameter_differences):
        raise ValueError('inherited left-endpoint coefficient enclosures must agree')
    # Source-bound endpoint and midpoint domains use the same left state
    ...
    longitudinal_axis=arb_mat(74,1,input_axis)
    longitudinal_direction=arb_mat(322,1,input_axis
        +(midpoint_error_map*longitudinal_axis).entries()
        +(endpoint_error_map*longitudinal_axis).entries())
    refined_midpoint_error=longitudinal_errors.load_direction(midpoint_error_refinement,midpoint_path,
        midpoint_adjoint_path,midpoint_refined_path,longitudinal_axis,midpoint_error_map,verified)
    refined_endpoint_error=longitudinal_errors.load_direction(endpoint_error_refinement,endpoint_path,
        adjoint_path,refined_path,longitudinal_axis,endpoint_error_map,verified)
    longitudinal_direction=arb_mat(322,1,input_axis+refined_midpoint_error.entries()+refined_endpoint_error.entries())
    ...
    with ExitStack() as stack:
        ...
        for i in range(74):
            ...
            endpoint_model=original_endpoint_W if i==73 else midratio*original_endpoint_W+dot(
                [L[i,j]-midratio*L[73,j] for j in range(98)],original_endpoint_numerator)
            ...
            uncorrected_axis_support=value.at_input(longitudinal_direction).support()
            value,midpoint_residual_weights=cancel_base(value)
            transported_endpoint_weights=ratio*endpoint_scalar_weights+arb_mat(124,198,
                (arb_mat(1,98,complement)*endpoint_weight_matrix).entries())
            embedded_endpoint_weights=arb_mat(124,322)
            for j,k in enumerate(endpoint_input):
                for row in range(124): embedded_endpoint_weights[row,k]=transported_endpoint_weights[row,j]
            residual_weights=arb_mat(248,322,midpoint_residual_weights.entries()+embedded_endpoint_weights.entries())
            added_axis_tail=base_cancellation.weighted_tail_at_input(residual_weights,longitudinal_direction)
            added_physical_tail=base_cancellation.weighted_tail_on_physical_input(residual_weights,common_error_maps)
            original_support=(value.support()+boxed_tail(residual_weights)).upper()
            longitudinal=value.at_input(longitudinal_direction)
            longitudinal=Taylor(common,longitudinal.c,longitudinal.a,longitudinal.r+added_axis_tail)
            ...
            value=InputLinearTaylor(common,value.c,value.a,value.r+added_physical_tail,input_groups)
            ...
            rows.append(dict(component=i,support=upper(support),linear=upper(value.linear_bound()),nonlinear=upper(value.r),
                             ...
                             uncorrected_longitudinal_input_support=upper(uncorrected_axis_support),
                             base_residual_axis_tail=upper(added_axis_tail),
                             base_residual_physical_input_tail=upper(added_physical_tail),
    ...
    long_fixed,long_coefficient=transport_coefficients(longQP,longQ,E,M,h,A0,U_mid,base.matrix(local['frozen_left']))
    ...
    long_endpoint_weights=ratio*endpoint_scalar_weights+arb_mat(124,198,
        (arb_mat(1,98,complement)*endpoint_weight_matrix).entries())
    ...
    long_value,long_midpoint_weights=cancel_base(long_value)
    long_embedded_endpoint_weights=arb_mat(124,322)
    for j,k in enumerate(endpoint_input):
        for row in range(124): long_embedded_endpoint_weights[row,k]=long_endpoint_weights[row,j]
    long_residual_weights=arb_mat(248,322,long_midpoint_weights.entries()+long_embedded_endpoint_weights.entries())
    long_axis_tail=base_cancellation.weighted_tail_at_input(long_residual_weights,longitudinal_direction)
    long_physical_tail=base_cancellation.weighted_tail_on_physical_input(long_residual_weights,common_error_maps)
    long_original=long_value
    long_on_axis=(long_value.at_input(longitudinal_direction).support()+long_axis_tail).upper()
    ...
    long_value=InputLinearTaylor(common,long_value.c,long_value.a,long_value.r+long_physical_tail,input_groups)
    ...
    result=dict(algorithm='LEFT_FULL_INPUT_REFINED_DIRECTIONAL_HS_TRANSPORT_V1',side='left',
        input_axis_node=13,output_axis_node=14,physical_input_columns=74,projected_output_rows=74,
        proof_assumptions=[
        ...
        identity='QP(frozen_left+I)E+(h/6)QPA0+Bmid+PM(E/2+hA0/8-U_mid)+[(h/6)QP+(h/8)PM](A-A0)',
        ...
        all_248_base_residuals_used=True,endpoint_base_cancelled_before_interval_midpoint_transport=True,
        base_residuals_cancelled_before_common_input_substitution=True,
        added_base_residual_remainders_retained_until_input_substitution=True,
        ...
        complete_local_left_block_norm_upper=upper(total),strict_local_gain_below_one=bool(total<1),rows=rows,
        longitudinal_input_transverse_output_norm_upper=upper(vector_norm(longitudinal_bounds)),
        longitudinal_input_axis_norm=upper(vector_norm(input_axis)),
        longitudinal_input_uses_original_directional_error_maps=True,
        longitudinal_errors_intersected_using_shared_implicit_residuals=True,
        ...
        source_hashes={**verified['paired_source_hashes'],
            ...
            'base_cancellation_arithmetic':saved.sha(Path(base_cancellation.__file__)),
            'base_residual_loader':saved.sha(Path(base_models.__file__)),
            'midpoint_base_residual_record':saved.sha(midpoint_base_path/'record.json'),
            'endpoint_base_residual_record':saved.sha(endpoint_base_path/'record.json')})
    (out/'record.json').write_bytes(saved.encoded(result))
    print(json.dumps(dict(local_left_block_norm_upper=float(total),Gate7_closed=False)),flush=True)
...
def main():
    ...
    parser.add_argument('--midpoint-base-residual',type=Path,required=True)
    parser.add_argument('--endpoint-base-residual',type=Path,required=True)
    parser.add_argument('--midpoint-error-refinement',type=Path,required=True)
    parser.add_argument('--endpoint-error-refinement',type=Path,required=True)
    ...
    evaluate(args.evidence_root.resolve(),args.midpoint.resolve(),args.endpoint.resolve(),
             ...
             args.midpoint_refined.resolve(),args.midpoint_corrected.resolve(),args.endpoint_corrected.resolve(),args.export_models,args.pullback_state_errors,args.midpoint_base_residual.resolve(),args.endpoint_base_residual.resolve(),args.midpoint_error_refinement.resolve(),args.endpoint_error_refinement.resolve())
