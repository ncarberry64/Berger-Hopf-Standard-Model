"""Diagnostic Hessian on one scaled, correlated transverse frame direction."""
import os
...
def evaluate(source,column,axis_column):
    ...
    if not 0<=axis_column<74:raise ValueError('transverse frame column in 0..73 required')
    residual=p.geometry.residual
    with np.load(residual.center.JACOBIAN.with_suffix('.npz'),allow_pickle=False) as data:
        frame=residual.center.cert._frame(data['endpoint_physical_tangent_action'][source['index']],
            residual.center.cert.TRIAL_DESCRIPTOR_SCALE)
    if np.shape(frame)!=(99,74):raise ArithmeticError('complete frozen augmented frame required')
    axis=np.array([arb(float(v)) for v in frame[:,axis_column]],dtype=object)*source['tube']['radius_transverse']
    ...
    if not all(a.contains(b) for a,b in zip(source['raw_domain'],raw_center+raw_axis,strict=True)):
        raise ArithmeticError('selected scaled frame direction must remain in the paired tube hull')
    ...
    return arrays,dict(point_eigenpair_checks=checks,solve_proofs=proofs,
        ...
        one_scaled_transverse_frame_direction_only=True,transverse_axis_column=axis_column,
...
def main():
    ...
    with cache.cache_hashes(targets,excluded_roots=[args.out]):
        ...
        try:arrays,proof=evaluate(source,args.column,args.axis_column)
        ...
        record=dict(binding=source['binding'],endpoint=args.endpoint,column=args.column,axis_column=args.axis_column,data_SHA256=p.values.sha(data),
