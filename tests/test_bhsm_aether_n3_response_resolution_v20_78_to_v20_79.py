...
def test_v21_isolated_eigenpair_event_hessian_and_corrected_continuation() -> None:
    ...
    for filename, key, expected in (
        ("BHSM_N3_THIRD_EXPANDED_RADIUS_REFRESHED_CONTINUATION_V21_25.json",
         "third_expanded_radius_refreshed_continuation", 0.780870640281120),
        ("BHSM_N3_THIRD_EXPANDED_PREDICTIVE_CONTINUATION_V21_26.json",
         "third_expanded_predictive_continuation", 0.780831062252768),
    ):
        payload = json.loads(Path("artifacts", filename).read_text(encoding="utf-8"))
        assert payload["validation_passed"]
        result = payload[key]
    ...
    for filename, key, expected in (
        ...
        ("BHSM_N3_FIFTH_EXPANDED_PREDICTIVE_CONTINUATION_V21_30.json",
         "fifth_expanded_predictive_continuation", 0.780382122748396),
    ):
        payload = json.loads(Path("artifacts", filename).read_text(encoding="utf-8"))
        assert payload["validation_passed"]
        result = payload[key]
    ...
    for filename, key, expected in (
        ("BHSM_N3_FOURTH_EXPANDED_RADIUS_REFRESHED_CONTINUATION_V21_27.json",
         "fourth_expanded_radius_refreshed_continuation", 0.780634951423550),
        ("BHSM_N3_FOURTH_EXPANDED_PREDICTIVE_CONTINUATION_V21_28.json",
         "fourth_expanded_predictive_continuation", 0.780582944226373),
    ):
        payload = json.loads(Path("artifacts", filename).read_text(encoding="utf-8"))
        assert payload["validation_passed"]
        result = payload[key]
