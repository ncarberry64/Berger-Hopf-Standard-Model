...
def test_y0_coordinates_not_fully_derived_by_default():
    mod = load_mod()
    data = load_results()
    assert mod.y0_coordinates_fully_derived() is False
    assert data["y0_coordinates_fully_derived"] is False
    assert "Y0_COORDINATES_REMAIN_OPEN" in data["verdict_labels"]
