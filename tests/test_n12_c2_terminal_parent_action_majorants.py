...
def test_extended_terminal_parent_action_majorants() -> None:
    extended = ROOT / (
        "artifacts/flagship_integration/"
        "BHSM_N12_C2_TERMINAL_PARENT_ACTION_MAJORANTS_2P0E10.json"
    )
    payload = json.loads(extended.read_text(encoding="utf-8"))
    assert payload["validation_passed"] is True
    assert payload["action_coordinate_ball_radius"] == 2.0e-10
    assert all(row["value_enclosed"] for row in payload["sectors"])
