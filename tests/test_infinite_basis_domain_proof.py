...
def test_ht_domain_bridge_strengthens_but_does_not_prove_ht():
    ...
    assert ht.v1_8_domain_bridge_status == HT_THEOREM_CONDITIONAL_ON_INDEX_MIRROR
...
def test_v18_exports_generate(tmp_path):
    ...
    assert json.loads(paths["bridge_json"].read_text())["domain_bridge_status"] == HT_THEOREM_CONDITIONAL_ON_INDEX_MIRROR
...
def test_generated_v18_artifacts_exist_and_are_conservative():
    ...
    assert "HT_THEOREM_CONDITIONAL_ON_INDEX_MIRROR" in text
