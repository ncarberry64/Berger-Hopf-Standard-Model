...
def test_artifact_report_separates_value_classes_and_keeps_runtime_targets_closed():
    ...
    assert "x_ch_production_vertex" not in report.missing_callables
    assert "cp_o_int_standalone_attachment" not in report.missing_callables
    assert "neutrino_physical_basis_scale" not in report.missing_callables
    formulas = {row["formula_key"]: row for row in report.callable_registry["formula_entries"]}
    assert formulas["x_ch_production_vertex"]["status"] == "RETIRED_TARGET"
    assert formulas["cp_o_int_standalone_attachment"]["status"] == "RETIRED_TARGET"
