...
def test_report_preserves_calibration_and_upper_limit_semantics() -> None:
    ...
    assert {CALIBRATION_WARNING, NEUTRINO_WARNING, CONDITIONAL_THEOREM_WARNING, RUNTIME_WARNING} <= set(report["warnings"])
    assert OPEN_THEOREM_WARNING not in report["warnings"]
...
def test_report_marks_blockers_and_runtime_gates() -> None:
    ...
    statuses = {row["particle_key"]: row["status"] for row in report["registry_statuses"]}
    assert statuses["charged_boundary_response_matrix"] == "CONDITIONAL_THEOREM"
    assert statuses["neutral_operator_kernel_BH"] == "CONDITIONAL_THEOREM"
    assert statuses["cp_holonomy_phase_attachment"] == "ARTIFACT_BACKED_CONSTRAINT"
    ...
    assert CONDITIONAL_THEOREM_WARNING in direct["warnings"]
...
def test_required_warnings_are_documented() -> None:
    ...
    for warning in (CALIBRATION_WARNING, NEUTRINO_WARNING, OPEN_THEOREM_WARNING, CONDITIONAL_THEOREM_WARNING, RUNTIME_WARNING):
