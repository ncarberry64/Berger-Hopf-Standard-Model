...
BHSM is an artifact-backed computational framework for Berger-Hopf boundary-mode physics. Current public status: v14.84 identifies the two shear strata with the existing reflected full-preimage caps and proves that the positive-semidefinite shear-softening sign survives for noncommuting cap inertia operators. Reflection covariance conditionally derives the normalized inertia fraction `nu=1/4`; it does not derive the actual reduced cap inertias, a physical nonzero relative transport, the degree-one self-adjoint background, the complete Hessian, CKM/PMNS, or particle observables. Mark III, Mark IV, and BHSM 1.0 release completion remain open. Frozen predictions and official prediction logic are unchanged.

## Current BHSM status — v14.84 cap-inertia theorem

Verdict: `BHSM_V14_84_THE_FULL_PREIMAGE_CAP_OPERATOR_PARALLEL_SUM_PRESERVES_POSITIVE_SEMIDEFINITE_DIFFERENTIAL_SHEAR_SOFTENING_AND_REFLECTION_CONDITIONALLY_DERIVES_THE_ONE_QUARTER_INERTIA_FRACTION_BUT_THE_CAP_CRITICAL_ACTIONS_PHYSICAL_RELATIVE_TRANSPORT_DEGREE_ONE_DOMAIN_AND_COMPLETE_HESSIAN_REMAIN_OPEN`. Exact next object: `ACTION_OWNED_FULL_PREIMAGE_TWO_STRATUM_KINETIC_REDUCTION_WITH_DERIVED_LAYER_INERTIAS_SHEAR_COVARIANCE_AND_DEGREE_ONE_SELF_ADJOINT_BACKGROUND`. See the [v14.84 operator theorem](docs/BHSM_FULL_PREIMAGE_CAP_INERTIA_OPERATOR_THEOREM_V14_84.md), [v14.83 recovery audit](docs/BHSM_MANUAL_CAMPAIGN_RECOVERY_V14_83.md), and [reviewer reproduction guide](docs/reviewer_reproduction_guide.md).
