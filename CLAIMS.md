...
Supported now: the event-to-complete-child correspondence is explicitly
implemented as 14 physical rows on 26 child variables. The complete child is
the reconstructed whole system, not an extra KKT coordinate or a static point.
At accepted frontiers its boundary traces, seven constraints, attachment
momentum, dynamic flux, eta domain, Calderon/core sectors, positive-duration
persistence, and nonzero relative motion pass. The child Jacobian has rank 14.
No equation 377 or empirical selector was added.

Supported now: the physical nonlinear problem remains the square 376-variable
KKT system with explicit event multiplier. Proposal models have no physical
authority. Exact unweighted corrected-Rayleigh `F376` decides merit; a fresh
complete-child solve decides physical promotion. Individual rows may worsen in
a coupled step, and leaving the previous iterate path is not a defect.

Supported snapshot: rolling checkpoint step 36 has exact
`||F376|| = 0.777030406838571`. Its fresh child has rank 14, eta minimum
`1.00003772292787`, two-scale flux envelope `1.6172791659e-5` below the
unchanged `2e-5` gate, maximum 14-row mismatch `6.51707072e-7`, and
persistence constraint `6.1803e-11`; trace, all constraints, momentum, flux,
persistence, and nonzero-motion gates pass. Frozen predictions are unchanged.

Validated numerical correction: near ordered-event degeneracy the legacy
event covector differed from the isolated-eigenvalue Rayleigh derivative by
approximately `25.0151%`. The corrected ordered-Rayleigh treatment changes the
derivative/evaluation machinery, not the physical event definition or KKT row
count. Legacy norms near `0.758...` and corrected-Rayleigh norms near
`0.787...` are different numerical series unless legacy states are reevaluated
with the corrected definition. Their apparent jump is not evidence of physical
regression.

Validated derivation: the terminal scale/v ordered-event second variation uses
the isolated-eigenpair formula with eigenvector response and the terminal
SBP/period second pullback. It reproduces the Rayleigh covector to `1.8e-14`,
is stable across the audited local derivative scales to `3.232284e-9`, and has
worst stable directional event-response discrepancy `1.185584e-4`. This
resolved `RAYLEIGH_EVENT_HESSIAN_TERMINAL_SCALE_V_ASSEMBLY` without changing
the event or acceptance gates.

Validated proposal workflow: bounded dual-metric range-space proposals use
accepted-history action-coordinate and physical-scaled radii. The rolling
driver was adopted only after exact reproduction of the manual v21.32 ->
v21.33 state. It then promoted 36 consecutive fresh-curvature states under the
unchanged exact-merit and complete-child gates. Fresh isolated-eigenpair
curvature is proposal information only; the rolling driver is workflow
automation, not new physics.

Validated/reclassified diagnostics:

- The historical plateau audit was Outcome E: the proposal mechanism stalled
  while the physical root remained open. Flux was oscillatory, no physical
  block owned at least 60%, no history-localized owner was found, and root-basin
  entry was not demonstrated.
- Structural hindsight was H5, `HINDSIGHT_SUBSPACE_NO_MATERIAL_RECOVERY`.
  Large accepted descents carried more scale/v/period and less shift, but the
  rank-14 child tangent supplied no new selector or equation.
- A structured shake recovered a useful collective proposal only after its
  temporary excitation was removed. Original exact `F376` and a fresh child
  alone decided promotion; no artificial force entered the physical solve.
- The latest one-time ownership audit is
  `DISTRIBUTED_DESCENT_CONTINUES`: period is largest at `45.9739%` but below
  dominance, scale and v continue falling, and stationarity weight is split
  approximately evenly between interior and event-near nodes. Period is not
  promoted to a physical blocker.

Numerically invalidated or insufficient as a closure mechanism: a common
coordinatewise event-Hessian finite-difference scale; the raw asymmetric 37x37
event block and naive symmetrization; an unbounded Moore-Penrose response step;
action-radius-only trust; unpreconditioned Krylov restarts 30 and 60; the
action-owned preconditioner as a sufficient cure; curvature transport;
corrected multisecants; and mandatory stale-curvature reuse. These are
proposal-model results, not failures of BHSM physics. No numerical rank cutoff,
trust radius, damping rule, preconditioner, or componentwise monotonicity rule
became a physical selector.

Not supported: a solved N=3 saddle, `F376 = 0`, a unique N=3 root, or an
approximately solved root. Also open are continuum N>=4 convergence,
reconstruction/return closure, absolute scale, mass matrices, CKM, PMNS, CP,
neutrino splittings, the absolute gauge spectrum, decay law, Unique
Actualization, empirical validation, and full BHSM completion.
...
- BHSM has artifact-backed adapters with provenance for CKM, PMNS, CP phase,
  boundary constants, and mass ratios where local artifacts support them.
- BHSM has executable theorem-closure machinery.
- BHSM has an ontology-aware executable minimal-action audit.
- BHSM modes are treated by the author ontology as physical boundary-local
  fields acted on by sector projectors.
- The CP/Z6 holonomy and CKM/PMNS phase attachment are artifact-backed; a
  standalone CP `O_int` production vertex is a retired target.
- `X_ch` is a conditional charged boundary-response action theorem under the
  author ontology.
...
- W is an independent prediction in a run where W calibrates the unit scale.
- Electron-neutrino mass is centrally measured or centrally validated.
- CP holonomy alone proves a standalone production interaction.
...
Status follows evidence. Artifact-backed means a local source and provenance
path exist. Candidate means an explicit construction exists with named proof
...
The v4.0 audit preserves `OPEN_MISSING_GAUGE_ACTION_COEFFICIENT_K`,
`OPEN_MISSING_ALPHA_I_ACTION_DERIVATION`, `OPEN_MISSING_G2_BH_ACTION_SOURCE`,
`OPEN_MISSING_CKM_COEFFICIENT_VALUE_SOURCE`, `OPEN_MISSING_NEUTRAL_ACTION_NORMALIZATION`,
`OPEN_MISSING_NEUTRAL_SCALE`, `DIMENSIONFUL_MASS_NOT_AVAILABLE`, and
`OPEN_MISSING_SCALAR_TOPOGRAPHIC_ACTION_SOURCE`. The CKM exponent remains `not_derived`,
and the completion conjunction remains `FULL_BHSM_NOT_COMPLETE`.
...
Allowed: BHSM v5.10 may claim a controlled partial effective-action audit, disjoint mode ownership, and the exact finite determinant of the single v5.7 homogeneous scalar/topographic mode orthogonal to retained `sigma_scale`. It may state the direct/zeta equality, explicit `mu` dependence, reduced force equations, and the absence of a finite positive `L` root in that partial system.

Result: `BHSM_QUANTUM_EFFECTIVE_ACTION_PARTIAL`.

This does not mean the full BHSM one-loop action, a finite Casimir remainder, total trace anomaly, RG-invariant transmutation scale, quantum stress tensor, nonlinear backreacted Berger geometry, or stable primordial size is derived. Gauge and ghost determinants, fermions and eta phase, geometry, charged and neutral sectors, full heat-kernel subtraction, coefficient running, and scheme closure remain unavailable.

Still not supported: using a candidate gauge Laplacian without ghosts; calling the finite homogeneous determinant a Casimir energy; promoting `mu`, a cutoff, box, subtraction point, wavefunction width, boundary location, or assumed coefficient to an absolute unit; restoring the old curvature-threshold mass gap; using `K[rho]=-Delta log rho` or its mass ansatz as an action result; deriving particle masses, gauge couplings, CKM values or exponent, rare-B observables, physical validation, white-hole observation, Yang-Mills mass gap, full Standard Model derivation, or full BHSM completion.

Preserved: `OPEN_MISSING_FULL_GAUGE_FIXED_DOMAIN`, `OPEN_MISSING_FADDEEV_POPOV_GHOST_OPERATOR`, `OPEN_MISSING_FULL_LOWER_ORDER_OPERATOR_TERMS`, `OPEN_MISSING_FERMION_DIRAC_OPERATOR_ACTION_SOURCE`, `OPEN_MISSING_FERMION_DETERMINANT_PHASE_ETA_INVARIANT`, `OPEN_MISSING_GEOMETRIC_FLUCTUATION_GAUGE_AND_DOMAIN`, `OPEN_MISSING_CHARGED_CURRENT_QUADRATIC_OPERATOR`, `OPEN_MISSING_NEUTRAL_RESPONSE_NORMALIZATION`, `OPEN_MISSING_COMPLETE_BERGER_HOPF_SPECTRAL_LEDGER`, `OPEN_MISSING_FIELD_THEORETIC_HEAT_KERNEL_COEFFICIENTS`, `OPEN_MISSING_RENORMALIZED_ACTION_COEFFICIENT_RUNNING`, `OPEN_MISSING_FINITE_CASIMIR_SPECTRAL_REMAINDER`, `OPEN_MISSING_TOTAL_TRACE_ANOMALY`, `OPEN_MISSING_NONLINEAR_GEOMETRIC_BACKREACTION`, `OPEN_MISSING_NONLINEAR_FULL_GEOMETRIC_BACKREACTION`, `OPEN_MISSING_GLOBAL_SCALE_MODULUS_ACTION_SOURCE`, `OPEN_MISSING_PRIMORDIAL_QUANTUM_BOUNDARY_STATE_CLOSURE`, `OPEN_MISSING_ABSOLUTE_UNIT_ANCHOR`, `OPEN_MISSING_GAUGE_COUPLING_ACTION_ATTACHMENT`, `OPEN_MISSING_ALPHA_I_ACTION_DERIVATION`, `OPEN_MISSING_G2_BH_ACTION_SOURCE`, `OPEN_MISSING_CKM_COEFFICIENT_VALUE_SOURCE`, `CKM_EXPONENT_NOT_DERIVED`, `OPEN_MISSING_NEUTRAL_SCALE`, and `FULL_BHSM_NOT_COMPLETE`.
...
## v6.6.0 constructive claim ledger

Adopted from established mathematics:

- the compact-open/k-space mapping-space adjunction;
- `pi4(S3)=Z2`, generated by the suspended Hopf map;
- FR quantization by characters of the configuration-space fundamental group;
- self-adjoint first-order elliptic boundary-domain machinery.

Adopted BHSM identifications and action inputs:

- `Q_N=Map_*^N(S3,S3)` as the simplest explicit matter configuration space;
- the nontrivial FR sign character;
- Architecture B's minimal local boundary invariant
  `C_BHSM+y_sigma sigma Gamma_star`;
- exactly one dimensionless primitive, `y_sigma`.

Derived or numerically validated:

- `pi1(Q_N)=pi4(S3)=Z2` by mapping-space adjunction;
- odd/even `N` rotation, exchange, and internal-rotation signs `(-1)^N`;
- conjugation preserves FR parity;
- FR supplies a flat sign line bundle, not local Berry curvature or a local
  M4 kinetic operator;
- the selected compact diagnostic retains index one and a positive gap across
  the tested `y_sigma` sweep;
- a Hermitian `K_prop/(2E)` term gives a conditional `L/E` phase and unitary,
  reversible, common-phase-independent transport;
- a static flavor-dependent `A0` remains `L E^0`.

Rejected or not derived:

- deriving the local M4 action or `y_sigma` from FR topology;
- calling a constant Lorentz-invariant vacuum `K_prop` anything other than an
  operational mass-squared operator;
- an automatic map from the `S3` soliton field to `G2/SU3`;
- fitting PMNS, CKM, masses, couplings, or oscillation data.

Still requiring construction:

- a parent source for the local carrier and `y_sigma`;
- an action-derived, propagation-supported `K_prop`;
- the full sheet/sign/domain-resolved B1 compact spectrum;
- dynamic polarization, absolute scale, and the empirical prediction layer.
