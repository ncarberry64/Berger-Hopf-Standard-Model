"""Check analytic preconditions recorded by the retained paired tube certificates.

The caller must verify source identity, independent receipts and tube identity.
...
def validate_segment_family(eigen, value):
    ...
    if (value.get('validation_passed') is not True
            or value.get('uniform_physical_value_enclosed') is not True
            or value.get('positive_physical_G_norm') is not True
            or value.get('scope') != 'SELECTED_FROZEN_AFFINE_ENDPOINT_TUBE'
